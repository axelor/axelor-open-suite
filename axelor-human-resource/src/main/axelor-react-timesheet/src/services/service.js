...
class Services {
  ...
  constructor(props) {
    const baseURL = process.env.NODE_ENV === 'production' ? "../../" : "/open-suite-wip/";
    this.state = {
            ...
            if (body === null) {
              ...
              if (body.data) {
                ...
                if (record.timesheetLineList.length > 0) {
                  record.timesheetLineList.forEach(({ id }) => value.push(id));
                }
                criteria.push({ fieldName: 'id', operator: 'in', value });
                if (criteria.length === 0) {
                  resolve({ timeline: [], timesheetVersion: record.version,timesheet: record, timesheetId: id, fromDate: record.fromDate, toDate: record.toDate, user: record.user })
                }
                context.search('com.axelor.apps.hr.db.TimesheetLine',criteria, 0, -1, sortBy, []).then((body) => {
                  const timeline = [];
                  body.data.length &&
  ...
  search(entity, criteria = [], offset, limit, sortBy,fields = [], domain = null) {
    let body = JSON.stringify({
        "data": {
          _domain: domain,
          criteria: criteria,
