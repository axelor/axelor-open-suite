...
public class ExpenseController {
  ...
  @Inject private Provider<HRConfigService> hrConfigServiceProvider;
  @Inject private Provider<ExpenseService> expenseServiceProvider;
  @Inject private Provider<GeneralService> generalServiceProvider;
  ...
  public void computeKilometricExpense(ActionRequest request, ActionResponse response)
    ...
    if (expense != null && expenseLine.getUser() != null) {
      ...
      userId =
          request
              ...
              .toString();
      userName =
          request.getContext().getParentContext().asType(Expense.class).getUser().getFullName();
    }
    Employee employee =
    ...
    if (employee == null) {
      throw new AxelorException(
          String.format(I18n.get(IExceptionMessage.LEAVE_USER_EMPLOYEE), userName),
          IException.CONFIGURATION_ERROR);
    }
    ...
    try {
      ...
      if (kilometricAllowParamList == null || kilometricAllowParamList.isEmpty()) {
        ...
        response.setAttr(
            ...
            "domain",
            "self.id IN (" + StringTool.getIdFromCollection(kilometricAllowParamList) + ")");
      }
