...
public class ExpenseServiceImpl implements ExpenseService {
  ...
  public void validate(Expense expense) throws AxelorException {
    ...
    if (expense.getUser().getEmployee() == null) {
      throw new AxelorException(
          String.format(
              I18n.get(IExceptionMessage.LEAVE_USER_EMPLOYEE), expense.getUser().getFullName()),
          IException.CONFIGURATION_ERROR);
    }

    if (expense.getPeriod() == null) {
      throw new AxelorException(
          I18n.get(IExceptionMessage.EXPENSE_MISSING_PERIOD), IException.MISSING_FIELD);
    }
  ...
  public Move ventilate(Expense expense) throws AxelorException {
    ...
    moveLines.add(
        moveLineService.createMoveLine(
            move,
            expense.getUser().getPartner(),
            accountConfigService.getExpenseEmployeeAccount(accountConfig),
  ...
  public void addPayment(Expense expense, BankDetails bankDetails) throws AxelorException {
    ...
    if (paymentMode == null) {
      ...
      if (paymentMode == null) {
        throw new AxelorException(
            I18n.get(IExceptionMessage.EXPENSE_MISSING_PAYMENT_MODE), IException.MISSING_FIELD);
      }
      expense.setPaymentMode(paymentMode);
    }
  ...
  public void cancelPayment(Expense expense) throws AxelorException {
    ...
    if (bankOrder != null) {
      if (bankOrder.getStatusSelect() == BankOrderRepository.STATUS_CARRIED_OUT
        ...
        throw new AxelorException(
            I18n.get(IExceptionMessage.EXPENSE_PAYMENT_CANCEL), IException.FUNCTIONNAL);
      } else {
