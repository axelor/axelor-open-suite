...
public class TimesheetLineRestController {
  ...
  @DELETE
  @HttpExceptionHandler
  public Response removeAllTimesheetLine(TimesheetLineDeleteRequest requestBody) {
    new SecurityCheck()
    ...
    Beans.get(TimesheetLineTimesheetEditorService.class).removeAllTimesheetLines(timesheet);

    return ResponseConstructor.build(
        Response.Status.OK, I18n.get(ITranslation.TIMESHEET_LINES_DELETED), TimesheetLine.class);
  }
}
