...
public class EmployeeServiceImpl extends UserServiceImpl implements EmployeeService {
    ...
    if (dailyWorkHrs == null || dailyWorkHrs.compareTo(BigDecimal.ZERO) == 0) {
      ...
      if (dailyWorkHrs.compareTo(BigDecimal.ZERO) == 0) {
        throw new AxelorException(
            String.format(
                I18n.get(IExceptionMessage.TIMESHEET_EMPLOYEE_DAILY_WORK_HOURS),
    ...
    try {
      Years years =
          Years.yearsBetween(
              ...
              refDate == null ? Beans.get(GeneralService.class).getTodayDate() : refDate);
      return years.getYears();
    } catch (IllegalArgumentException e) {
      throw new AxelorException(
          String.format(I18n.get(IExceptionMessage.EMPLOYEE_NO_SENIORITY_DATE), employee.getName()),
          IException.NO_VALUE);
    }
  }
  ...
  public int getAge(Employee employee, LocalDate refDate) throws AxelorException {
    ...
    try {
      ...
      throw new AxelorException(
          String.format(I18n.get(IExceptionMessage.EMPLOYEE_NO_BIRTH_DATE), employee.getName()),
          IException.NO_VALUE);
    }
  }
