...
public class HumanResourceMobileController {
  ...
  public void insertKMExpenses(ActionRequest request, ActionResponse response)
    ...
    if (user != null) {
      ...
      expenseLine.setProjectTask(
          Beans.get(ProjectTaskRepository.class)
              .find(new Long(request.getData().get("projectTask").toString())));
      ...
      if (employee != null) {
        ...
        expenseLine.setKilometricAllowParam(
            kilometricAllowParamRepo.find(
                new Long(request.getData().get("kilometricAllowParam").toString())));
    ...
    try {
      ...
      Product product =
          Beans.get(ProductRepository.class)
              .find(new Long(request.getData().get("expenseProduct").toString()));
      if (user != null) {
        ...
        expenseLine.setUntaxedAmount(
            new BigDecimal(request.getData().get("amountWithoutVat").toString()));
      ...
      BigDecimal minutes =
          new BigDecimal(
              Minutes.minutesBetween(
                      new LocalTime(0, 0),
                      new LocalTime(request.getData().get("duration").toString()))
                  .getMinutes());
  ...
  public void insertLeave(ActionRequest request, ActionResponse response) throws AxelorException {
    ...
    LeaveReason leaveReason =
        Beans.get(LeaveReasonRepository.class)
            .find(new Long(request.getData().get("leaveReason").toString()));
  ...
  public void getExpensesTypes(ActionRequest request, ActionResponse response) {
    ...
    try {
      List<Product> productList =
          Beans.get(ProductRepository.class).all().filter("self.expense = true").fetch();
      for (Product product : productList) {
        Map<String, String> map = new HashMap<>();
        map.put("name", product.getName());
        map.put("id", product.getId().toString());
