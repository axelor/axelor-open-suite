...
public class ProjectTaskController {
    ...
    try {
      response.setValue(
          ...
          employeeService.getUserDuration(
              new BigDecimal(request.getContext().get("visibleDuration").toString()),
              AuthUtils.getUser(),
              true));
    } catch (Exception e) {
  ...
  public void setProjectVisibleDuration(ActionRequest request, ActionResponse response) {
    try {
      ...
      response.setValue(
          "$visibleDuration", employeeService.getUserDuration(project.getDuration(), user, false));
      response.setValue(
          "$visibleTimeSpent",
          employeeService.getUserDuration(project.getTimeSpent(), user, false));
      response.setValue(
          "$visibleLeadDelay",
          employeeService.getUserDuration(project.getLeadDelay(), user, false));
    } catch (Exception e) {
      TraceBackService.trace(response, e);
    }
  }
