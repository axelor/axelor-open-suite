...
public class TimesheetController {
  ...
  @Inject private Provider<ProjectTaskRepository> projectTaskRepoProvider;
  ...
  public void generateLines(ActionRequest request, ActionResponse response) throws AxelorException {
    ...
    if (projectTaskContext != null) {
      projectTask =
          projectTaskRepoProvider.get().find(((Integer) projectTaskContext.get("id")).longValue());
    }
    ...
    timesheet =
        timesheetServiceProvider
            .get()
            .generateLines(
                timesheet, fromGenerationDate, toGenerationDate, logTime, projectTask, product);
  ...
  public void historicTimesheet(ActionRequest request, ActionResponse response) {
    ...
    if (employee == null || !employee.getHrManager()) {
      actionView
          .domain(actionView.get().getDomain() + " AND self.user.employee.manager = :_user")
          .context("_user", user);
    }
  ...
  public void showSubordinateTimesheets(ActionRequest request, ActionResponse response) {
    ...
    String domain =
        "self.user.employee.manager.employee.manager = :_user AND self.company = :_activeCompany AND self.statusSelect = 2";
  ...
  public void setVisibleDuration(ActionRequest request, ActionResponse response) {
    try {
      ...
      response.setValue(
          "timesheetLineList", timesheetServiceProvider.get().computeVisibleDuration(timesheet));
    } catch (Exception e) {
      TraceBackService.trace(response, e);
    }
  }
  ...
  public void timesheetPeriodTotalController(ActionRequest request, ActionResponse response) {
    try {
      ...
      response.setAttr(
          "$periodTotalConvert",
          "value",
          Beans.get(EmployeeService.class).getUserDuration(periodTotal, user, false));
      response.setAttr(
          "$periodTotalConvert",
          "title",
          timesheetServiceProvider.get().getPeriodTotalConvertTitleByUserPref(user));
    } catch (Exception e) {
      TraceBackService.trace(response, e);
    }
  }
