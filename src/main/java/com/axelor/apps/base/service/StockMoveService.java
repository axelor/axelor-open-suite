...
public class StockMoveService {
	...
	public String getSequenceStockMove(Location fromLocation, Location toLocation, Company company) throws AxelorException {
		...
		if (ref == null || ref.isEmpty() || ref.equals(""))
			throw new AxelorException("Aucune séquence configurée pour les livraisons de la société "+company.getName(), IException.CONFIGURATION_ERROR);
		return ref;
	}
	...
	public StockMove createStocksMoves(Address toAddress, Company company, Partner clientPartner, Location location) throws AxelorException {
		...
		// Set the type select
		if(toLocation != null && fromLocation.getTypeSelect() == ILocation.INTERNAL && toLocation.getTypeSelect() == ILocation.INTERNAL) {
			stockMove.setTypeSelect(IStockMove.INTERNAL);
		}
		else if(toLocation != null && fromLocation.getTypeSelect() == ILocation.SUPPLIER && toLocation.getTypeSelect() == ILocation.INTERNAL) {	
			stockMove.setTypeSelect(IStockMove.INCOMING);
		}
		else if(toLocation != null && fromLocation.getTypeSelect() == ILocation.INTERNAL && toLocation.getTypeSelect() == ILocation.CUSTOMER) {
