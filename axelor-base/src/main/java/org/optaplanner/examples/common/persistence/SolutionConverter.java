...
public class SolutionConverter<Solution_> extends LoggingMain {
  ...
  public static <Solution_> SolutionConverter<Solution_> createImportConverter(
      String dataDirName,
      AbstractSolutionImporter<Solution_> importer,
      Class<Solution_> solutionClass) {
    SolutionFileIO<Solution_> inputSolutionFileIO =
        new SolutionFileIO<Solution_>() {
          @Override
    ...
    XStreamSolutionFileIO<Solution_> outputSolutionFileIO =
        new XStreamSolutionFileIO<>(solutionClass);
    return new SolutionConverter<>(
        dataDirName,
        inputSolutionFileIO,
        "import",
        importer.isInputFileDirectory(),
        outputSolutionFileIO,
        "unsolved");
  }

  public static <Solution_> SolutionConverter<Solution_> createExportConverter(
      String dataDirName,
      Class<Solution_> solutionClass,
      AbstractSolutionExporter<Solution_> exporter) {
    XStreamSolutionFileIO<Solution_> inputSolutionFileIO =
        new XStreamSolutionFileIO<>(solutionClass);
    SolutionFileIO<Solution_> outputSolutionFileIO =
        new SolutionFileIO<Solution_>() {
          @Override
          public String getInputFileExtension() {
            throw new UnsupportedOperationException();
    ...
    return new SolutionConverter<>(
        dataDirName, inputSolutionFileIO, "solved", false, outputSolutionFileIO, "export");
  }
  ...
  private SolutionConverter(
    ...
    if (!inputDir.exists()) {
      throw new IllegalStateException(
          "The directory inputDir (" + inputDir.getAbsolutePath() + ") does not exist.");
    }
  ...
  public void convert(String inputFileName) {
    String outputFileName =
        inputFileName.substring(
                0, inputFileName.length() - inputSolutionFileIO.getInputFileExtension().length())
  ...
  public void convert(String inputFileName, String outputFileName) {
    ...
    if (!inputFile.exists()) {
      throw new IllegalStateException(
          "The file inputFile (" + inputFile.getAbsolutePath() + ") does not exist.");
    }
