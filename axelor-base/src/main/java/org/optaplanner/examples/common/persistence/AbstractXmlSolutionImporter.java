...
public abstract class AbstractXmlSolutionImporter<Solution_>
  ...
  public Solution_ readSolution(File inputFile) {
    try (InputStream in = new BufferedInputStream(new FileInputStream(inputFile))) {
      ...
      throw new IllegalArgumentException(
          "Could not parse the XML file (" + inputFile.getName() + ").", e);
    }
  }

  public abstract static class XmlInputBuilder<Solution_> extends InputBuilder {
    ...
    protected void assertElementName(Element element, String name) {
      if (!element.getName().equals(name)) {
        throw new IllegalStateException(
            "Element name (" + element.getName() + ") should be " + name + ".");
      }
    }
  }
}
