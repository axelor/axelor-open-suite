...
public abstract class CommonApp<Solution_> extends LoggingMain {
  ...
  /**
   * The path to the data directory, preferably with unix slashes for portability. For example:
   * -D{@value #DATA_DIR_SYSTEM_PROPERTY}=sources/data/
   */
  ...
  public void init(Component centerForComponent, boolean exitOnClose) {
    ...
    solverAndPersistenceFrame =
        new SolverAndPersistenceFrame<>(solutionBusiness, createSolutionPanel());
  ...
  /**
   * Used for the unsolved and solved directories, not for the import and output directories, in the
   * data directory.
   *
   * @return never null
   */
