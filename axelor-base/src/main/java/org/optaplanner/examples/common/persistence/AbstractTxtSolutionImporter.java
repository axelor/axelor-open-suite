...
public abstract class AbstractTxtSolutionImporter<Solution_>
  ...
  public Solution_ readSolution(URL inputURL) {
    try (BufferedReader bufferedReader =
        new BufferedReader(new InputStreamReader(inputURL.openStream(), "UTF-8"))) {
      TxtInputBuilder<Solution_> txtInputBuilder = createTxtInputBuilder();
  ...
  public abstract static class TxtInputBuilder<Solution_> extends InputBuilder {
    ...
    public void readConstantLine(String constantRegex) throws IOException {
      ...
      if (line == null) {
        throw new IllegalArgumentException(
            "File ends before a line is expected to be a constant regex (" + constantRegex + ").");
      }
      String value = line.trim();
      if (!value.matches(constantRegex)) {
        throw new IllegalArgumentException(
            "Read line (" + line + ") is expected to be a constant regex (" + constantRegex + ").");
      }
    }
    ...
    public void readUntilConstantLine(String constantRegex) throws IOException {
      ...
      do {
        ...
        if (line == null) {
          throw new IllegalArgumentException(
              "File ends before a line is expected to be a constant regex ("
                  + constantRegex
                  + ").");
        }
        value = line.trim();
    ...
    public int readIntegerValue(String prefixRegex, String suffixRegex) throws IOException {
      ...
      if (line == null) {
        throw new IllegalArgumentException(
            "File ends before a line is expected to contain an integer value ("
                + prefixRegex
                + "<value>"
                + suffixRegex
                + ").");
      ...
      try {
        ...
        throw new IllegalArgumentException(
            "Read line (" + line + ") is expected to contain an integer value (" + value + ").", e);
      }
    }
    ...
    public String readStringValue(String prefixRegex, String suffixRegex) throws IOException {
      ...
      if (line == null) {
        throw new IllegalArgumentException(
            "File ends before a line is expected to contain an string value ("
                + prefixRegex
                + "<value>"
                + suffixRegex
                + ").");
    ...
    public String readOptionalStringValue(String prefixRegex, String defaultValue)
        throws IOException {
      return readOptionalStringValue(prefixRegex, "", defaultValue);
    }

    public String readOptionalStringValue(
        String prefixRegex, String suffixRegex, String defaultValue) throws IOException {
      bufferedReader.mark(1024);
      boolean valid = true;
    ...
    public String removePrefixSuffixFromLine(String line, String prefixRegex, String suffixRegex) {
      ...
      if (!value.matches("^" + prefixRegex + ".*")) {
        throw new IllegalArgumentException(
            "Read line ("
                + line
                + ") is expected to start with prefixRegex ("
                + prefixRegex
                + ").");
      }
      value = value.replaceAll("^" + prefixRegex + "(.*)", "$1");
      if (!value.matches(".*" + suffixRegex + "$")) {
        throw new IllegalArgumentException(
            "Read line (" + line + ") is expected to end with suffixRegex (" + suffixRegex + ").");
      }
      value = value.replaceAll("(.*)" + suffixRegex + "$", "$1");
      value = value.trim();
      return value;
    ...
    public String[] splitBySpace(
        String line, Integer minimumNumberOfTokens, Integer maximumNumberOfTokens) {
      return splitBy(
          line, "\\ ", "a space ( )", minimumNumberOfTokens, maximumNumberOfTokens, false, false);
    }
    ...
    public String[] splitBySpacesOrTabs(
        String line, Integer minimumNumberOfTokens, Integer maximumNumberOfTokens) {
      return splitBy(
          line,
          "[\\ \\t]+",
    ...
    public String[] splitByCommaAndTrim(
        String line, Integer minimumNumberOfTokens, Integer maximumNumberOfTokens) {
      return splitBy(
          line, "\\,", "a comma (,)", minimumNumberOfTokens, maximumNumberOfTokens, true, false);
    }
    ...
    public String[] splitBy(
        ...
        boolean removeQuotes) {
      return splitBy(
          line, delimiterRegex, delimiterName, numberOfTokens, numberOfTokens, trim, removeQuotes);
    }
    ...
    public String[] splitBy(
      ...
      if (removeQuotes) {
        ...
        for (int i = 0; i < lineTokens.length; i++) {
          ...
          while ((trim ? token.trim() : token).startsWith("\"")
              && !(trim ? token.trim() : token).endsWith("\"")) {
            i++;
            if (i >= lineTokens.length) {
              throw new IllegalArgumentException(
                  "The line (" + line + ") has an invalid use of quotes (\").");
            }
            String delimiter;
            switch (delimiterRegex) {
              ...
              default:
                throw new IllegalArgumentException(
                    "Not supported delimiterRegex (" + delimiterRegex + ")");
            }
      ...
      if (minimumNumberOfTokens != null && lineTokens.length < minimumNumberOfTokens) {
        throw new IllegalArgumentException(
            "Read line ("
                ...
                + ") has "
                + lineTokens.length
                + " tokens but is expected to contain at least "
                + minimumNumberOfTokens
                + " tokens separated by "
                + delimiterName
                + ".");
      ...
      if (maximumNumberOfTokens != null && lineTokens.length > maximumNumberOfTokens) {
        throw new IllegalArgumentException(
            "Read line ("
                ...
                + ") has "
                + lineTokens.length
                + " tokens but is expected to contain at most "
                + maximumNumberOfTokens
                + " tokens separated by "
                + delimiterName
                + ".");
    ...
    public boolean parseBooleanFromNumber(String token) {
      switch (token) {
        ...
        default:
          throw new IllegalArgumentException(
              "The token (" + token + ") is expected to be 0 or 1 representing a boolean.");
      }
    }
  }
}
