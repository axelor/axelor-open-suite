...
public abstract class AbstractXlsxSolutionImporter<Solution_>
  ...
  public abstract static class XslxInputBuilder<Solution_> extends InputBuilder {
      ...
      if (!sheet.getSheetName().equals(name)) {
        throw new IllegalArgumentException(
            "The sheet ("
                ...
                + ") at index ("
                + index
                + ") is expected to have another name ("
                + name
                + ")");
      }
      return sheet;
      ...
      if (!constant.equals(cell.getStringCellValue())) {
        throw new IllegalArgumentException(
            "The cell ("
                ...
                + ") is expected to have the constant ("
                + constant
                + ")");
      }
    }
    ...
    protected String readStringParameter(Row row, String key) {
      ...
      if (!key.equals(keyCell.getStringCellValue())) {
        throw new IllegalArgumentException(
            "The keyCell ("
                + keyCell.getRow().getRowNum()
                + ","
                + keyCell.getColumnIndex()
                + ") with value ("
                + keyCell.getStringCellValue()
                + ") is expected to have the key ("
                + key
                + ")");
      }
