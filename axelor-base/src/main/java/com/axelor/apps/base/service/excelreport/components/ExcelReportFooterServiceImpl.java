...
public class ExcelReportFooterServiceImpl implements ExcelReportFooterService {
  ...
  public Map<Integer, Map<String, Object>> getFooterInputMap(Workbook wb, Print print) {
    ...
    if (ObjectUtils.notEmpty(print.getPrintPdfFooter())) {
      ...
      font.setColor(
          Beans.get(ExcelReportCellService.class)
              .getCellFooterFontColor(print.getFooterFontColor()));
  ...
  public Sheet setFooter(
    ...
    int footerStartRow =
        Beans.get(ExcelReportCellMergingService.class)
                .getSheetLastRowNum(sheet, mergedCellsRangeAddressSetPerSheet)
            + 3;

    if (ObjectUtils.notEmpty(footerOutputMap)) {
      Beans.get(ExcelReportShiftingService.class).shiftAll(footerOutputMap, footerStartRow);
    }
    sheet =
        Beans.get(ExcelReportWriteService.class)
            .write(footerOutputMap, originSheet, sheet, 0, true);
