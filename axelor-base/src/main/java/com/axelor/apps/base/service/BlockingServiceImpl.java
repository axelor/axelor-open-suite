...
public class BlockingServiceImpl implements BlockingService {
  ...
  @Override
  public boolean isOngoing(Blocking blocking, Company company) {
    Objects.requireNonNull(blocking);

    if (blocking.getBlockingToDate() == null) {
      return true;
    } else {
      return !blocking
          .getBlockingToDate()
          .isBefore(Beans.get(AppBaseService.class).getTodayDate(company));
    }
  }
}
