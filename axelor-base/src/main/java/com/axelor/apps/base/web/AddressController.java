...
public class AddressController {
  ...
  public void check(ActionRequest request, ActionResponse response) {
    ...
    String msg =
        Beans.get(AddressService.class).check(g.getQasWsdlUrl())
            ? g.getQasWsdlUrl() + " " + I18n.get(IExceptionMessage.ADDRESS_1)
  ...
  public void validate(ActionRequest request, ActionResponse response) {
    ...
    Map<String, Object> retDict =
        Beans.get(AddressService.class)
            .validate(generalService.getGeneral().getQasWsdlUrl(), search);
    ...
    if (a.getPickList().size() > 0) {
      ...
      if (moniker != null) {
        com.qas.web_2005_02.Address address =
            Beans.get(AddressService.class)
                .select(generalService.getGeneral().getQasWsdlUrl(), moniker);
