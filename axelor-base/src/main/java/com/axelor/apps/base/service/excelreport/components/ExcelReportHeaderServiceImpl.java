...
public class ExcelReportHeaderServiceImpl implements ExcelReportHeaderService {
  ...
  public Sheet setHeader(
    ...
    if (ObjectUtils.notEmpty(headerOutputMap)) {
      ...
      sheet =
          Beans.get(ExcelReportWriteService.class)
              .write(headerOutputMap, originSheet, sheet, 0, true);
    }
    ...
    if (offset != 0) {
      Beans.get(ExcelReportShiftingService.class).shiftAll(outputMap, offset);
      Beans.get(ExcelReportCellMergingService.class)
          .shiftMergedRegions(mergedCellsRangeAddressSetPerSheet, offset);
    }
