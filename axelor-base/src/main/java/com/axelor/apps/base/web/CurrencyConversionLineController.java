...
public class CurrencyConversionLineController {
  ...
  @Inject private GeneralService gs;

  @Inject private GeneralRepository generalRepo;
  ...
  public void applyExchangeRate(ActionRequest request, ActionResponse response) {
    ...
    if (currencyFrom.get("id") != null && currencyTo.get("id") != null) {
      ...
      CurrencyConversionLine cclCoverd =
          cclRepo
              .all()
              .filter(
      ...
      if (cclCoverd == null) {
        ...
        if (ccl != null) variation = ccs.getVariations(rate, ccl.getExchangeRate());
        General general = null;

        if (context.get("general") != null && ((Map) context.get("general")).get("id") != null)
          general =
              generalRepo.find(Long.parseLong(((Map) context.get("general")).get("id").toString()));

        ccs.createCurrencyConversionLine(fromCurrency, toCurrency, today, rate, general, variation);

      } else response.setFlash(I18n.get(IExceptionMessage.CURRENCY_3));
    } else response.setFlash(I18n.get(IExceptionMessage.CURRENCY_5));
