...
public class IndicatorResultServiceImpl implements IndicatorResultService {
  ...
  @Inject
  public IndicatorResultServiceImpl(IndicatorResultRepository resultRepo) {
    this.resultRepo = resultRepo;
  }

  @Override
  public IndicatorResult initParent(IndicatorConfig config, LocalDateTime iterationDate) {
    IndicatorResult parent = new IndicatorResult();
    parent.setIndicatorConfig(config);
    parent.setDateT(iterationDate);
    return resultRepo.save(parent);
  }

  @Override
  public void updateQueryLog(IndicatorResult result, String message) {
    result.setQueryLog(message);
