...
/**
 * SchedulerService est une classe implémentant l'ensemble des services des planificateurs.
 *
 * @author P. Belloy
 * @version 1.0
 */
public class SchedulerService {
  ...
  @Inject private SchedulerInstanceRepository schedulerInstanceRepo;
   ...
   * @return boolean True si il est temps de lancer, sinon False
  ...
  public boolean isSchedulerInstanceIsReady(SchedulerInstance schedulerI) throws AxelorException {
    ...
    if (lastTheoreticalExecution == null) {
      ...
      if (firstExecution == null) {
        ...
        LOG.debug(
            String.format(
                "La facturation ne peux pas encore être exécuté : %s < %s", today, firstExecution));
      }
    } else {
  ...
  private boolean processScheduler(SchedulerInstance schedulerI) throws AxelorException {
    ...
    if (date != null && (date.equals(today) || date.isBefore(today))) return true;
   ...
   *
   * @param schedulerI Instance de planificateur
   * @return LocalDate Prochaine date d'éxécution
   * @throws AxelorException
   */
  public LocalDate getComputeDate(SchedulerInstance schedulerI) throws AxelorException {

    return this.getComputeDate(
        schedulerI.getScheduler(), schedulerI.getLastTheoreticalExecutionDate());
  }
  ...
  public LocalDate getComputeDate(Scheduler scheduler, LocalDate date) throws AxelorException {
    ...
    if (scheduler.getAnnually()) return this.getAnnualComputeDate(scheduler, date);
    else if (scheduler.getMonthly()) return this.getMonthlyComputeDate(scheduler, date);
    else if (scheduler.getWeekly()) return this.getWeeklyComputeDate(scheduler, date);
    else if (scheduler.getDaily()) return this.getDailyComputeDate(scheduler, date);
    else
      throw new AxelorException(
          String.format(I18n.get(IExceptionMessage.SCHEDULER_1), scheduler.getName()),
   ...
   *
   * @param scheduler Instance de planificateur
   * @param date Derniere date d'éxécution théorique
   * @return LocalDate Prochaine date d'éxécution
   */
  ...
  /**
   * Méthode qui détermine la prochaine date d'éxécution d'un planificateur pour un rythme
   * hebdomadaire
   *
   * @param scheduler Instance de planificateur
   * @param date Derniere date d'éxécution théorique
  ...
  private LocalDate getWeeklyComputeDate(Scheduler scheduler, LocalDate date) {
    ...
    if (scheduler.getMonday()) weekDay = DateTimeConstants.MONDAY;
    else if (scheduler.getTuesday()) weekDay = DateTimeConstants.TUESDAY;
    else if (scheduler.getWednesday()) weekDay = DateTimeConstants.WEDNESDAY;
    else if (scheduler.getThursday()) weekDay = DateTimeConstants.THURSDAY;
    else if (scheduler.getFriday()) weekDay = DateTimeConstants.FRIDAY;
    else if (scheduler.getSaturday()) weekDay = DateTimeConstants.SATURDAY;
    else if (scheduler.getSunday()) weekDay = DateTimeConstants.SUNDAY;
  ...
  private LocalDate getMonthlyComputeDate(Scheduler scheduler, LocalDate date) {
    ...
    if (scheduler.getDayMonthly() == 0)
      ...
      if (start <= scheduler.getDayMonthly() && scheduler.getDayMonthly() <= end)
        return date.plusMonths(scheduler.getMonthMonthly())
            .withDayOfMonth(scheduler.getDayMonthly());
  ...
  private LocalDate getAnnualComputeDate(Scheduler scheduler, LocalDate date) {
    ...
    if (monthOfYear != 0) {
      ...
      if (start <= scheduler.getDayAnnually() && scheduler.getDayAnnually() <= end)
        return date.plusYears(scheduler.getYearAnnually())
  ...
  public void addInHistory(SchedulerInstance schedulerI, LocalDate currentDay, boolean isImmediate)
      throws AxelorException {
  ...
  public LocalDate getTheoricalExecutionDate(SchedulerInstance schedulerI) throws AxelorException {
    ...
    } else theoricalExecutionDate = schedulerI.getFirstExecutionDate();
   ...
   *
   * @param localDate Date de référence
   * @param dayMonthly Jour du mois
   * @return LocalDate Date correspondant au prochain jour du mois
   */
  public LocalDate getNextDayMonth(LocalDate localDate, int dayMonthly) {
    ...
    if (localDate.dayOfMonth().get() <= dayMonthly) {
      ...
      if (start <= dayMonthly && dayMonthly <= end) date = localDate.withDayOfMonth(dayMonthly);
      else if (dayMonthly < start) date = localDate.dayOfMonth().withMinimumValue();
      else if (dayMonthly > end) date = localDate.dayOfMonth().withMaximumValue();
      ...
      else if (dayMonthly > endNext) date = localDate.plusMonths(1).dayOfMonth().withMaximumValue();
    }
