...
public class IndicatorEvaluationServiceImpl implements IndicatorEvaluationService {
  ...
  public void evaluateTarget(
    ...
    try {
      ExpressionEvaluator groovyEvaluator =
          evaluatorRegistry.get(IndicatorConfigRepository.EXPRESSION_TYPE_GROOVY);

      String modelVar = config.getTargetModel().getName();
      Optional<BigDecimal> value =
          groovyEvaluator.evaluate(targetExpression, model, modelVar, params);
  ...
  public void evaluateAlert(
    ...
    try {
      ExpressionEvaluator groovyEvaluator =
          evaluatorRegistry.get(IndicatorConfigRepository.EXPRESSION_TYPE_GROOVY);
      if (!(groovyEvaluator instanceof GroovyExpressionEvaluator)) {
        return;
      }
      ...
      boolean matches =
          ((GroovyExpressionEvaluator) groovyEvaluator).test(alertExpression, bindings);
