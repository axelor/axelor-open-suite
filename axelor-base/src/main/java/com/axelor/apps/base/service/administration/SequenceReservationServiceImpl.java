...
public class SequenceReservationServiceImpl implements SequenceReservationService {
  ...
  protected ReservedSequence findReleasedSequence(Sequence sequence, LocalDate refDate) {
    ...
    return JPA.em()
        ...
        .setParameter("sequence", sequence).setParameter("version", version)
        .setParameter("status", ReservedSequenceRepository.STATUS_RELEASED)
        .setLockMode(LockModeType.PESSIMISTIC_WRITE).setHint("javax.persistence.lock.timeout", 5000)
        .setFlushMode(FlushModeType.COMMIT).setMaxResults(1).getResultList().stream()
        .findFirst()
        .orElse(null);
  }
