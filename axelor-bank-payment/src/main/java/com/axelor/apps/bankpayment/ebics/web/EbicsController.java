...
public class EbicsController {
  ...
  @Inject private EbicsBankRepository bankRepo;

  @Inject private EbicsCertificateService certificateService;

  @Inject private EbicsCertificateRepository certificateRepo;
  ...
  public void addCertificates(ActionRequest request, ActionResponse response)
    ...
    try {
      ...
      certificateService.createCertificate(
          certificate, bank, EbicsCertificateRepository.TYPE_ENCRYPTION);

      certificate =
          certificateService.convertToCertificate((String) context.get("certificateX002"));
      certificateService.createCertificate(
          certificate, bank, EbicsCertificateRepository.TYPE_AUTHENTICATION);

    } catch (CertificateException | IOException e) {
      e.printStackTrace();
      throw new AxelorException(
          I18n.get("Error in adding bank certificate"), IException.CONFIGURATION_ERROR);
    }
  ...
  public void loadCertificate(ActionRequest request, ActionResponse response)
    ...
    if (certs != null && certs.length > 0) {
      X509Certificate certificate =
          EbicsCertificateService.getCertificate(certs, cert.getTypeSelect());
