...
public class BankOrderMergeServiceImpl implements BankOrderMergeService {
  ...
  public BankOrder mergeBankOrderList(List<BankOrder> bankOrderList) throws AxelorException {
    ...
    if (bankOrderList == null || bankOrderList.size() <= 1) {
      throw new AxelorException(
          I18n.get(IExceptionMessage.BANK_ORDER_MERGE_AT_LEAST_TWO_BANK_ORDERS),
  ...
  protected void checkSameElements(List<BankOrder> bankOrderList) throws AxelorException {
    ...
    for (BankOrder bankOrder : bankOrderList) {
      ...
      if (statusSelect != BankOrderRepository.STATUS_DRAFT
        ...
        throw new AxelorException(
            I18n.get(IExceptionMessage.BANK_ORDER_MERGE_STATUS), IException.INCONSISTENCY);
      }

      if (statusSelect != refStatusSelect) {
        throw new AxelorException(
            I18n.get(IExceptionMessage.BANK_ORDER_MERGE_SAME_STATUS), IException.INCONSISTENCY);
      }
      ...
      if (bankOrder.getIsMultiCurrency() != isMultiCurrency
        ...
        throw new AxelorException(
            I18n.get(IExceptionMessage.BANK_ORDER_MERGE_SAME_CURRENCY), IException.INCONSISTENCY);
      }
    }
  }
  ...
  public BankOrder mergeFromInvoicePayments(List<InvoicePayment> invoicePaymentList)
      throws AxelorException {

    if (invoicePaymentList == null || invoicePaymentList.isEmpty()) {
      throw new AxelorException(
          I18n.get(IExceptionMessage.BANK_ORDER_MERGE_NO_BANK_ORDERS), IException.INCONSISTENCY);
    }
