...
public class BankStatementController {
	...
	@Inject
	public BankStatementController(BankStatementService bankStatementService, BankStatementRepository bankStatementRepo,
	...
	public void runImport(ActionRequest request, ActionResponse response) {
		try {
			...
			// Load lines
			initialLines = bankStatementLineAFB120Repo.all()
					.filter("self.bankStatement = :bankStatement AND self.lineTypeSelect = :lineTypeSelect")
					.bind("bankStatement", bankStatement)
					.bind("lineTypeSelect", BankStatementLineAFB120Repository.LINE_TYPE_INITIAL_BALANCE).fetch();
			finalLines = bankStatementLineAFB120Repo.all()
					.filter("self.bankStatement = :bankStatement AND self.lineTypeSelect = :lineTypeSelect")
					.bind("bankStatement", bankStatement)
					.bind("lineTypeSelect", BankStatementLineAFB120Repository.LINE_TYPE_FINAL_BALANCE).fetch();

			// Check doublons
			for (BankStatementLineAFB120 bslAFB120 : initialLines) {
				tempBankStatementLineAFB120 = bankStatementLineAFB120Repo.all().filter(
						...
						.bind("bankDetails", bslAFB120.getBankDetails()).fetchOne();
			...
			if (!alreadyImported) {
				// Check is following
				bankDetails = bankStatementLineAFB120Repo.all().filter("self.bankStatement = :bankStatement")
						.bind("bankStatement", bankStatement).order("-id").fetchOne().getBankDetails();

				initialBankStatementLineAFB120 = bankStatementLineAFB120Repo.all()
						.filter("self.bankStatement = :bankStatement AND self.lineTypeSelect = :lineTypeSelect")
						.bind("bankStatement", bankStatement)
						.bind("lineTypeSelect", BankStatementLineAFB120Repository.LINE_TYPE_INITIAL_BALANCE)
						.order("sequence").fetchOne();
				finalBankStatementLineAFB120 = bankStatementLineAFB120Repo.all().filter(
						"self.bankStatement != :bankStatement AND self.bankDetails = :bankDetails AND self.lineTypeSelect = :lineTypeSelect")
						.bind("bankStatement", bankStatement).bind("bankDetails", bankDetails)
						.bind("lineTypeSelect", BankStatementLineAFB120Repository.LINE_TYPE_FINAL_BALANCE).order("-id")
						.fetchOne();
				if(ObjectUtils.notEmpty(finalBankStatementLineAFB120))
				if (initialBankStatementLineAFB120.getCredit().equals(finalBankStatementLineAFB120.getCredit())
						&& initialBankStatementLineAFB120.getDebit().equals(finalBankStatementLineAFB120.getDebit()))
					bankStatementService.generateMoves(bankStatement);
				else {
					// delete imported
					response.setError(I18n.get("Current bank statement's initial balance does not match previous bank statement's final balance"));
					bankStatementService.deleteBankStatementLines(bankStatementRepo.find(bankStatement.getId()));
				}
			} else {
				// Delete imported
	...
	public void generateMovesFromBankStatement(ActionRequest request, ActionResponse response) {
		BankStatement bankStatement = bankStatementRepo.find(request.getContext().asType(BankStatement.class).getId());
		bankStatementService.generateMoves(bankStatement);
	}
}
