...
public class BankStatementLineCreateCAMT53Service extends BankStatementLineCreateAbstractService {
        ...
        try {
            ...
            if (acct != null) {
                ...
                if (currencyCodeFromStmt == null) {
                    currencyCodeFromStmt = bankDetails.getCompany().getCurrency().getCode();
                }
            }
            ...
            if (balList != null && !balList.isEmpty()) {
                for (CashBalance3 balanceEntry : balList) {
                    sequence =
                            createBalanceLine(bankDetails, balanceEntry, sequence, BALANCE_TYPE_INITIAL_BALANCE, currencyCodeFromStmt);
                    if (sequence % 10 == 0) {
                        JPA.clear();
                        findBankStatement();
                    }
            ...
            if (balList != null && !balList.isEmpty()) {
                for (CashBalance3 balanceEntry : balList) {
                    sequence =
                            createBalanceLine(bankDetails, balanceEntry, sequence, BALANCE_TYPE_FINAL_BALANCE, currencyCodeFromStmt);
                    if (sequence % 10 == 0) {
                        JPA.clear();
                        findBankStatement();
                    }
    ...
    @Transactional
    protected int createEntryLine(BankDetails bankDetails, ReportEntry2 ntry, int sequence, String currencyCodeFromStmt) {
        ...
        if (CREDIT_DEBIT_INDICATOR_CREDIT.equals(creditOrDebit)) {
            credit =
                    Optional.of(ntry)
                            ...
                            .orElse(null);
            if (!currencyCodeFromStmt.equals(currencyCode)) {
    ...
    */
        ...
        if (description.length() >= 1 && description.charAt(description.length() - 1) == ';') {
