...
public class BankStatementFileAFB120Service extends BankStatementFileService  {
	...
	public void process() throws IOException, AxelorException  {
		...
		for(Map<String, Object> structuredContentLine : structuredContentFile)  {
			...
			try {
				...
				TraceBackService.trace(new Exception(String.format("Line %s : %s", sequence, e), e), IException.IMPORT);
			}
			finally {
				if(sequence % 10 == 0)  {
					JPA.clear();
	...
	public void createBankStatementLine (Map<String, Object> structuredContentLine, int sequence)  {
		...
		BankStatementLineAFB120 bankStatementLineAFB120 = bankStatementLineAFB120Service.createBankStatementLine(
	...
	protected List<Map<String, Object>> readFile() throws IOException, AxelorException  {
		...
		for(String lineContent : fileContent)  {
			...
			// Code enregistrement
			String operationCode = cfonbToolService.readZone("Record code", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_NUMERIC, 1, 2);
			...
			switch (operationCode) {
				...
				case COMPLEMENT_MOVEMENT_OPERATION_CODE:
					...
					additionalInformation += (String) readAdditionalMovementRecord(lineContent).get("additionalInformation");
	...
	protected Map<String, Object> readPreviousBalanceRecord(String lineContent) throws AxelorException  {
		...
		structuredLineContent.put("lineType", BankStatementLineAFB120Repository.LINE_TYPE_INITIAL_BALANCE);
		
		// Zone 1-B : Code banque
		String bankCode = cfonbToolService.readZone("1-B : bank code", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_NUMERIC, 3, 5);

		// Zone 1-D : Code guichet
		String sortCode = cfonbToolService.readZone("1-D : sort code", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_NUMERIC, 12, 5);
		
		// Zone 1-E : Code devise ISO
		String currencyCode = cfonbToolService.readZone("1-E : currency code", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_ALPHA, 17, 3);
		...
		String date = cfonbToolService.readZone("1-J : date", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_NUMERIC, 35, 6);
	...
	protected Map<String, Object> readMovementRecord(String lineContent) throws AxelorException  {
		...
		String bankCode = cfonbToolService.readZone("2-B : bank code", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_NUMERIC, 3, 5);

		// Zone 2-D : Code guichet
		String sortCode = cfonbToolService.readZone("2-D : sort code", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_NUMERIC, 12, 5);
		
		// Zone 2-E : Code devise ISO
		String currencyCode =  cfonbToolService.readZone("2-E : currency code", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_ALPHA, 17, 3);
		structuredLineContent.put("currency", getCurrency(currencyCode));
		...
		String operationInterbankCode = cfonbToolService.readZone("2-I : interbank operation code", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_ALPHA_NUMERIC, 33, 2);
		structuredLineContent.put("operationInterbankCodeLine", getInterbankCodeLine(operationInterbankCode)); 

		// Zone 2-J : Date de comptabilisation de l'opération (JJMMAA)  
		String movementDate = cfonbToolService.readZone("2-J : operation date", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_NUMERIC, 35, 6);
		structuredLineContent.put("operationDate", getDate(movementDate)); 
		
		// Zone 2-K : Code motif de rejet
		String rejectInterbankCodeLine = cfonbToolService.readZone("2-K : interbank reject code", lineContent, cfonbToolService.STATUS_DEPENDENT, cfonbToolService.FORMAT_NUMERIC, 41, 2);
		structuredLineContent.put("rejectInterbankCodeLine", getInterbankCodeLine(rejectInterbankCodeLine)); 
		
		// Zone 2-L : Date de valeur (JJMMAA)  
		String valueDate = cfonbToolService.readZone("2-L : value date", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_NUMERIC, 43, 6);
		structuredLineContent.put("valueDate", getDate(valueDate)); 

		// Zone 2-M : Libellé  
		structuredLineContent.put("description", cfonbToolService.readZone("2-M : label", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_ALPHA_NUMERIC, 49, 31));
		
		// Zone 2-O : Numéro d'écriture  
		structuredLineContent.put("origin", cfonbToolService.readZone("2-O : move number", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_ALPHA_NUMERIC, 82, 7));
		
		// Zone 2-P : Indice d'exonération de commission de mouvement de compte   
		structuredLineContent.put("commissionExemptionIndexSelect", cfonbToolService.readZone("2-P : turnover commission exemption index", lineContent, cfonbToolService.STATUS_OPTIONAL, cfonbToolService.FORMAT_ALPHA_NUMERIC, 89, 1));
		
		// Zone 2-Q : Indice d'indisponibilité 
		structuredLineContent.put("unavailabilityIndexSelect", cfonbToolService.readZone("2-Q : unavailability index", lineContent, cfonbToolService.STATUS_OPTIONAL, cfonbToolService.FORMAT_ALPHA_NUMERIC, 90, 1));
		...
		structuredLineContent.put("reference", cfonbToolService.readZone("2-S : reference zone", lineContent, cfonbToolService.STATUS_OPTIONAL, cfonbToolService.FORMAT_ALPHA_NUMERIC, 105, 16));
	...
	protected Map<String, Object> readAdditionalMovementRecord(String lineContent) throws AxelorException  {
		...
		String bankCode = cfonbToolService.readZone("2b-B : bank code", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_NUMERIC, 3, 5);

		// Zone 2b-D : Code guichet
		String sortCode = cfonbToolService.readZone("2b-D : sort code", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_NUMERIC, 12, 5);
		
		// Zone 2b-E : Code devise ISO
		String currencyCode =  cfonbToolService.readZone("2b-E : currency code", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_ALPHA, 17, 3);
		structuredLineContent.put("currency", getCurrency(currencyCode));
		
		// Zone 2b-F : Nombre de décimales du montant du mouvement
		cfonbToolService.readZone("2b-F : decimal number", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_NUMERIC, 20, 1);
		
		// Zone 2b-H : Numéro de compte 
		String accountNumber = cfonbToolService.readZone("2b-H : account number", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_ALPHA_NUMERIC, 22, 11);
		
		structuredLineContent.put("bankDetails", getBankDetails(accountNumber, bankCode, sortCode));
		
		// Zone 2b-I : Code opération interbancaire
		String operationInterbankCode = cfonbToolService.readZone("2b-I : interbank operation code", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_ALPHA_NUMERIC, 33, 2);
		structuredLineContent.put("operationInterbankCodeLine", getInterbankCodeLine(operationInterbankCode)); 

		// Zone 2b-J : Date de comptabilisation de l'opération (JJMMAA)  
		String date = cfonbToolService.readZone("2b-J : operation date", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_NUMERIC, 35, 6);
		structuredLineContent.put("operationDate", getDate(date)); 
		
		// Zone 2b-L : Qualifiant de la zone "Informations complémentaires"  
		String additionalInformationType = cfonbToolService.readZone("2b-L : qualifying of additional information zone", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_ALPHA_NUMERIC, 46, 3);
		
		switch (additionalInformationType) {
		case "LIB":
			// Zone 2b-M : Informations complémentaires  
			structuredLineContent.put("additionalInformation", cfonbToolService.readZone("2b-M : additional informations", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_ALPHA_NUMERIC, 49, 70));
			break;
		case "MMO":
			// Zone 2b-M : Informations complémentaires  
	...
	protected Map<String, Object> readNewBalanceRecord(String lineContent) throws AxelorException  {
		...
		structuredLineContent.put("lineType", BankStatementLineAFB120Repository.LINE_TYPE_FINAL_BALANCE);
		
		// Zone 1-B : Code banque
		String bankCode = cfonbToolService.readZone("3-B : bank code", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_NUMERIC, 3, 5);

		// Zone 1-D : Code guichet
		String sortCode = cfonbToolService.readZone("3-D : sort code", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_NUMERIC, 12, 5);
		
		// Zone 1-E : Code devise ISO
		String currencyCode =  cfonbToolService.readZone("3-E : currency code", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_ALPHA, 17, 3);
		structuredLineContent.put("currency", getCurrency(currencyCode));
		...
		String date = cfonbToolService.readZone("3-J : date", lineContent, cfonbToolService.STATUS_MANDATORY, cfonbToolService.FORMAT_NUMERIC, 35, 6);
		structuredLineContent.put("operationDate", getDate(date)); 
	...
	protected BankDetails getBankDetails(String accountNumber, String bankCode, String sortCode)  {
		...
		return bankDetailsRepository.all().filter("self.accountNbr = ?1 and self.bankCode = ?2 and self.sortCode = ?3", accountNumber, bankCode, sortCode).fetchOne();
	...
	/**
	 * Le montant est cadré à droite, complété à gauche par des zéros ; le
