...
public class ConfiguratorCheckServiceImpl implements ConfiguratorCheckService {
    ...
    @Override
    public void checkLinkedSaleOrderLine(Configurator configurator, Product product) throws AxelorException {
        Objects.requireNonNull(configurator);
        Objects.requireNonNull(product);


        var terminatedSOL = saleOrderLineRepository.all().filter("self.product = :product AND self.configurator = :configurator AND self.saleOrder.statusSelect IN (:finalizedStatus, :confirmedStatus, :completedStatus)")
        ...
        if (terminatedSOL != null) {
            throw new AxelorException(TraceBackRepository.CATEGORY_INCONSISTENCY, I18n.get(SaleExceptionMessage.CONFIGURATOR_CAN_NOT_REGENERATE_PRODUCT));
        }
    }
}
