...
public class RevenueVsCostController {
    ...
    try {
      ...
      List<DTO> resultList =
          new ArrayList<DTO>() {
            {
              addAll(
                  DTO.generateDtoListOutOfMonthDtoList(
              ...
              addAll(
                  DTO.generateDtoListOutOfMonthDtoList(
                      AccountInvoiceChartHelper.getCostByMonths(
                              AccountInvoiceChartConstants.CATEGORY_CODE_ALL)
                          .stream()
                          .filter(e -> e.year == year)
                          .collect(Collectors.toList()),
              ...
              addAll(
                  DTO.generateDtoListOutOfMonthDtoList(
                      AccountInvoiceChartHelper.getWorkforceCostByMonths(
                              Stream.of(year).collect(Collectors.toSet()), false)
                          .stream()
                          .filter(e -> e.year == year)
                          .collect(Collectors.toList()),
                      "Workforce cost"));
              addAll(
                  DTO.generateDtoListOutOfMonthDtoList(
                      AccountInvoiceChartHelper.getEBITDADTOByMonth().stream()
                          .filter(e -> e.year == year)
                          .collect(Collectors.toList()),
                      "EBITDA"));
              addAll(
                  DTO.generateDtoListOutOfMonthDtoList(
                      AccountInvoiceChartHelper.getProfitDTOByMonth(
                              AccountInvoiceChartConstants.CATEGORY_CODE_ALL)
                          .stream()
                          .filter(e -> e.year == year)
                          .collect(Collectors.toList()),
  ...
  private static class DTO {
    ...
    public static List<DTO> generateDtoListOutOfMonthDtoList(
        List<ChartMonthTurnoverDto> dto, String typeName) {
      return dto.stream()
          .map(e -> generateDtoOutOfMonthDto(e, typeName))
          .collect(Collectors.toList());
    }
