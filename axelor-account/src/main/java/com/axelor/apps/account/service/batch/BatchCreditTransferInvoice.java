...
public abstract class BatchCreditTransferInvoice extends BatchStrategy {
  ...
  protected List<InvoicePayment> processInvoices(int operationTypeSelect) {
    ...
    for (List<Invoice> invoiceList; !(invoiceList = query.fetch(FETCH_LIMIT)).isEmpty(); ) {
  ...
  protected void stop() {
    ...
    sb.append(
        String.format(
            I18n.get(
                    AccountExceptionMessage.BATCH_CREDIT_TRANSFER_INVOICE_DONE_SINGULAR,
                    AccountExceptionMessage.BATCH_CREDIT_TRANSFER_INVOICE_DONE_PLURAL,
                    batch.getDone())
                + " ",
            batch.getDone()));
    sb.append(
