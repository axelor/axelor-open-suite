...
public class AccountClearanceService {
  ...
  public List<? extends MoveLine> getExcessPayment(AccountClearance accountClearance)
    ...
    List<? extends MoveLine> moveLineList =
        moveLineRepo
            .all()
            .filter(
                "self.company = ?1 AND self.account.reconcileOk = 'true' AND self.fromSchedulePaymentOk = 'false' "
  ...
  public void validateAccountClearance(AccountClearance accountClearance) throws AxelorException {
    ...
    accountClearance.setName(
        sequenceService.getSequenceNumber(IAdministration.ACCOUNT_CLEARANCE, company));
    accountClearanceRepo.save(accountClearance);
  }
  ...
  public Move createAccountClearanceMove(
    ...
    MoveLine debitMoveLine =
        moveLineService.createMoveLine(
            move, partner, moveLine.getAccount(), amount, true, todayTime.toLocalDate(), 1, null);
    ...
    BigDecimal profitAmount =
        amount
            .divide(divid, IAdministration.DEFAULT_NB_DECIMAL_DIGITS, RoundingMode.HALF_EVEN)
            .setScale(IAdministration.DEFAULT_NB_DECIMAL_DIGITS, RoundingMode.HALF_EVEN);
    MoveLine creditMoveLine1 =
        moveLineService.createMoveLine(
            move, partner, profitAccount, profitAmount, false, todayTime.toLocalDate(), 2, null);
    ...
    MoveLine creditMoveLine2 =
        moveLineService.createMoveLine(
            move, partner, taxAccount, taxAmount, false, todayTime.toLocalDate(), 3, null);
