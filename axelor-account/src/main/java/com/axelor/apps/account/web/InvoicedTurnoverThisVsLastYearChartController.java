...
public class InvoicedTurnoverThisVsLastYearChartController {
    ...
    /**
     * Function which called on chart, to get data
     * Data sets into response
     *  from @request takes param 'invoiceType'. Accepted invoiceTypes are: 'WORKFORCE_COST', 'COST', 'REVENUE', 'GROSS_PROFIT' and 'EBITDA'
     *  from @request takes params 'companyCode'. Accepted companyCodes are: 'ALL', 'IT', 'MAIN', 'TRN' and 'COMM'
     *
     *  Algorithm:
     *  1) Get year and month for future calculations
        ...
        try {
            ...
            switch (operation) {
                ...
                case AccountInvoiceChartConstants.WORKFORCE_COST:
                    resultList = AccountInvoiceChartHelper.getWorkforceCostByYears(
                            Stream.of(currentYear, currentYear - 1).collect(Collectors.toCollection(HashSet::new)),
                            false
                    );
                    break;
            ...
            resultList = resultList.stream().filter(e-> currentYear - 1 == e.year || currentYear == e.year).collect(Collectors.toList());
