...
public class BatchReimbursementExport extends BatchStrategy {
  ...
  public void runCreateReimbursementExport(Company company) {
    ...
    List<Partner> partnerList =
        (List<Partner>)
            partnerRepository.all().filter("?1 IN self.companySet = ?1", company).fetch();
    ...
    for (Partner partner : partnerList) {
      ...
      try {
        ...
        if (reimbursementExportService.canBeReimbursed(
            partner, companyRepo.find(company.getId()))) {
        ...
        TraceBackService.trace(
            new AxelorException(
                String.format(
                    I18n.get("Tiers") + "%s", partnerRepository.find(partner.getId()).getName()),
                e,
                e.getcategory()),
            IException.REIMBURSEMENT,
            batch.getId());
