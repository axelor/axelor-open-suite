...
public class WorkflowVentilationServiceImpl implements WorkflowVentilationService {
    ...
    public void afterVentilation(Invoice invoice) throws AxelorException {
        ...
        if (!accountConfigService.getAccountConfig(company)
        ...
        if (invoice.getInvoiceAutomaticMail()) {
            try {
                Message message = Beans.get(TemplateMessageService.class).generateAndSendMessage(invoice, invoice.getInvoiceMessageTemplate());
    ...
    /**
     * Copy payments from selected advance payment invoices
     * to this invoice.
     *
     *
     * @param invoice
    ...
    protected void copyAdvancePaymentToInvoice(Invoice invoice) throws AxelorException {
        ...
        for (Invoice advancePaymentInvoice : advancePaymentInvoiceSet) {
            ...
            List<InvoicePayment> advancePayments = advancePaymentInvoice
        ...
        BigDecimal totalPayments = invoicePayments
                .stream()
