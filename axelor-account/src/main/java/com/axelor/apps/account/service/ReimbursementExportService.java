...
public class ReimbursementExportService {
  ...
  public ReimbursementExportService(
      ...
      AccountBlockingService accountBlockingService,
        ...
        if (amountRemaining.compareTo(BigDecimal.ZERO) > 0) {
          ...
          MoveLine newDebitMoveLine =
              moveLineService.createMoveLine(
                  newMove, partner, moveLine.getAccount(), amountRemaining, true, today, seq, null);
  ...
  public Reimbursement createReimbursement(Partner partner, Company company)
    ...
    reimbursement.setRef(
        sequenceService.getSequenceNumber(IAdministration.REIMBOURSEMENT, company));

    return reimbursement;
  }

  /**
   * Le tiers peux t-il être remboursé ? Si le tiers est bloqué en remboursement et que la date de
   * fin de blocage n'est pas passée alors on ne peut pas rembourser.
   *
   * @return
   */
  ...
  public void createReimbursementInvoice(Invoice invoice) throws AxelorException {
    ...
    List<? extends MoveLine> moveLineList =
        moveLineRepo
            .all()
            .filter(
                "self.account.reconcileOk = 'true' AND self.fromSchedulePaymentOk = 'false' "
