    ...
    try {
      ...
      if (debtRecoveryHistoryIds != null && !debtRecoveryHistoryIds.isEmpty()) {
        ...
        if (zipPath.isPresent()) {
          ...
          response.setFlash(I18n.get(AccountExceptionMessage.NO_DEBT_RECOVERY_HISTORY_FILE));
          return;
        }

      } else {
        response.setFlash(I18n.get(AccountExceptionMessage.NO_DEBT_RECOVERY_HISTORY_SELECTED));
      }
    } catch (Exception e) {
