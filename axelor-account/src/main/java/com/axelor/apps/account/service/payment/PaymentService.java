...
public class PaymentService {
  ...
  /**
   * Utiliser le trop perçu entre deux listes de lignes d'écritures (une en débit, une en crédit) Si
   * cette methode doit être utilisée, penser à ordonner les listes qui lui sont passées par date
   * croissante Ceci permet de payer les facture de manière chronologique.
   *
          ...
          for (MoveLine debitMoveLine : debitMoveLines) {
            if ((debitMoveLine.getAmountRemaining().compareTo(BigDecimal.ZERO) == 1)
              ...
              if (debitMoveLine.getMaxAmountToReconcile() != null
                ...
                amount =
                    debitMoveLine
                        ...
                        .min(creditMoveLine.getAmountRemaining());
                debitMoveLine.setMaxAmountToReconcile(null);
              } else {
                amount =
                    creditMoveLine.getAmountRemaining().min(debitMoveLine.getAmountRemaining());
              }
              ...
              if (nextDebitTotalRemaining.compareTo(BigDecimal.ZERO) <= 0
                ...
                log.debug("last loop");
                reconcile =
                    reconcileService.createReconcile(debitMoveLine, creditMoveLine, amount, true);
              } else {
                reconcile =
                    reconcileService.createReconcile(debitMoveLine, creditMoveLine, amount, false);
              }
  ...
  public int createExcessPaymentWithAmount(
    ...
    if (remainingPaidAmount2.compareTo(BigDecimal.ZERO) > 0) {
      ...
      MoveLine moveLine =
          moveLineService.createMoveLine(
              move, partner, account, remainingPaidAmount2, false, this.today, moveLineNo2, null);
  ...
  public int useExcessPaymentWithAmountConsolidated(
      ...
      for (Map<Account, BigDecimal> map : allMap) {
        ...
        if (amountDebit.compareTo(BigDecimal.ZERO) > 0) {
          MoveLine debitMoveLine =
              moveLineService.createMoveLine(
                  move, partner, accountMap, amountDebit, true, date, dueDate, moveLineNo2, null);
    ...
    if (remainingPaidAmount2.compareTo(BigDecimal.ZERO) > 0) {
      ...
      MoveLine debitmoveLine =
          moveLineService.createMoveLine(
              move, partner, account, remainingPaidAmount2, true, date, dueDate, moveLineNo2, null);
