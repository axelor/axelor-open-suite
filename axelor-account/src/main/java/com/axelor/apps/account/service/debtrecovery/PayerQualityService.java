...
public class PayerQualityService {
  ...
  public List<ReminderHistory> getReminderHistoryList(Partner partner) {
    ...
    if (partner.getAccountingSituationList() != null) {
      for (AccountingSituation accountingSituation : partner.getAccountingSituationList()) {
        ...
        if (reminder != null
            && reminder.getReminderHistoryList() != null
            && !reminder.getReminderHistoryList().isEmpty()) {
          for (ReminderHistory reminderHistory : reminder.getReminderHistoryList()) {
            if ((reminderHistory.getReminderDate() != null
                && reminderHistory.getReminderDate().isAfter(today.minusYears(1)))) {
  ...
  public BigDecimal getPayerQualityNote(
      ReminderHistory reminderHistory, List<PayerQualityConfigLine> payerQualityConfigLineList) {
  ...
  public List<Partner> getPartnerList() {
    return partnerRepository
        .all()
        .filter("self.isCustomer = true and self.hasOrdered = true")
        .fetch();
  }
  ...
  public void payerQualityProcess() throws AxelorException {
    List<PayerQualityConfigLine> payerQualityConfigLineList =
        generalService.getGeneral().getPayerQualityConfigLineList();
    if (payerQualityConfigLineList == null || payerQualityConfigLineList.size() == 0) {
      throw new AxelorException(
          String.format(I18n.get(IExceptionMessage.PAYER_QUALITY_1), GeneralServiceImpl.EXCEPTION),
