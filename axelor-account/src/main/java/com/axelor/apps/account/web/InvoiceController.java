...
public class InvoiceController {
  ...
  public void showInvoice(ActionRequest request, ActionResponse response) throws AxelorException {
    ...
    if (context.containsKey("_ids") && !ObjectUtils.isEmpty(request.getContext().get("_ids"))) {
      ...
      reportSetting =
          invoiceService.printInvoice(request.getContext().asType(Invoice.class), false);
    } else {
    ...
    response.setView(
        ActionView.define(reportSetting.getOutputName())
            .add("html", reportSetting.getFileLink())
            .map());
  }
