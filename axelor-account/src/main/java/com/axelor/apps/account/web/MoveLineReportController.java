...
public class MoveLineReportController {
  ...
  @Inject MoveLineReportService moveLineReportService;

  @Inject MoveLineReportRepository moveLineReportRepo;
  /**
   * @param request
   * @param response
   */
  ...
  public void getJournalType(ActionRequest request, ActionResponse response) {
    ...
    try {
      ...
      JournalType journalType =
          Beans.get(MoveLineReportService.class).getJournalType(moveLineReport);
      if (journalType != null) {
  ...
  public void printExportMoveLine(ActionRequest request, ActionResponse response) {
    ...
    try {
      if (moveLineReport.getExportTypeSelect() == null
          || moveLineReport.getExportTypeSelect().isEmpty()
          || moveLineReport.getTypeSelect() == 0) {
      ...
      if ((moveLineReport.getTypeSelect() >= MoveLineReportRepository.EXPORT_ADMINISTRATION
        ...
        String fileLink =
            ReportFactory.createReport(
                    String.format(IReport.MOVE_LINE_REPORT_TYPE, moveLineReport.getTypeSelect()),
                    name + "-${date}")
