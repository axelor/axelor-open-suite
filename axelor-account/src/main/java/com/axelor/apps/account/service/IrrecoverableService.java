...
public class IrrecoverableService extends IrrecoverableRepository{
	...
	public List<Invoice> getInvoiceList(Company company)   {
		return (List<Invoice>) invoiceService.all().filter("self.irrecoverableStateSelect = ?1 AND self.company = ?2 AND self.statusSelect = ?3 " +
	...
	public List<PaymentScheduleLine> getPaymentScheduleLineList(Company company)   {
		return (List<PaymentScheduleLine>) paymentScheduleLineService.all()
	...
	public List<IrrecoverableReportLine> createIrrecoverableReportLineList(IrrecoverableInvoiceLine iil, Invoice invoice, BigDecimal prorataRate)  {
		...
		for(InvoiceLine invoiceLine : consolidateInvoiceLine(invoice.getInvoiceLineList()))  {
			irlList.add(this.createIrrecoverableReportLine(iil, invoiceLine.getInvoiceLineType().getName(), invoiceLine.getExTaxTotal().multiply(prorataRate).setScale(IAdministration.NB_DECIMAL_TOTAL, RoundingMode.HALF_EVEN), seq));
			seq++;
		}
		for(InvoiceLineTax invoiceLineTax : invoice.getInvoiceLineTaxList())  {
			irlList.add(this.createIrrecoverableReportLine(iil, invoiceLineTax.getTaxLine().getTax().getName(), invoiceLineTax.getTaxTotal().multiply(prorataRate).setScale(IAdministration.NB_DECIMAL_TOTAL, RoundingMode.HALF_EVEN), seq));
			seq++;
		}
	...
	public List<IrrecoverableReportLine> createIrrecoverableReportLineList(IrrecoverablePaymentScheduleLineLine ipsll, PaymentScheduleLine paymentScheduleLine, Tax tax) throws AxelorException  {
		...
		// Montant hors-Taxe
		BigDecimal irrecoverableAmount = amount.divide(divid, 6, RoundingMode.HALF_EVEN).setScale(IAdministration.NB_DECIMAL_TOTAL, RoundingMode.HALF_EVEN);
