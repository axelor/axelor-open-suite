...
public class IrrecoverableService {
      ...
      for (Invoice invoice : irrecoverable.getInvoiceSet()) {
        ...
        try {
          ...
          TraceBackService.trace(
              new AxelorException(
                  String.format(I18n.get("Invoice") + " %s", invoice.getInvoiceId()),
                  e,
                  e.getcategory()),
  ...
  public void createMoveForPaymentScheduleLineReject(
    ...
    if (move == null) {
      throw new AxelorException(
          String.format(I18n.get(IExceptionMessage.IRRECOVERABLE_2), GeneralServiceImpl.EXCEPTION),
  ...
  public void createIrrecoverableInvoiceLineMove(Irrecoverable irrecoverable, Invoice invoice)
    ...
    Move move =
        this.createIrrecoverableMove(invoice, prorataRate, invoice.getRejectMoveLine() != null);
    if (move == null) {
      throw new AxelorException(
          String.format(I18n.get(IExceptionMessage.IRRECOVERABLE_2), GeneralServiceImpl.EXCEPTION),
          IException.INCONSISTENCY);
  ...
  public Move createIrrecoverableMove(
      Invoice invoice, BigDecimal prorataRate, boolean isInvoiceReject) throws AxelorException {
  ...
  public Move createIrrecoverableMove(MoveLine moveLine) throws AxelorException {
    ...
    MoveLine creditMoveLine =
        moveLineService.createMoveLine(
            move, payerPartner, moveLine.getAccount(), amount, false, date, seq, null);
    move.getMoveLineList().add(creditMoveLine);
    ...
    MoveLine creditMoveLine2 =
        moveLineService.createMoveLine(
            move, payerPartner, taxAccount, taxAmount, true, date, 3, null);
