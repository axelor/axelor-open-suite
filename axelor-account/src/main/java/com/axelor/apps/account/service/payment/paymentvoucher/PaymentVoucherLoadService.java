...
public class PaymentVoucherLoadService {
    ...
    if (paymentVoucherContext.getPayVoucherElementToPayList() != null) {
      ...
      if (paymentVoucherContext.getPaidAmount() == null) {
        throw new AxelorException(
            String.format(
                I18n.get(IExceptionMessage.PAYMENT_VOUCHER_LOAD_1), GeneralServiceImpl.EXCEPTION),
  ...
  public boolean mustBeBalanced(
    ...
    // Et que le montant déjà payé en devise sur la facture, plus le montant réglé par la nouvelle
    // saisie paiement en devise, est égale au montant total en devise de la facture
  ...
  public List<MoveLine> assignMaxAmountToReconcile(
    ...
    if (moveLineInvoiceToPay != null && moveLineInvoiceToPay.size() != 0) {
      // Récupération du montant imputé sur l'échéance, et assignation de la valeur dans la moveLine
      // (champ temporaire)
