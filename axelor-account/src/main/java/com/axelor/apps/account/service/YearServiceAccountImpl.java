...
public class YearServiceAccountImpl extends YearServiceImpl {
  ...
  public YearServiceAccountImpl(
      ...
      ReportedBalanceRepository reportedBalanceRepo,
  ...
  public void closeYear(Year year) throws AxelorException {
    ...
    if (company == null) {
      throw new AxelorException(
          String.format(
              I18n.get(IExceptionMessage.YEAR_1), GeneralServiceImpl.EXCEPTION, year.getName()),
      ...
      for (ReportedBalance reportedBalance : partner.getReportedBalanceList()) {
        if (reportedBalance.getCompany().equals(company)) {
          ...
          ReportedBalanceLine reportedBalanceLine =
              this.createReportedBalanceLine(
                  reportedBalance,
  ...
  public ReportedBalanceLine createReportedBalanceLine(
      ReportedBalance reportedBalance, BigDecimal amount, Year year) {
  ...
  public BigDecimal computeReportedBalance(
    ...
    Query q =
        JPA.em()
            .createQuery(
                "select SUM(ml.credit-ml.debit) FROM MoveLine as ml "
                    + "WHERE ml.partner = ?1 AND ml.ignoreInAccountingOk = false AND ml.date >= ?2 AND ml.date <= ?3 AND (ml.account = ?4 OR ml.account = ?5) ",
                BigDecimal.class);
