...
public class MoveLineReportServiceImpl implements MoveLineReportService {
  ...
  @Inject
  public MoveLineReportServiceImpl(
  ...
  public String buildQuery(MoveLineReport moveLineReport) throws AxelorException {
    ...
    if (moveLineReport.getAccountSet() != null && !moveLineReport.getAccountSet().isEmpty()) {
      this.addParams(
          "(self.account in (?%d) or self.account.parent in (?%d) "
              ...
              + "or self.account.parent.parent.parent.parent.parent.parent in (?%d))",
          moveLineReport.getAccountSet());
    }
    ...
    if (moveLineReport.getTypeSelect() > MoveLineReportRepository.EXPORT_PAYROLL_JOURNAL_ENTRY) {
      this.addParams(
          "(self.move.accountingOk = false OR (self.move.accountingOk = true and self.move.moveLineReport = ?%d))",
          moveLineReport);
    }
    ...
    if (moveLineReport.getTypeSelect() <= 5 || moveLineReport.getTypeSelect() >= 10) {
      ...
      String seq =
          sequenceService.getSequenceNumber(
              IAdministration.MOVE_LINE_REPORT, moveLineReport.getCompany());
      if (seq == null) {
        throw new AxelorException(
            String.format(
                I18n.get(IExceptionMessage.MOVE_LINE_REPORT_1),
                GeneralServiceImpl.EXCEPTION,
                moveLineReport.getCompany().getName()),
      ...
      String seq =
          sequenceService.getSequenceNumber(
              IAdministration.MOVE_LINE_EXPORT, moveLineReport.getCompany());
      if (seq == null) {
        throw new AxelorException(
            String.format(
                I18n.get(IExceptionMessage.MOVE_LINE_REPORT_2),
                GeneralServiceImpl.EXCEPTION,
                moveLineReport.getCompany().getName()),
            IException.CONFIGURATION_ERROR);
      }
  ...
  public Account getAccount(MoveLineReport moveLineReport) {
    if (moveLineReport.getTypeSelect() == 13 && moveLineReport.getCompany() != null) {
      return accountRepo
          .all()
          .filter("self.company = ?1 AND self.code LIKE '58%'", moveLineReport.getCompany())
          .fetchOne();
    }
    return null;
  }
