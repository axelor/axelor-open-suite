...
public class AccountInvoiceChartHelper {
    ...
    private static List<ChartMonthTurnoverDto> calculateProfitDTOByMonth(String companyCode) {
        ...
        List<ChartMonthTurnoverDto> revenueDTO = AccountInvoiceChartHelper.getRevenueByMonths(companyCode);

        // Combine data
        List<RecordDto> combined = RecordDto.generateRecordDtoList(revenueDTO, costsDTO, null);

        return combined.stream()
                .map(RecordDto::getProfitDTO)
    ...
    private static List<ChartMonthTurnoverDto> calculateEBITDADTOByMonth() {
        // Get data
        List<ChartMonthTurnoverDto> costsDTO = AccountInvoiceChartHelper.getCostByMonths(AccountInvoiceChartConstants.CATEGORY_CODE_ALL);
        List<ChartMonthTurnoverDto> revenueDTO = AccountInvoiceChartHelper.getRevenueByMonths(AccountInvoiceChartConstants.CATEGORY_CODE_ALL);
        ...
        List<ChartMonthTurnoverDto> workforceCostsDTO = AccountInvoiceChartHelper.getWorkforceCostByMonths(yearSet, false);

        // Combine everything
        List<RecordDto> combined = RecordDto.generateRecordDtoList(revenueDTO, costsDTO, workforceCostsDTO);

        // Returned mapped data
        return combined.stream()
                .map(RecordDto::getEbitdaDTO)
                .collect(Collectors.toList());
