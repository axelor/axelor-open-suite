...
public class AccountInvoiceWorkforceCostsHelper {
     ...
     */
    public static List<ChartYearTurnoverDto> getWorkforceCostByYears(Set<Integer> years, boolean isForecast) {
     ...
     * @return DTO's wheres turnover is calculated workforce cost
     */
    public static List<ChartMonthTurnoverDto> getWorkforceCostByMonths(Set<Integer> years, boolean isForecast) {
        ...
        if (!isForecast) {
            ...
            result = result.stream()
                    .filter( chartMonthTurnoverDto -> chartMonthTurnoverDto.year != currentDate.getYear() || chartMonthTurnoverDto.monthNo <= currentDate.getMonthValue() )
     ...
     * @return Month dto object wheres amoalount is workforce cost, year and month are workforce cost year and month
     */
    private static List<ChartMonthTurnoverDto> calculateWorkforceCostByMonths(List<Contract> contracts, int year) {
        ...
        for (int monthCountNumber = 1; monthCountNumber <= 12; monthCountNumber++) {
            try {
                ...
                String dateString = year + "-" + String.format("%02d", monthCountNumber) + "-" + String.format("%02d", 1);
                ...
                List<Contract> filteredContracts = contracts.stream()
                        .filter(contract -> contract.isContractActiveInDates(dateWithLastDayOfMonth, dateForCheck)
                        ).collect(Collectors.toList());


                // Create dto with calculated amount
                ChartMonthTurnoverDto dto = new ChartMonthTurnoverDto(
                        filteredContracts
                                .stream()
    ...
    static class Contract {
        ...
        public String toString() {
            return "employee: " + employee + ", cost: " + monthlyGlobalCost.toString() + ", end_date: " + endDate + ", start_date: " + startDate;
        }
