...
public class VentilateState extends WorkflowInvoice {
  ...
  @Inject private MoveService moveService;

  @Inject private AccountConfigService accountConfigService;
  ...
  protected void setPartnerAccount() throws AxelorException {
    ...
    if (accountSituation == null) {
      accountSituation =
          Beans.get(AccountingSituationService.class)
              .createAccountingSituation(invoice.getPartner(), invoice.getCompany());
    }

    if (invoice.getPartnerAccount() == null) {
      Account account =
          InvoiceToolService.isPurchase(invoice)
              ? accountSituation.getSupplierAccount()
              : accountSituation.getCustomerAccount();

      if (account == null) {
        throw new AxelorException(
            I18n.get(IExceptionMessage.VENTILATE_STATE_5), IException.CONFIGURATION_ERROR);
      }
  ...
  protected void setDate() throws AxelorException {
    ...
    if (invoice.getInvoiceDate() == null) {
      ...
      throw new AxelorException(
          I18n.get(IExceptionMessage.VENTILATE_STATE_FUTURE_DATE), IException.CONFIGURATION_ERROR);
    }
  ...
  protected void checkInvoiceDate(Sequence sequence) throws AxelorException {
    ...
    if (invoiceRepo.all().filter(query, params.toArray()).count() > 0) {
      if (sequence.getMonthlyResetOk()) {
        throw new AxelorException(
            I18n.get(IExceptionMessage.VENTILATE_STATE_2), IException.CONFIGURATION_ERROR);
      }
      if (sequence.getYearlyResetOk()) {
        throw new AxelorException(
            I18n.get(IExceptionMessage.VENTILATE_STATE_3), IException.CONFIGURATION_ERROR);
      }
      throw new AxelorException(
          I18n.get(IExceptionMessage.VENTILATE_STATE_1), IException.CONFIGURATION_ERROR);
    }
  }
  ...
  protected void setInvoiceId(Sequence sequence) throws AxelorException {
    ...
    throw new AxelorException(
        String.format(
            I18n.get(IExceptionMessage.VENTILATE_STATE_4), invoice.getCompany().getName()),
  ...
  protected Sequence getSequence() throws AxelorException {
    ...
    switch (invoice.getOperationTypeSelect()) {
      ...
      default:
        throw new AxelorException(
            String.format(I18n.get(IExceptionMessage.JOURNAL_1), invoice.getInvoiceId()),
