	...
	protected void process() {
		if(!stop)  {
			...
			try  {
				...
				TraceBackService.trace(new AxelorException(e, e.getCategory(), I18n.get(IExceptionMessage.BATCH_REIMBURSEMENT_6), batch.getId()), IException.REIMBURSEMENT, batch.getId());				
				incrementAnomaly();
				...
				TraceBackService.trace(new Exception(String.format(I18n.get(IExceptionMessage.BATCH_REIMBURSEMENT_6), batch.getId()), e), IException.REIMBURSEMENT, batch.getId());
			...
			for(List<String[]> rejectList : data.keySet())  {
				...
				for(String[] reject : rejectList)  {
					...
					try  {
						...
						TraceBackService.trace(new AxelorException(e, e.getCategory(), I18n.get(IExceptionMessage.BATCH_REIMBURSEMENT_7), reject[1]), IException.REIMBURSEMENT, batch.getId());						
						incrementAnomaly();
						
					} catch (Exception e) {
						
						TraceBackService.trace(new Exception(String.format(I18n.get(IExceptionMessage.BATCH_REIMBURSEMENT_7), reject[1]), e), IException.REIMBURSEMENT, batch.getId());
