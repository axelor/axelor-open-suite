...
public class MoveServiceImpl implements MoveService {
	...
	@Inject
	public MoveServiceImpl(AppAccountService appAccountService, MoveLineService moveLineService,
			MoveCreateService moveCreateService, MoveValidateService moveValidateService,
			MoveToolService moveToolService, MoveRemoveService moveRemoveService, ReconcileService reconcileService,
			MoveDueService moveDueService, PaymentService paymentService,
			MoveExcessPaymentService moveExcessPaymentService, MoveRepository moveRepository,
	...
	public Move createMove(Invoice invoice) throws AxelorException {
		...
		if (invoice != null && invoice.getInvoiceLineList() != null) {
			...
			log.debug("Création d'une écriture comptable spécifique à la facture {} (Société : {}, Journal : {})",
			...
			move = moveCreateService.createMove(journal, company, invoice.getCurrency(), partner,
					invoice.getInvoiceDate(), invoice.getPaymentMode(), MoveRepository.TECHNICAL_ORIGIN_AUTOMATIC,
					functionalOrigin, origin, null);
			...
			if (move != null) {
				...
				move.getMoveLineList().addAll(moveLineService.createMoveLines(invoice, move, company, partner, account,
						journal.getIsInvoiceMoveConsolidated(), isPurchase, isDebitCustomer));
	...
	/**
	 * Méthode permettant d'employer les trop-perçus 2 cas : - le compte des
	 * trop-perçus est le même que celui de la facture : alors on lettre directement
	 * - le compte n'est pas le même : on créée une O.D. de passage sur le bon
	 * compte
	 *
	 * @param invoice
	 * @return
	...
	/**
	 * Méthode permettant d'employer les dûs sur l'avoir On récupère prioritairement
	 * les dûs (factures) selectionné sur l'avoir, puis les autres dûs du tiers
	 *
	 * <p>
	 * 2 cas : - le compte des dûs est le même que celui de l'avoir : alors on
	 * lettre directement - le compte n'est pas le même : on créée une O.D. de
	 * passage sur le bon compte
	 *
	 * @param invoice
	 * @return
	 * @throws AxelorException
	...
	public Move createMoveUseInvoiceDue(Invoice invoice) throws AxelorException {
		...
		List<MoveLine> debitMoveLines = moveDueService.getInvoiceDue(invoice,
	...
	public void createMoveUseExcessPayment(Invoice invoice) throws AxelorException {
		...
		if (creditMoveLineList != null && creditMoveLineList.size() != 0) {
			...
			// trop-perçus
				...
				Move move = moveCreateService.createMove(journal, company, null, partner, invoice.getInvoiceDate(),
						null, MoveRepository.TECHNICAL_ORIGIN_AUTOMATIC, MoveRepository.FUNCTIONAL_ORIGIN_PAYMENT,
						origin, null);
				...
				if (move != null) {
					...
					MoveLine creditMoveLine = moveLineService.createMoveLine(move, partner, account, amount, false,
							appAccountService.getTodayDate(company), 1, origin, null);
					...
					Reconcile reconcile = reconcileService.createReconcile(invoiceCustomerMoveLine, creditMoveLine,
							amount, false);
					if (reconcile != null) {
	...
	@Override
	public Move createMoveUseDebit(Invoice invoice, List<MoveLine> debitMoveLines, MoveLine invoiceCustomerMoveLine)
			throws AxelorException {
		Company company = invoice.getCompany();
		Partner partner = invoice.getPartner();
		Account account = invoice.getPartnerAccount();
		...
		Move oDmove = moveCreateService.createMove(journal, company, null, partner, invoice.getInvoiceDate(), null,
				MoveRepository.TECHNICAL_ORIGIN_AUTOMATIC, MoveRepository.FUNCTIONAL_ORIGIN_PAYMENT, origin, null);
		...
		if (oDmove != null) {
			...
			MoveLine debitMoveLine = moveLineService.createMoveLine(oDmove, partner, account, amount, true,
					appAccountService.getTodayDate(company), 1, origin, null);
			oDmove.getMoveLineList().add(debitMoveLine);

			// Emploie des dûs sur les lignes de credit qui seront créées au fil de l'eau
			paymentService.createExcessPaymentWithAmount(debitMoveLines, amount, oDmove, 2, partner, company, null,
					account, appAccountService.getTodayDate(company));
	...
	@Transactional(rollbackOn = { Exception.class })
	@Override
	public Move generateReverse(Move move, boolean isAutomaticReconcile, boolean isAutomaticAccounting,
			boolean isUnreconcileOriginalMove, LocalDate dateOfReversion) throws AxelorException {

		Move newMove = moveCreateService.createMove(move.getJournal(), move.getCompany(), move.getCurrency(),
				move.getPartner(), dateOfReversion, move.getPaymentMode(), MoveRepository.TECHNICAL_ORIGIN_ENTRY,
				move.getFunctionalOriginSelect(), move.getIgnoreInDebtRecoveryOk(), move.getIgnoreInAccountingOk(),
				move.getAutoYearClosureMove(), move.getOrigin(), move.getDescription());

		move.setInvoice(move.getInvoice());
		move.setPaymentVoucher(move.getPaymentVoucher());
		move.setDescription(move.getDescription());

		boolean validatedMove = move.getStatusSelect() == MoveRepository.STATUS_ACCOUNTED
		...
		for (MoveLine moveLine : move.getMoveLineList()) {
			...
			if (moveLine.getAnalyticDistributionTemplate() != null) {
				...
				List<AnalyticMoveLine> analyticMoveLineList = Beans.get(AnalyticMoveLineService.class).generateLines(
						newMoveLine.getAnalyticDistributionTemplate(),
						newMoveLine.getDebit().add(newMoveLine.getCredit()),
						AnalyticMoveLineRepository.STATUS_REAL_ACCOUNTING, dateOfReversion);
				if (CollectionUtils.isNotEmpty(analyticMoveLineList)) {
					analyticMoveLineList
							.forEach(analyticMoveLine -> newMoveLine.addAnalyticMoveLineListItem(analyticMoveLine));
				}
			}
			...
			if (isUnreconcileOriginalMove) {
				List<Reconcile> reconcileList = Beans.get(ReconcileRepository.class).all()
						.filter("self.statusSelect != ?1 AND (self.debitMoveLine = ?2 OR self.creditMoveLine = ?2)",
								ReconcileRepository.STATUS_CANCELED, moveLine)
						.fetch();
	...
	public MoveLine findMoveLineByAccount(Move move, Account account) throws AxelorException {
		return move.getMoveLineList().stream().filter(moveLine -> moveLine.getAccount().equals(account)).findFirst()
				.orElseThrow(() -> new AxelorException(move, TraceBackRepository.CATEGORY_NO_VALUE,
						I18n.get("%s account not found in move %s"), account.getName(), move.getReference()));
	}
	...
	public Map<String, Object> computeTotals(Move move) {
		...
		BigDecimal totalCurrency = move.getMoveLineList().stream().map(MoveLine::getCurrencyAmount)
				.reduce(BigDecimal.ZERO, BigDecimal::add);
		values.put("$totalCurrency", totalCurrency.divide(BigDecimal.ONE.add(BigDecimal.ONE)));

		BigDecimal totalDebit = move.getMoveLineList().stream().map(MoveLine::getDebit).reduce(BigDecimal.ZERO,
				BigDecimal::add);
		values.put("$totalDebit", totalDebit);

		BigDecimal totalCredit = move.getMoveLineList().stream().map(MoveLine::getCredit).reduce(BigDecimal.ZERO,
				BigDecimal::add);
	...
	public Move generateReverse(Move move, Map<String, Object> assistantMap) throws AxelorException {
		move = generateReverse(move, (boolean) assistantMap.get("isAutomaticReconcile"),
	...
	protected MoveLine generateReverseMoveLine(Move reverseMove, MoveLine orgineMoveLine, LocalDate dateOfReversion,
			boolean isDebit) throws AxelorException {
		MoveLine reverseMoveLine = moveLineService.createMoveLine(reverseMove, orgineMoveLine.getPartner(),
				orgineMoveLine.getAccount(), orgineMoveLine.getCurrencyAmount(),
				orgineMoveLine.getDebit().add(orgineMoveLine.getCredit()), orgineMoveLine.getCurrencyRate(), !isDebit,
				dateOfReversion, dateOfReversion, dateOfReversion, orgineMoveLine.getCounter(),
				orgineMoveLine.getName(), null);
	...
	public String filterPartner(Move move) {
		...
		if (move.getJournal() != null && !Strings.isNullOrEmpty(move.getJournal().getCompatiblePartnerTypeSelect())) {
			domain += " AND (";
	...
	@Override
	public Move updateMoveLinesDateExcludeFromPeriodOnlyWithoutSave(Move move) throws AxelorException {
		...
		if (move.getJournal().getJournalType().getTechnicalTypeSelect()
			...
			for (int i = 0; i < move.getPartner().getAccountingSituationList().size(); i++) {
				if (move.getPartner().getAccountingSituationList().get(i).equals(move.getCompany())) {
					if (move.getJournal().getJournalType().getTechnicalTypeSelect()
							.equals(JournalTypeRepository.TECHNICAL_TYPE_SELECT_EXPENSE))
						accountingAccount = move.getPartner().getAccountingSituationList().get(i).getSupplierAccount();
					else if (move.getJournal().getJournalType().getTechnicalTypeSelect()
							.equals(JournalTypeRepository.TECHNICAL_TYPE_SELECT_SALE))
						accountingAccount = move.getPartner().getAccountingSituationList().get(i).getCustomerAccount();
				}
			}
		} else if (move.getJournal().getJournalType().getTechnicalTypeSelect()
			...
			if (move.getPaymentMode() != null)
				for (int i = 0; i < move.getPaymentMode().getAccountManagementList().size(); i++) {
					if (move.getPaymentMode().getAccountManagementList().get(0).getCompany()
							.equals(move.getCompany())) {
						accountingAccount = move.getPaymentMode().getAccountManagementList().get(0).getCashAccount();
					}
				}
		}
		if (accountingAccount == null) {
	...
	public Account getAccountingAccountFromAccountConfig(Move move) {
		List<AccountingSituation> accountConfigs = move.getPartner().getAccountingSituationList().stream()
				.filter(accountingSituation -> accountingSituation.getCompany().equals(move.getCompany()))
				.collect(Collectors.toList());
		Account accountingAccount = null;

		if (move.getJournal().getJournalType()
				.getTechnicalTypeSelect() == JournalTypeRepository.TECHNICAL_TYPE_SELECT_EXPENSE) {
			if (accountConfigs.size() > 0) {
				accountingAccount = accountConfigs.get(0).getDefaultExpenseAccount();
			}
		} else if (move.getJournal().getJournalType()
				.getTechnicalTypeSelect() == JournalTypeRepository.TECHNICAL_TYPE_SELECT_SALE) {
			if (accountConfigs.size() > 0)
				accountingAccount = accountConfigs.get(0).getDefaultIncomeAccount();
		} else if (move.getJournal().getJournalType()
				.getTechnicalTypeSelect() == JournalTypeRepository.TECHNICAL_TYPE_SELECT_TREASURY) {
			if (move.getPaymentMode() != null) {
			...
			for (TaxEquiv taxEquiv : partner.getFiscalPosition().getTaxEquivList()) {
				if (accountingAccount != null)
					if (taxEquiv.getFromTax().equals(accountingAccount.getDefaultTax())) {
						...
						if (taxLine == null || !taxLine.getStartDate().isBefore(moveLine.getDate())) {
							taxLineList = taxEquiv.getToTax().getTaxLineList().stream()
									.filter(tl -> !moveLine.getDate().isBefore(tl.getEndDate())
											&& !tl.getStartDate().isAfter(moveLine.getDate()))
									.collect(Collectors.toList());
							if (taxLineList.size() > 0)
