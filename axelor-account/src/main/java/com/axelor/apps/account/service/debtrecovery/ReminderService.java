...
public class ReminderService {
  ...
  public ReminderService(
      ReminderSessionService reminderSessionService,
      ReminderActionService reminderActionService,
    ...
    for (MoveLine moveLine : moveLineQuery) {
      if (moveLine.getMove() != null && !moveLine.getMove().getIgnoreInReminderOk()) {
        ...
        if (move.getInvoice() != null
            && !move.getInvoice().getReminderBlockingOk()
   ...
   */
  @Transactional(rollbackOn = {AxelorException.class, Exception.class})
  public boolean reminderGenerate(Partner partner, Company company)
    ...
    if (balanceDue.compareTo(BigDecimal.ZERO) > 0) {
      ...
      BigDecimal balanceDueReminder =
          accountCustomerService.getBalanceDueReminder(partner, company);
      ...
      if (balanceDueReminder.compareTo(BigDecimal.ZERO) > 0) {
        ...
        this.updatePaymentScheduleLineReminder(
            reminder, this.getPaymentScheduleList(moveLineList, partner));
              ...
              new AxelorException(
                  String.format(
                      "%s :\n"
                          ...
                          + I18n.get("Company")
                          + " %s : "
                          + I18n.get(IExceptionMessage.REMINDER_4),
                      GeneralServiceImpl.EXCEPTION,
                      partner.getName(),
                      company.getName()),
  ...
  public void updatePaymentScheduleLineReminder(
      Reminder reminder, List<PaymentScheduleLine> paymentSchedueLineList) {
