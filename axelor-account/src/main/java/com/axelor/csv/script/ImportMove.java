...
public class ImportMove {
  ...
  @Transactional
  public Object importMove(Object bean, Map<String, Object> values) throws AxelorException {
    ...
    try {
      ...
      if (move == null) {
        ...
        System.err.println(move.getCompany());
  ...
  private Company getCompany(Map<String, Object> values) {
    ...
    Partner partner =
        Beans.get(PartnerRepository.class)
            .all()
            .filter("self.registrationCode = ?", fileName.substring(0, fileName.indexOf('F')))
            .fetchOne();

    if (partner != null && !partner.getCompanySet().isEmpty()) {
      return partner.getCompanySet().stream().findFirst().get();
