...
public class RequiredDocumentController {
  ...
  public void setDMSFile(ActionRequest request, ActionResponse response) {
    try {
      ...
      requiredDocument = Beans.get(RequiredDocumentRepository.class).find(requiredDocument.getId());
      Beans.get(RequiredDocumentDMSService.class).setDMSFile(requiredDocument);
