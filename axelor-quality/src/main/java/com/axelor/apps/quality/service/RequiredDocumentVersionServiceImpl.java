...
public class RequiredDocumentVersionServiceImpl implements RequiredDocumentVersionService {
  ...
  @Inject
  public RequiredDocumentVersionServiceImpl(RequiredDocumentRepository requiredDocRepo) {
  ...
  protected int computeNextVersion(RequiredDocument original) {
    ...
    int base = (highest != null ? highest.getDocVersion() : original.getDocVersion());
    return base + 1;
  }

  protected void markObsolete(RequiredDocument doc) {
    doc.setStatusSelect(RequiredDocumentRepository.STATUS_OBSOLETE);
    doc.setIsActiveVersion(false);
    requiredDocRepo.save(doc);
  }

  protected void deactivate(RequiredDocument active) {
    active.setIsActiveVersion(false);
    requiredDocRepo.save(active);
  }
}
