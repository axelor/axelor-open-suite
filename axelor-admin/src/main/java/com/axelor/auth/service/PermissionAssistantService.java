...
public class PermissionAssistantService {
            ...
            if (perm.getObject().equals(object.getFullName())) {
              for (MetaPermissionRule fieldPerm : perm.getRules()) {
                if (field.getName().equals(fieldPerm.getField())) {
                  ...
                  row[colIndex++] =
                      Strings.isNullOrEmpty(fieldPerm.getReadonlyIf())
                          ? ""
                          : fieldPerm.getReadonlyIf(); // readonly if
                  row[colIndex++] =
                      Strings.isNullOrEmpty(fieldPerm.getHideIf())
                          ? ""
                          : fieldPerm.getHideIf(); // hide if
                }
              }
            }
          }
  ...
  private void processCSV(
      ...
      String[] groupRow,
      String objectName,
      Map<String, MetaPermission> metaPermissionDict,
