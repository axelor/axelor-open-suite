...
public class CalendarService extends ICalendarService {
  ...
  @Inject protected CalendarRepository calendarRepo;
  ...
  //		props.add(new
  // Source(URI.create("ldap://ldap.example.com/cn=Babs%20Jensen,%20o=Babsco,%20c=US")));
  ...
  public File export(net.fortuna.ical4j.model.Calendar calendar)
    ...
    try {
      ...
      if (calendar.getLogin() != null
        ...
        throw new AxelorException(
            String.format(I18n.get(IExceptionMessage.CALENDAR_NOT_VALID)),
  ...
  protected Calendar doSync(Calendar calendar, CalDavCalendarCollection collection)
    ...
    for (ICalendarEvent item : calendar.getEventsCrm()) {
      ...
      if (target != null) {
        ...
        localEvents.put(
            target, item2.getTypeSelect()); // Associate event with typeSelect for further use
    ...
    for (String uid : remoteEvents.keySet()) {
      if (!synced.contains(uid)) {
        ...
        localEvents.put(
            vEvent,
            2); // events imported from remote calendar are by default 'meetings' (typeSelect = 2)
      }
    }
  ...
  public List<Long> showSharedEvents(User user) {
      ...
      for (CalendarManagement calendarManagement : userIt.getCalendarManagementList()) {
        if ((user.equals(calendarManagement.getUser()))
          ...
          if (calendarManagement.getAllCalendars()) {
            List<ICalendarUser> userList =
                Beans.get(ICalendarUserRepository.class)
                    .all()
                    .filter("self.user.id = ?1", userIt.getId())
                    .fetch();
            ...
            List<Calendar> calList =
                Beans.get(CalendarRepository.class)
                    .all()
                    .filter("self.user.id = ?1", userIt.getId())
                    .fetch();
            ...
            for (ICalendarUser iCalendarUser : userList) {
              eventList =
                  Beans.get(EventRepository.class)
                      .all()
                      .filter(
                          "?1 MEMBER OF self.attendees OR self.organizer.id = ?1",
                          iCalendarUser.getId())
                      .fetch();
              for (Event event : eventList) {
                eventIdlist.add(event.getId());
            ...
            if (calendarManagement.getErpCalendars()) {
              ...
              List<Event> eventList =
                  Beans.get(EventRepository.class)
                      .all()
                      .filter("self.user.id = ?1 AND self.calendarCrm is NULL", userIt.getId())
                      .fetch();
              for (Event event : eventList) {
                eventIdlist.add(event.getId());
              }
              for (ICalendarUser iCalendarUser : userList) {
                eventList =
                    Beans.get(EventRepository.class)
                        .all()
                        .filter(
                            "(?1 MEMBER OF self.attendees OR self.organizer.id = ?1) AND self.calendarCrm is NULL",
                            iCalendarUser.getId())
                        .fetch();
                for (Event event : eventList) {
                  eventIdlist.add(event.getId());
    ...
    List<ICalendarUser> userList =
        Beans.get(ICalendarUserRepository.class)
            .all()
            .filter("self.user.id = ?1", user.getId())
            .fetch();

    List<Event> eventList =
        Beans.get(EventRepository.class).all().filter("self.user.id = ?1", user.getId()).fetch();
    for (Event event : eventList) {
      eventIdlist.add(event.getId());
    }
    List<Calendar> calList =
        Beans.get(CalendarRepository.class).all().filter("self.user.id = ?1", user.getId()).fetch();
    for (Calendar calendar : calList) {
      for (Event event : calendar.getEventsCrm()) {
        eventIdlist.add(event.getId());
      }
    ...
    for (ICalendarUser iCalendarUser : userList) {
      eventList =
          Beans.get(EventRepository.class)
              .all()
              .filter(
                  "?1 MEMBER OF self.attendees OR self.organizer.id = ?1", iCalendarUser.getId())
              .fetch();
      for (Event event : eventList) {
        eventIdlist.add(event.getId());
      }
