...
public class CalendarService extends ICalendarService{
	...
	@Inject
	private ICalendarEventRepository icalEventRepo;
			...
			for (CalendarManagement calendarManagement : userIt.getCalendarManagementList()) {
				if((user.equals(calendarManagement.getUser())) || (team != null && team.equals(calendarManagement.getTeam()))){
					if(calendarManagement.getAllCalendars()){
						List<ICalendarEvent> eventList = icalEventRepo.all().filter("self.user.id = ?1 or self.calendar.user.id = ?1"
						...
						if(calendarManagement.getErpCalendars()){
							List<Event> eventList = eventRepo.all().filter("self.calendar = null and (self.user.id = ?1 "
									+ " or self.attendees.user.id = ?1 or self.organizer.user.id = ?1)",
									userIt.getId()).fetch();
							eventIdlist.addAll(Lists.transform(eventList, it->it.getId()));
						}
						if(calendarManagement.getIcalCalendars()){
							List<ICalendarEvent> eventList = icalEventRepo.all().filter("self.calendar in (?2) and (self.user.id = ?1 or self.calendar.user.id = ?1"
									+ " or self.attendees.user.id = ?1 or  self.organizer.user.id = ?1)",
									userIt.getId(),  calendarManagement.getCalendarSet()).fetch();
