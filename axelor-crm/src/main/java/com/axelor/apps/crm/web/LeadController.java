...
public class LeadController {
    ...
    if (!leadIds.equals("")) {
      ...
      try {
        ...
        if (user != null && !Strings.isNullOrEmpty(user.getLanguage())) {
          ...
          language =
              lead.getPartner().getLanguageSelect() != null
                  ? lead.getPartner().getLanguageSelect()
                  : "en";
        }
      } catch (Exception e) {
      }
  ...
  public void setSocialNetworkUrl(ActionRequest request, ActionResponse response)
    ...
    Map<String, String> urlMap =
        leadService.getSocialNetworkUrl(
            lead.getName(), lead.getFirstName(), lead.getEnterpriseName());
  ...
  public void getLeadImportConfig(ActionRequest request, ActionResponse response) {
    ImportConfiguration leadImportConfig =
        Beans.get(ImportConfigurationRepository.class)
            .all()
            .filter("self.bindMetaFile.fileName = ?1", "import-config-lead.xml")
            .fetchOne();
