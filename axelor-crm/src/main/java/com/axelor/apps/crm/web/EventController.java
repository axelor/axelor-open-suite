...
public class EventController {
  ...
  @Inject protected CalendarService calendarService;
  ...
  public void computeFromStartDateTime(ActionRequest request, ActionResponse response) {
    ...
    if (event.getStartDateTime() != null) {
      if (event.getDuration() != null && event.getDuration() != 0) {
        response.setValue(
            "endDateTime",
            eventService.computeEndDateTime(
                event.getStartDateTime(), event.getDuration().intValue()));
      } else if (event.getEndDateTime() != null
          && event.getEndDateTime().isAfter(event.getStartDateTime())) {
        Duration duration =
            eventService.computeDuration(event.getStartDateTime(), event.getEndDateTime());
  ...
  public void computeFromEndDateTime(ActionRequest request, ActionResponse response) {
    ...
    if (event.getEndDateTime() != null) {
      if (event.getStartDateTime() != null
        ...
        response.setValue(
            "startDateTime",
            eventService.computeStartDateTime(
                event.getDuration().intValue(), event.getEndDateTime()));
      }
    }
  }
    ...
    if (event.getTicketNumberSeq() == null && event.getTypeSelect() == IEvent.TICKET) {
      ...
      if (seq == null)
        throw new AxelorException(
            I18n.get(IExceptionMessage.EVENT_1), IException.CONFIGURATION_ERROR);
      else response.setValue("ticketNumberSeq", seq);
    }
  }
      ...
      if (result != null) {
        ...
        response.setFlash(
            String.format(
                ...
                event.getLocation()));
    } else response.setFlash(I18n.get(IExceptionMessage.EVENT_2));
  }
  ...
  public void assignToMeLead(ActionRequest request, ActionResponse response) {
    ...
    if (request.getContext().get("id") != null) {
      ...
      if (lead.getStatusSelect() == ILead.STATUS_NEW) lead.setStatusSelect(ILead.STATUS_ASSIGNED);
  ...
  public void checkModifications(ActionRequest request, ActionResponse response)
    ...
    if (deletedGuestsTemplate == null
      ...
      response.setFlash(
          String.format(
              I18n.get(IExceptionMessage.CRM_CONFIG_TEMPLATES_NONE),
              event.getUser().getActiveCompany().getName()));
    } else if (deletedGuestsTemplate == null
        || addedGuestsTemplate == null
        || changedDateTemplate == null) {
      throw new AxelorException(
          String.format(
              I18n.get(IExceptionMessage.CRM_CONFIG_TEMPLATES),
              event.getUser().getActiveCompany().getName()),
    ...
    if (request.getContext().containsKey("guestEmail")) {
      EmailAddress emailAddress =
          Beans.get(EmailAddressRepository.class)
              .find(
                  new Long(
                      ((Map<String, Object>) request.getContext().get("guestEmail"))
                          .get("id")
                          .toString()));
  ...
  public void generateRecurrentEvents(ActionRequest request, ActionResponse response)
    ...
    if (request.getContext().get("recurrenceType") == null) {
      throw new AxelorException(
          String.format(I18n.get(IExceptionMessage.RECURRENCE_RECURRENCE_TYPE)),
          IException.CONFIGURATION_ERROR);
    }
    ...
    if (request.getContext().get("periodicity") == null) {
      throw new AxelorException(
          String.format(I18n.get(IExceptionMessage.RECURRENCE_PERIODICITY)),
          IException.CONFIGURATION_ERROR);
    }
    ...
    if (recurrenceType == 2) {
      ...
      if (daysMap.isEmpty()) {
        throw new AxelorException(
            String.format(I18n.get(IExceptionMessage.RECURRENCE_DAYS_CHECKED)),
            IException.CONFIGURATION_ERROR);
      }
    }
    ...
    if (endType == 1) {
      if (request.getContext().get("repetitionsNumber") == null) {
        throw new AxelorException(
            String.format(I18n.get(IExceptionMessage.RECURRENCE_REPETITION_NUMBER)),
            IException.CONFIGURATION_ERROR);
      }
    ...
    if (endType == 2) {
      if (request.getContext().get("endDate") == null) {
        throw new AxelorException(
            String.format(I18n.get(IExceptionMessage.RECURRENCE_END_DATE)),
            IException.CONFIGURATION_ERROR);
      }
  ...
  public void setCalendarCrmDomain(ActionRequest request, ActionResponse response) {
    ...
    if (calendarIdlist.isEmpty()) {
      ...
      response.setAttr(
          "calendarCrm", "domain", "self.id in (" + Joiner.on(",").join(calendarIdlist) + ")");
    }
  }
