	...
	public void computeFromStartDateTime(ActionRequest request, ActionResponse response) {
		...
		if(event.getStartDate() != null) {
			if(event.getDuration() != null) {
				response.setValue("endDate", eventService.computeEndDateTime(event.getStartDate(), event.getDuration().intValue()));
			}
	...
	public void computeFromEndDateTime(ActionRequest request, ActionResponse response) {
		...
		if(event.getEndDate() != null) {
			if(event.getStartDate() != null && event.getStartDate().isBefore(event.getEndDate())) {
				...
				response.setValue("startDate", eventService.computeStartDateTime(event.getDuration().intValue(), event.getEndDate()));
			}
		}
	}
