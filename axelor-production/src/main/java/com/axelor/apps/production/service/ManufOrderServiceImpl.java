...
public class ManufOrderServiceImpl implements ManufOrderService {
  ...
  @Inject protected OperationOrderService operationOrderService;

  @Inject protected ManufOrderWorkflowService manufOrderWorkflowService;

  @Inject protected ProductVariantService productVariantService;
      ...
      for (BillOfMaterial billOfMaterialLine : billOfMaterial.getBillOfMaterialSet()) {
        ...
        if (!billOfMaterialLine.getHasNoManageStock()) {
          ...
          manufOrder.addToConsumeProdProductListItem(
              new ProdProduct(
                  product,
                  billOfMaterialLine.getQty().multiply(manufOrderQty),
  ...
  public void createToProduceProdProductList(ManufOrder manufOrder) {
    ...
    manufOrder.addToProduceProdProductListItem(
        new ProdProduct(
            ...
            billOfMaterial.getQty().multiply(manufOrderQty),
            billOfMaterial.getUnit()));

    // Add the residual products
    if (generalService.getGeneral().getManageResidualProductOnBom()
      ...
      for (ProdResidualProduct prodResidualProduct : billOfMaterial.getProdResidualProductList()) {
        ...
        manufOrder.addToProduceProdProductListItem(
            new ProdProduct(
                product,
                prodResidualProduct.getQty().multiply(manufOrderQty),
  ...
  public String getManufOrderSeq() throws AxelorException {
    ...
    if (seq == null) {
      throw new AxelorException(
          I18n.get(IExceptionMessage.MANUF_ORDER_SEQ), IException.CONFIGURATION_ERROR);
    }
  ...
  public String getLanguageToPrinting(ManufOrder manufOrder) {
    ...
    if (company != null
        && company.getPrintingSettings() != null
