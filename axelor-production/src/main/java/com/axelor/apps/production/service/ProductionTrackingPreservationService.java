...
public interface ProductionTrackingPreservationService {
  ...
  class PreservedTrackingNumber {
    ...
    public boolean restoreOrigin() {
      return restoreOrigin;
    }
  }

  class PreservedTrackingNumbersByProduct {
    private final Map<Long, Deque<PreservedTrackingNumber>> values;

    public PreservedTrackingNumbersByProduct(Map<Long, Deque<PreservedTrackingNumber>> values) {
      this.values = values;
    }

    public Map<Long, Deque<PreservedTrackingNumber>> values() {
      return values;
    }
