...
public class OperationOrderController {
  ...
  @Inject protected ManufOrderService manufOrderService;
  ...
  public void computeDuration(ActionRequest request, ActionResponse response) {
    ...
    if (operationOrder.getPlannedStartDateT() != null
      ...
      response.setValue(
          ...
          operationOrderWorkflowService.getDuration(
              operationOrderWorkflowService.computeDuration(
    ...
    if (operationOrder.getRealStartDateT() != null && operationOrder.getRealEndDateT() != null) {
      response.setValue(
          ...
          operationOrderWorkflowService.getDuration(
              operationOrderWorkflowService.computeDuration(
                  operationOrder.getRealStartDateT(), operationOrder.getRealEndDateT())));
    }
  }
  ...
  public void machineChange(ActionRequest request, ActionResponse response) throws AxelorException {
    ...
    if (operationOrder != null
        && operationOrder.getStatusSelect() == IOperationOrder.STATUS_PLANNED) {
  ...
  public void plan(ActionRequest request, ActionResponse response) throws AxelorException {
    ...
    if (operationOrder.getManufOrder() != null
        && operationOrder.getManufOrder().getStatusSelect() < IManufOrder.STATUS_PLANNED) {
      return;
    }
    operationOrder =
        operationOrderWorkflowService.plan(operationOrderRepo.find(operationOrder.getId()));
  ...
  public void finish(ActionRequest request, ActionResponse response) throws AxelorException {
    ...
    operationOrder =
        operationOrderWorkflowService.finish(operationOrderRepo.find(operationOrder.getId()));
    ...
    if (!operationOrderIds.equals("")) {
      ...
      String fileLink =
          ReportFactory.createReport(IReport.OPERATION_ORDER, name + "-${date}")
              .addParam(
                  "Locale", manufOrderService.getLanguageToPrinting(operationOrder.getManufOrder()))
  ...
  public void chargeByMachineHours(ActionRequest request, ActionResponse response)
    ...
    LocalDateTime fromDateTime =
        LocalDateTime.parse(request.getContext().get("fromDateTime").toString(), parser);
    LocalDateTime toDateTime =
        LocalDateTime.parse(request.getContext().get("toDateTime").toString(), parser);
    LocalDateTime itDateTime = new LocalDateTime(fromDateTime);

    if (Days.daysBetween(
                new LocalDate(
                    fromDateTime.getYear(),
                    fromDateTime.getMonthOfYear(),
      ...
      throw new AxelorException(
          String.format(I18n.get(IExceptionMessage.CHARGE_MACHINE_DAYS)),
      ...
      for (OperationOrder operationOrder : operationOrderList) {
        if (operationOrder.getWorkCenter() != null
          ...
          if (operationOrder.getPlannedStartDateT().isBefore(itDateTime)) {
            numberOfMinutes =
                Minutes.minutesBetween(itDateTime, operationOrder.getPlannedEndDateT())
                    .getMinutes();
          } else if (operationOrder.getPlannedEndDateT().isAfter(itDateTime.plusHours(1))) {
            numberOfMinutes =
                Minutes.minutesBetween(
                        operationOrder.getPlannedStartDateT(), itDateTime.plusHours(1))
                    .getMinutes();
          } else {
            numberOfMinutes =
                Minutes.minutesBetween(
          ...
          if (operationOrder.getWorkCenter().getMachine().getWeeklyPlanning() != null) {
            ...
            if (dayPlanning != null) {
              numberOfMinutesPerDay =
                  Minutes.minutesBetween(dayPlanning.getMorningFrom(), dayPlanning.getMorningTo())
                      .getMinutes();
              numberOfMinutesPerDay +=
                  Minutes.minutesBetween(
