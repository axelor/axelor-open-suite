...
public class ManufacturingOperationStockMoveService {
  ...
  @Inject
  public ManufacturingOperationStockMoveService(
  ...
  public void createToConsumeStockMove(ManufacturingOperation manufacturingOperation)
      throws AxelorException {

    Company company = manufacturingOperation.getManufOrder().getCompany();

    if (manufacturingOperation.getToConsumeProdProductList() != null && company != null) {

      StockMove stockMove = this._createToConsumeStockMove(manufacturingOperation, company);
      stockMove.setManufacturingOperation(manufacturingOperation);
      stockMove.setOrigin(manufacturingOperation.getOperationName());
      ...
      if (manufacturingOperation.getManufOrder().getIsConsProOnOperation()
      ...
      if (stockMove.getStockMoveLineList() != null && !stockMove.getStockMoveLineList().isEmpty()) {
        ...
        manufacturingOperation.addInStockMoveListItem(stockMove);
      }
      ...
      if (stockMove.getStockMoveLineList() != null) {
        for (StockMoveLine stockMoveLine : stockMove.getStockMoveLineList()) {
          manufacturingOperation.addConsumedStockMoveLineListItem(stockMoveLine);
        }
      }
    }
  }
  ...
  protected StockMove _createToConsumeStockMove(
      ManufacturingOperation manufacturingOperation, Company company) throws AxelorException {
    ...
    return stockMoveService.createStockMove(
        ...
        null,
        manufacturingOperation.getPlannedStartDateT().toLocalDate(),
        null,
  ...
  public void finish(ManufacturingOperation manufacturingOperation) throws AxelorException {
    ...
    List<StockMove> stockMoveList = manufacturingOperation.getInStockMoveList();
  ...
  public void partialFinish(ManufacturingOperation manufacturingOperation) throws AxelorException {
    ManufOrder manufOrder = manufacturingOperation.getManufOrder();
    ...
    stockMoveList = manufacturingOperation.getInStockMoveList();
    fromStockLocation = manufOrderStockMoveService.getDefaultInStockLocation(manufOrder, company);
    toStockLocation =
        stockConfigService.getProductionVirtualStockLocation(
            stockConfig,
            manufOrderOutsourceService.isOutsource(manufacturingOperation.getManufOrder()));
    ...
    StockMove newStockMove =
        stockMoveService.createStockMove(
            ...
            null,
            StockMoveRepository.TYPE_INTERNAL);
    newStockMove.setOrigin(manufacturingOperation.getOperationName());
    newStockMove.setManufacturingOperation(manufacturingOperation);

    newStockMove.setStockMoveLineList(new ArrayList<>());
    createNewStockMoveLines(
        manufacturingOperation, newStockMove, fromStockLocation, toStockLocation);
    ...
    if (!newStockMove.getStockMoveLineList().isEmpty()) {
      ...
      newStockMove
          ...
          .forEach(manufacturingOperation::addConsumedStockMoveLineListItem);
  ...
  public void createNewStockMoveLines(
    ...
    Beans.get(ManufacturingOperationService.class)
        .updateDiffProdProductList(manufacturingOperation);
    diffProdProductList = new ArrayList<>(manufacturingOperation.getDiffConsumeProdProductList());
  ...
  public void createNewConsumedStockMoveLineList(
      ManufacturingOperation manufacturingOperation, BigDecimal qtyToUpdate)
      throws AxelorException {

    // find planned stock move
    Optional<StockMove> stockMoveOpt =
        manufOrderStockMoveService.getPlannedStockMove(manufacturingOperation.getInStockMoveList());
    ...
    for (ProdProduct prodProduct : manufacturingOperation.getToConsumeProdProductList()) {
      BigDecimal qty =
          manufOrderStockMoveService.getFractionQty(
              manufacturingOperation.getManufOrder(), prodProduct, qtyToUpdate);
      ...
      stockMove.getStockMoveLineList().stream()
          .filter(
              stockMoveLine1 ->
                  !manufacturingOperation.getConsumedStockMoveLineList().contains(stockMoveLine1))
          .forEach(manufacturingOperation::addConsumedStockMoveLineListItem);
    }
