...
public class OperationOrderServiceImpl implements OperationOrderService {
  ...
  protected void _createToConsumeProdProductList(
      OperationOrder operationOrder, ProdProcessLine prodProcessLine) {
  ...
  // prodProductService.computeQuantity(ProdProduct.filter("self.producedOperationOrder = ?1 AND
  // self.product = ?2", operationOrder, prodProduct.getProduct()).fetch());
  //
