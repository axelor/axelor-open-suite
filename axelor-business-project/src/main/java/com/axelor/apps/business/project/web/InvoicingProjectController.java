...
public class InvoicingProjectController extends InvoicingProjectRepository{
	...
	public void generateInvoice(ActionRequest request, ActionResponse response) throws AxelorException{
		...
		Partner clientPartner = invoicingProject.getProjectTask().getClientPartner();
		if (clientPartner == null){
			clientPartner = invoicingProjectService.findRootClientPartner(invoicingProject.getProjectTask(), 0);
		}
		if(clientPartner == null){
			throw new AxelorException(String.format(I18n.get(IExceptionMessage.INVOICING_PROJECT_CUSTOMER)), IException.CONFIGURATION_ERROR);
		}
