...
public class TimesheetLineProjectServiceImpl extends TimesheetLineServiceImpl {
    ...
    public TimesheetLine createTimesheetLine(Project project, Product product, User user, LocalDate date, Timesheet timesheet, BigDecimal hours, String comments){
        ...
        if(project != null && (project.getProjInvTypeSelect() == ProjectRepository.INVOICING_TYPE_TIME_BASED
                || (project.getParentProject() != null
                && project.getParentProject().getProjInvTypeSelect() == ProjectRepository.INVOICING_TYPE_TIME_BASED)))
