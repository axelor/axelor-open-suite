...
public class InvoicingProjectService {
  ...
  @Inject protected PurchaseOrderInvoiceServiceImpl purchaseOrderInvoiceServiceImpl;
  ...
  public Invoice generateInvoice(InvoicingProject invoicingProject) throws AxelorException {
    ...
    if (company == null) {
      throw new AxelorException(
          String.format(I18n.get(IExceptionMessage.INVOICING_PROJECT_PROJECT_TASK_COMPANY)),
    ...
    AccountConfigBankPaymentService accountConfigService =
        Beans.get(AccountConfigBankPaymentService.class);
  ...
  public List<InvoiceLine> createInvoiceLine(Invoice invoice, ProjectTask projectTask, int priority)
    ...
    if (product == null) {
      throw new AxelorException(
          String.format(
              I18n.get(IExceptionMessage.INVOICING_PROJECT_PROJECT_TASK_PRODUCT),
    ...
    InvoiceLineGenerator invoiceLineGenerator =
        new InvoiceLineGenerator(
            ...
            projectTask.getPrice().multiply(projectTask.getQty()),
            null,
            false) {
  ...
  public void setLines(InvoicingProject invoicingProject, ProjectTask projectTask, int counter) {
    ...
    List<ProjectTask> projectTaskChildrenList =
  ...
  public void fillLines(InvoicingProject invoicingProject, ProjectTask projectTask) {
    if (projectTask.getProjTaskInvTypeSelect() == ProjectTaskRepository.INVOICING_TYPE_FLAT_RATE
        || projectTask.getProjTaskInvTypeSelect()
            == ProjectTaskRepository.INVOICING_TYPE_TIME_BASED) {
      ...
      invoicingProject
          ...
          .addAll(
              Beans.get(SaleOrderLineRepository.class)
                  .all()
                  .filter(
                      "self.saleOrder.project = ?1 AND self.toInvoice = true AND self.invoiced = false AND (self.saleOrder.creationDate < ?2 or ?3 is null)",
                      projectTask,
      ...
      invoicingProject
          ...
          .addAll(
              Beans.get(PurchaseOrderLineRepository.class)
                  .all()
                  .filter(
                      "self.projectTask = ?1 AND self.toInvoice = true AND self.invoiced = false AND (self.purchaseOrder.orderDate < ?2 or ?3 is null)",
                      projectTask,
                      invoicingProject.getDeadlineDate(),
                      invoicingProject.getDeadlineDate())
                  .fetch());
      ...
      invoicingProject
          ...
          .addAll(
              Beans.get(TimesheetLineRepository.class)
                  .all()
                  .filter(
                      "self.timesheet.statusSelect = 3 AND self.projectTask = ?1 AND self.toInvoice = true AND self.invoiced = false AND (self.date < ?2 or ?3 is null)",
                      projectTask,
                      invoicingProject.getDeadlineDate(),
                      invoicingProject.getDeadlineDate())
                  .fetch());
      ...
      invoicingProject
          ...
          .addAll(
              Beans.get(ExpenseLineRepository.class)
                  .all()
                  .filter(
                      "self.projectTask = ?1 AND self.toInvoice = true AND self.invoiced = false AND (self.expenseDate < ?2 or ?3 is null)",
                      projectTask,
                      invoicingProject.getDeadlineDate(),
                      invoicingProject.getDeadlineDate())
                  .fetch());
      ...
      invoicingProject
          ...
          .addAll(
              Beans.get(ElementsToInvoiceRepository.class)
                  .all()
                  .filter(
                      "self.project = ?1 AND self.toInvoice = true AND self.invoiced = false AND (self.date < ?2 or ?3 is null)",
                      projectTask,
                      invoicingProject.getDeadlineDate(),
                      invoicingProject.getDeadlineDate())
                  .fetch());
      ...
      if (projectTask.getProjTaskInvTypeSelect() == ProjectTaskRepository.INVOICING_TYPE_FLAT_RATE
          && !projectTask.getInvoiced()) invoicingProject.addProjectTaskSetItem(projectTask);
    }
  }
  ...
  public InvoicingProject createInvoicingProject(
    ...
    saleOrderLineList.addAll(
        Beans.get(SaleOrderLineRepository.class)
            .all()
            .filter(
                "self.saleOrder = ?1 AND self.toInvoice = true AND self.invoiced = false AND (self.saleOrder.creationDate < ?2 or ?3 is null)",
                saleOrder,
                deadlineDate,
                deadlineDate)
            .fetch());
