...
public class ProjectController {
  ...
  public void generateInvoiceForReleasedHoldBacks(ActionRequest request, ActionResponse response)
      throws AxelorException {

    Project project = request.getContext().asType(Project.class);
    project = Beans.get(ProjectRepository.class).find(project.getId());
    List<Map<String, Object>> lines =
        (List<Map<String, Object>>) request.getContext().get("projectHoldBackList");
    List<Integer> projectHoldBacksIds = new ArrayList<>();
    if (CollectionUtils.isEmpty(lines)) {
      return;
    }
    for (Map<String, Object> line : lines) {
      if (line.get("selected") != null && (boolean) line.get("selected")) {
    ...
    Invoice invoice =
        Beans.get(ProjectGenerateInvoiceService.class)
            .generateInvoiceForReleasedHoldBacks(project, projectHoldBacksIds);
    response.setReload(true);
    response.setView(
