...
public class TimesheetProjectServiceImpl extends TimesheetServiceImpl {
    ...
    for (TimesheetLine timesheetLine : timesheetLineList) {
      ...
      if (consolidate) {
        ...
        if (timeSheetInformationsMap.containsKey(key)) {
          ...
          tabInformations[4] =
              ((BigDecimal) tabInformations[4]).add(timesheetLine.getVisibleDuration());
        } else {
    ...
    for (Object[] timesheetInformations : timeSheetInformationsMap.values()) {
      ...
      invoiceLineList.addAll(
          this.createInvoiceLine(
              invoice, product, user, strDate, visibleDuration, priority * 100 + count));
  ...
  public TimesheetLine createTimesheetLine(
    ...
    if (project != null
        && (project.getProjTaskInvTypeSelect() == ProjectTaskRepository.INVOICING_TYPE_TIME_BASED
            || (project.getProject() != null
                && project.getProject().getProjTaskInvTypeSelect()
