    ...
    try {
      ...
      if (user != null) {
        List<ProjectTask> projectTaskList =
            Beans.get(ProjectTaskRepository.class).all().filter("self.imputable = true").fetch();
        for (ProjectTask projectTask : projectTaskList) {
          if ((projectTask.getMembersUserSet() != null
                  && projectTask.getMembersUserSet().contains(user))
