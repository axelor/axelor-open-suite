...
public class ProjectPlanningService {
  ...
  public static String getNameForColumns(int year, int week, int day) {
    ...
    return " "
        + Integer.toString(newDate.getDayOfMonth())
        + "/"
        + Integer.toString(newDate.getMonthOfYear());
  }

  @Transactional
  public List<ProjectPlanningLine> populateMyPlanning(ProjectPlanning planning, User user)
    ...
    List<ProjectTask> projectTaskList =
        Beans.get(ProjectTaskRepository.class).all().filter(query, user).fetch();
    if (projectTaskList == null || projectTaskList.isEmpty()) {
      throw new AxelorException(
          String.format(I18n.get(IExceptionMessage.PROJECT_PLANNING_NO_TASK)),
          IException.CONFIGURATION_ERROR);
    }
    for (ProjectTask projectTask : projectTaskList) {
      ProjectPlanningLine projectPlanningLine = null;
      projectPlanningLine =
          projectPlanningLineRepository
              .all()
              .filter(
                  "self.user = ?1 AND self.projectTask = ?2 AND self.year = ?3 AND self.week = ?4",
                  user,
                  projectTask,
                  planning.getYear(),
                  planning.getWeek())
  ...
  @Transactional
  public List<ProjectPlanningLine> populateMyTeamPlanning(ProjectPlanning planning, Team team)
      throws AxelorException {
    List<ProjectPlanningLine> planningLineList = new ArrayList<ProjectPlanningLine>();
    ...
    if (planningLineList.isEmpty()) {
      throw new AxelorException(
          String.format(I18n.get(IExceptionMessage.PROJECT_PLANNING_NO_TASK_TEAM)),
          IException.CONFIGURATION_ERROR);
    }
    return planningLineList;
  }
  ...
  public void getTasksForUser(ActionRequest request, ActionResponse response) {
    ...
    try {
      ...
      List<ProjectPlanningLine> linesList =
          Beans.get(ProjectPlanningLineRepository.class)
              .all()
              .filter(
                  "self.user.id = ?1 AND self.year >= ?2 AND self.week >= ?3",
      ...
      for (ProjectPlanningLine line : linesList) {
        if (line.getMonday().compareTo(BigDecimal.ZERO) != 0) {
          LocalDate date =
              new LocalDate()
                  .withYear(line.getYear())
                  .withWeekOfWeekyear(line.getWeek())
