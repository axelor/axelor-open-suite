...
public class ProjectTaskService {
  ...
  public ProjectTask generateProject(
      ProjectTask parentProject,
      String fullName,
      User assignedTo,
      Company company,
  ...
  private Partner getClientPartnerFromProjectTask(ProjectTask projectTask, int counter)
    ...
    if (projectTask.getProject() == null) {
      ...
      if (projectTask.getClientPartner() == null) {
        throw new AxelorException(
            String.format(I18n.get(IExceptionMessage.PROJECT_CUSTOMER_PARTNER)),
      ...
      if (counter > MAX_LEVEL_OF_PROJECT) {
        throw new AxelorException(
            String.format(I18n.get(IExceptionMessage.PROJECT_DEEP_LIMIT_REACH)),
            IException.CONFIGURATION_ERROR);
      } else {
