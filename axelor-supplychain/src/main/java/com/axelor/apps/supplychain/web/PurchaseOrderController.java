	...
	public void createStockMove(ActionRequest request, ActionResponse response) throws AxelorException {
		...
		if (purchaseOrder.getId() != null) {
			if (purchaseOrderServiceSupplychain.existActiveStockMoveForPurchaseOrder(purchaseOrder.getId())) {
				...
				Long stockMoveId = purchaseOrderServiceSupplychain
						.createStocksMove(Beans.get(PurchaseOrderRepository.class).find(purchaseOrder.getId()));
				if (!generalService.getGeneral().getSupplierStockMoveGenerationAuto()) {
					response.setView(ActionView.define(I18n.get("Stock move")).model(StockMove.class.getName())
							.add("grid", "stock-move-grid").add("form", "stock-move-form").param("forceEdit", "true")
	...
	public void mergePurchaseOrder(ActionRequest request, ActionResponse response) {
		...
		if (request.getContext().get("purchaseOrderToMerge") != null) {
			...
			if (request.getContext().get("purchaseOrderToMerge") instanceof List) {
				// No confirmation popup, purchase orders are content in a
				// parameter list
				...
				// After confirmation popup, purchase order's id are in a string
				// separated by ","
		...
		// Check if currency, supplierPartner and company are the same for all
		// selected purchase orders
		Currency commonCurrency = null;
		Partner commonSupplierPartner = null;
		Company commonCompany = null;
		Partner commonContactPartner = null;
		// Useful to determine if a difference exists between contact partners
		...
		// Useful to determine if a difference exists between locations of all
		// purchase orders
		...
		for (Long purchaseOrderId : purchaseOrderIdList) {
			...
			if (count == 1) {
				...
				if (commonLocation != null && !commonLocation.equals(purchaseOrderTemp.getLocation())) {
					commonLocation = null;
		...
		// Check if priceList or contactPartner or location are content in
		// parameters
		if (request.getContext().get("priceList") != null) {
			commonPriceList = JPA.em().find(PriceList.class,
					new Long((Integer) ((Map) request.getContext().get("priceList")).get("id")));
		}
		if (request.getContext().get("contactPartner") != null) {
			commonContactPartner = JPA.em().find(Partner.class,
					new Long((Integer) ((Map) request.getContext().get("contactPartner")).get("id")));
		}
		if (request.getContext().get("location") != null) {
			commonLocation = JPA.em().find(Location.class,
					new Long((Integer) ((Map) request.getContext().get("location")).get("id")));
		}

		if (!fromPopup && (existContactPartnerDiff || existPriceListDiff || existLocationDiff)) {
			// Need to display intermediate screen to select some values
			ActionViewBuilder confirmView = ActionView.define("Confirm merge purchase order")
					.model(Wizard.class.getName()).add("form", "purchase-order-merge-confirm-form")
					.param("popup", "true").param("show-toolbar", "false").param("show-confirm", "false")
					.param("popup-save", "false").param("forceEdit", "true");
		...
		try {
			PurchaseOrder purchaseOrder = purchaseOrderServiceSupplychain.mergePurchaseOrders(purchaseOrderList,
					commonCurrency, commonSupplierPartner, commonCompany, commonLocation, commonContactPartner,
					commonPriceList);
			if (purchaseOrder != null) {
				// Open the generated purchase order in a new tab
				response.setView(ActionView.define("Purchase Order").model(PurchaseOrder.class.getName())
						.add("grid", "purchase-order-grid").add("form", "purchase-order-form")
						.param("forceEdit", "true").context("_showRecord", String.valueOf(purchaseOrder.getId()))
						.map());
