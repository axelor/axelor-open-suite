...
public class MrpServiceImpl implements MrpService {
  ...
  public MrpServiceImpl(
      ...
      LocationLineRepository locationLineRepository,
      ...
      if (mrpLine.getMrpLineType().getElementSelect()
        ...
        MinStockRules minStockRules =
            minStockRulesService.getMinStockRules(
                product, mrpLine.getLocation(), MinStockRulesRepository.TYPE_FUTURE);
  ...
  public MrpLine getPreviousProposalMrpLine(
      Product product, MrpLineType mrpLineType, Location location, LocalDate maturityDate) {
    ...
    if (mrpLine != null) {
      ...
      mrpLine =
          mrpLineRepository.save(
              this.createMrpLine(
                  product, mrpLineType, reorderQty, maturityDate, BigDecimal.ZERO, location));
  ...
  protected MrpLineType getMrpLineTypeForProposal(MinStockRules minStockRules)
      throws AxelorException {
      ...
      saleOrderLineList.addAll(
          saleOrderLineRepository
              ...
              .filter(
                  ...
                  this.productMap.keySet(),
                  this.locationList,
                  SaleOrderRepository.STATE_NOT_DELIVERED,
                  ISaleOrder.STATUS_ORDER_CONFIRMED)
              .fetch());
    ...
    if (mrp.getMrpForecastSet().isEmpty()) {
      ...
      mrpForecastList.addAll(
          mrpForecastRepository
              .all()
              .filter(
                  "self.product in (?1) AND self.location in (?2) AND self.forecastDate >= ?3",
                  this.productMap.keySet(),
                  this.locationList,
                  today,
                  today)
  ...
  protected MrpLine createAvailableStockMrpLine(
      Product product, Location location, MrpLineType availableStockMrpLineType) {
  ...
  protected LocationLine getLocationLine(Product product, Location location) {
    ...
    return locationLineRepository
        .all()
        .filter("self.location = ?1 AND self.product = ?2", location, product)
        .fetchOne();
  }
  ...
  protected List<Location> getAllLocationAndSubLocation(Location location) {
    ...
    List<Location> subLocationList =
        locationRepository.all().filter("self.parentLocation = ?1", location).fetch();
