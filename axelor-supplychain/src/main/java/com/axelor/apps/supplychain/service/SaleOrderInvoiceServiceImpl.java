...
public class SaleOrderInvoiceServiceImpl implements SaleOrderInvoiceService {
  ...
  public InvoiceGenerator createInvoiceGenerator(SaleOrder saleOrder) throws AxelorException {
    ...
    if (saleOrder.getCurrency() == null) {
      throw new AxelorException(
          String.format(I18n.get(IExceptionMessage.SO_INVOICE_6), saleOrder.getSaleOrderSeq()),
  ...
  // Need to create this new method because createInvoiceLines doesn't take into account lines with
  // subscriptable products anymore
  ...
  public List<InvoiceLine> createSubscriptionInvoiceLine(
      Invoice invoice, Subscription subscription, Integer sequence) throws AxelorException {
  ...
  public Invoice generateSubscriptionInvoice(
    ...
    invoiceGenerator.populate(
        invoice, this.createSubscriptionInvoiceLines(invoice, subscriptionList));
  ...
  @Override
  @Transactional(rollbackOn = {AxelorException.class, Exception.class})
  public Invoice generateSubcriptionInvoiceForSaleOrder(SaleOrder saleOrder)
      throws AxelorException {
    List<Subscription> subscriptionList =
        Beans.get(SubscriptionRepository.class)
            .all()
            .filter(
                "self.invoicingDate <= ?1 AND self.saleOrderLine.saleOrder.id = ?2 AND self.invoiced = false",
  ...
  public Invoice generateSubcriptionInvoiceForSaleOrderAndListSubscrip(
    ...
    List<Subscription> subscriptionList =
        Beans.get(SubscriptionRepository.class)
            .all()
            .filter("self.id IN (:subscriptionIds)")
            .bind("subscriptionIds", subscriptionIdList)
            .fetch();
    if (subscriptionList != null && !subscriptionList.isEmpty()) {
  ...
  public Invoice generateSubcriptionInvoiceForSaleOrderLine(SaleOrderLine saleOrderLine)
    ...
    List<Subscription> subscriptionList =
        Beans.get(SubscriptionRepository.class)
            .all()
            .filter(
                "self.invoicingDate <= ?1 AND self.saleOrderLine.id = ?2 AND self.invoiced = false",
                generalService.getTodayDate(),
                saleOrderLine.getId())
            .fetch();
    if (subscriptionList != null && !subscriptionList.isEmpty()) {
