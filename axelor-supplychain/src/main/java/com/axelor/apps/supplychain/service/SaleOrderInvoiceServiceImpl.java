        ...
        if (saleOrderLine.getQty().compareTo(BigDecimal.ZERO) != 0) {
          qty =
              qtyToInvoiceItem
                  ...
                  .divide(saleOrderLine.getQty(), appBaseService.getNbDecimalDigitForQty(), RoundingMode.HALF_EVEN);
        }
        ...
        if (isPercent) {
          ...
          BigDecimal realQty =
              saleOrderLine
                  .getQty()
                  .multiply(percent)
                  .divide(new BigDecimal("100"), appBaseService.getNbDecimalDigitForQty(), RoundingMode.HALF_EVEN);
