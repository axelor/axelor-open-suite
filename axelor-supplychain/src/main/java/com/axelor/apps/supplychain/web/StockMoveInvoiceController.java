...
public class StockMoveInvoiceController {
	...
	public void generateInvoiceConcatStockMove(ActionRequest request, ActionResponse response)  {
		...
		if (request.getContext().get("paymentCondition") != null){
			paymentCondition = JPA.em().find(PaymentCondition.class, new Long((Integer)((Map)request.getContext().get("paymentCondition")).get("id")));
		}
		//paymentMode, only for customer stockMove
		if (request.getContext().get("paymentMode") != null){
			paymentMode = JPA.em().find(PaymentMode.class, new Long((Integer)((Map)request.getContext().get("paymentMode")).get("id")));
		}
		if (request.getContext().get("contactPartner") != null){
			contactPartner = JPA.em().find(Partner.class, new Long((Integer)((Map)request.getContext().get("contactPartner")).get("id")));
		}
		try{
			...
			if (mapResult.get("invoiceId") != null){
				...
				response.setFlash(I18n.get(IExceptionMessage.STOCK_MOVE_NO_INVOICE_GENERATED) + ": <br/>" + (String)mapResult.get("information"));
			}else{
	...
	public void generateMultiInvoice(ActionRequest request, ActionResponse response){
		...
		if (isCustomerStockMove){
			for (Map map : stockMoveMap) {
				try{
					...
					stockMovesInError.append(String.format(I18n.get(IExceptionMessage.STOCK_MOVE_GENERATE_INVOICE), stockMove.getName(), ae.getLocalizedMessage()));
				}catch (Exception e){
		...
		if (!invoiceIdList.isEmpty()){
			...
			viewBuilder.model(Invoice.class.getName())
						.add("grid", "invoice-grid")
						.add("form", "invoice-form")
