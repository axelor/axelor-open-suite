...
public class CustomerCreditLineServiceImpl implements CustomerCreditLineService {
  ...
  @Inject protected CustomerCreditLineRepository customerCreditLineRepo;

  @Inject protected SaleConfigService saleConfigService;
  ...
  public Partner generateLines(Partner partner) throws AxelorException {
    ...
    if (partner.getCustomerCreditLineList() != null
        && !partner.getCustomerCreditLineList().isEmpty()) {
      customerCreditLineList =
          new ArrayList<CustomerCreditLine>(partner.getCustomerCreditLineList());
    ...
    for (Company company : companyList) {
      ...
      customerCreditLine.setAcceptedCredit(
          saleConfigService.getSaleConfig(company).getAcceptedCredit());
  ...
  public void updateLinesFromOrder(Partner partner, SaleOrder saleOrder) throws AxelorException {
    ...
    for (CustomerCreditLine customerCreditLine : customerCreditLineList) {
      if (customerCreditLine.getCompany().equals(saleOrder.getCompany())) {
        customerCreditLine = this.computeUsedCredit(customerCreditLine);
        customerCreditLine.setUsedCredit(
            customerCreditLine
                .getUsedCredit()
                .add(saleOrder.getExTaxTotal().subtract(saleOrder.getAmountInvoiced())));
  ...
  public CustomerCreditLine computeUsedCredit(CustomerCreditLine customerCreditLine) {
    ...
    if (customerCreditLine.getPartner().getAccountingSituationList() != null) {
      List<AccountingSituation> accountingSituationList =
          customerCreditLine.getPartner().getAccountingSituationList();
          ...
          List<SaleOrder> saleOrderList =
              Beans.get(SaleOrderRepository.class)
                  ...
                  .filter(
                      ...
                      company,
                      customerCreditLine.getPartner(),
                      ISaleOrder.STATUS_DRAFT,
                      ISaleOrder.STATUS_CANCELED)
                  .fetch();
  ...
  public boolean checkBlockedPartner(Partner partner, Company company) throws AxelorException {
    CustomerCreditLine customerCreditLine =
        customerCreditLineRepo
            .all()
            .filter("self.company = ?1 AND self.partner = ?2", company, partner)
            .fetchOne();
