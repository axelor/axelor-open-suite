...
public class SubscriptionController {
  ...
  @Transactional
  public void generateAllSubscriptions(ActionRequest request, ActionResponse response)
      throws AxelorException {
    ...
    if (saleOrderLine != null) {
      ...
      Invoice invoice =
          Beans.get(SaleOrderInvoiceService.class)
              .generateSubcriptionInvoiceForSaleOrderLine(saleOrderLine);
  ...
  public void generateInvoiceForAllSubscriptions(ActionRequest request, ActionResponse response)
      throws AxelorException {

    Query q =
        JPA.em()
            .createQuery(
                "SELECT DISTINCT saleOrderLine.saleOrder.id FROM Subscription WHERE invoicingDate <= ?1 AND invoiced = false ",
                Long.class);
    ...
    if (saleOrderIdList != null) {
      ...
      for (Long saleOrderId : saleOrderIdList) {
        ...
        try {
          Invoice invoice =
              Beans.get(SaleOrderInvoiceService.class)
                  .generateSubcriptionInvoiceForSaleOrder(saleOrder);
  ...
  @SuppressWarnings("unchecked")
  public void generateInvoiceForSelectedSubscriptions(
    ...
    Query q =
        JPA.em()
            .createQuery(
                "SELECT DISTINCT new List(saleOrderLine.saleOrder.id as saleOrderId, id as subscriptionId) FROM Subscription WHERE id IN (:subscriptionIds)",
                List.class);
    ...
    if (listSelectedSubscriptions != null) {
      ...
      for (Map.Entry<Long, List<Long>> entry : subscriptionsBySaleOrderMap.entrySet()) {
        try {
          Invoice invoice =
              Beans.get(SaleOrderInvoiceService.class)
                  .generateSubcriptionInvoiceForSaleOrderAndListSubscrip(
