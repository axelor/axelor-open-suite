...
public class StockMoveLineController {
	...
	public void getProductPrice(ActionRequest request, ActionResponse response) {
		...
		if(stockMoveLine.getPackPriceSelect() != null &&
				stockMoveLine.getPackPriceSelect() == StockMoveLineRepository.SUBLINE_PRICE_ONLY && stockMoveLine.getLineTypeSelect() == StockMoveLineRepository.TYPE_PACK) {
				response.setValue("unitPriceUntaxed", 0.00);
				response.setValue("unitPriceTaxed", 0.00);
				
		} else if (stockMoveLine.getPackPriceSelect() != null && 
				stockMoveLine.getParentLine() != null || parentPackPriceSelect != null){
			
			if(parentPackPriceSelect != null && stockMoveLine.getIsSubLine() != null) {
				if (parentPackPriceSelect != StockMoveLineRepository.SUBLINE_PRICE_ONLY && !stockMoveLine.getIsSubLine()) {
					response.setValue("unitPriceUntaxed", 0.00);
					response.setValue("unitPriceTaxed", 0.00);
				}
			} else {
	...
	public void updateSubLineQty(ActionRequest request, ActionResponse response) {
		...
		if(titleMoveLine.getId() !=null) {
			...
			titleMoveLine.setSubLineList(line.getSubLineList());
