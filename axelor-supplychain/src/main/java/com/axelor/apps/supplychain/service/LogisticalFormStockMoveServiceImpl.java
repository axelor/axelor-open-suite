...
public class LogisticalFormStockMoveServiceImpl implements LogisticalFormStockMoveService {
  ...
  @Inject
  public LogisticalFormStockMoveServiceImpl(
  ...
  @Override
  public void addStockMoveToLogisticalForm(LogisticalForm logisticalForm, StockMove stockMove)
      throws AxelorException {
    checkStockMove(logisticalForm, stockMove);
    logisticalForm.addStockMoveListItem(stockMove);
    attachExistingPackagingsToLogisticalForm(logisticalForm, stockMove);
    logisticalFormRepository.save(logisticalForm);
  }

  protected void checkStockMove(LogisticalForm logisticalForm, StockMove stockMove)
      throws AxelorException {
    if (!stockMoveRepository
        .all()
        .filter(logisticalFormService.getStockMoveDomain(logisticalForm))
        .bind(logisticalFormService.getStockMoveDomainParam(logisticalForm))
        .fetch()
        .contains(stockMove)) {
      throw new AxelorException(
          TraceBackRepository.CATEGORY_INCONSISTENCY,
          I18n.get(SupplychainExceptionMessage.LOGISTICAL_FORM_STOCK_MOVE_NOT_VALID));
    }
  }

  @Transactional(rollbackOn = {Exception.class})
  @Override
  public void removeStockMoveFromLogisticalForm(LogisticalForm logisticalForm, StockMove stockMove)
      throws AxelorException {
    String error = detachPackagingsFromLogisticalForm(logisticalForm, stockMove);
    if (StringUtils.notEmpty(error)) {
      throw new AxelorException(TraceBackRepository.CATEGORY_INCONSISTENCY, error);
    }
    logisticalForm.removeStockMoveListItem(stockMove);
    logisticalFormRepository.save(logisticalForm);
  }
  ...
  protected String detachPackagingsFromLogisticalForm(
    ...
    for (Packaging packaging :
      ...
      logisticalForm.removePackagingListItem(packaging);
    }
