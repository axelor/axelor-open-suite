...
public class MetaModelController {
  ...
  @Inject ViewLoaderService viewLoaderService;
  ...
  public void openForm(ActionRequest request, ActionResponse response) throws AxelorException {
    ...
    ViewBuilder viewBuilder =
        viewLoaderService.getDefaultForm(
            model.getMetaModule().getName(), metaModelRepo.find(model.getId()), null, true);

    response.setView(
        ActionView.define(model.getName())
            ...
            .context("__check_version", "silent")
            .map());
  }
  ...
  public void openGrid(ActionRequest request, ActionResponse response) throws AxelorException {
    ...
    ViewBuilder viewBuilder =
        viewLoaderService.getDefaultGrid(
            model.getMetaModule().getName(), metaModelRepo.find(model.getId()), false);
