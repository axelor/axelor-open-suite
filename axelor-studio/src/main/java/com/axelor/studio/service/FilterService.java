...
public class FilterService {
  ...
  public static final List<String> NO_PARAMS =
      Arrays.asList(new String[] {"isNull", "notNull", "empty", "notEmpty"});
  ...
  private String createGroovyFilter(Filter filter, String parentField) {
    ...
    if (relationship != null && targetField != null) {
      ...
      if (relationship.equals("ManyToOne")) {
        field = targetField;
      } else if (relationship.equals("ManyToMany") && !operator.contains("mpty")) {
      ...
      if (fields.size() >= 2) {
        ...
        for (String field : fields) {
          MetaField subField =
              metaFieldRepo
                  .all()
                  .filter("self.name = ?1 and self.metaModel.name = ?2", field, modelName)
                  .fetchOne();
          if (subField == null) {
            break;
          }
  ...
  /**
   * Method to generate simple query condition for non relational chart filter fields.
   *
   * @param filter Chart filter record of chart builder
  ...
  public String getRelationalCondition(Filter filter, String paramName) {
    ...
    if (targetType == null) {
      MetaField targetField = (MetaField) getTargetField(metaField, filter.getTargetField()).get(1);
    ...
    switch (operator) {
      ...
      case "notInclude":
        return getM2MCondition(fieldName, targetType, conditionField, value, false);
      case "include":
        return getM2MCondition(fieldName, targetType, conditionField, value, true);
      default:
        break;
    }
   ...
   *
   * @param field M2M field name
   * @param nameColumn Array of string with first element as nameColumn field name and second as its
   *     type.
   * @param value Value of M2M field
   * @param like boolean to check if its like or non like condition.
   * @param notTag boolean to check if tag used in value.
   * @return String condition
   */
  private String getM2MCondition(
      String field, String targetType, String targetName, String value, boolean like) {
  ...
  /**
   * Method create like condition for chart filter with string field. Also it add LOWER function
   * condition.
   *
   ...
   * @return String condition.
   */
  private String getLikeCondition(String conditionField, String value, boolean isLike) {
