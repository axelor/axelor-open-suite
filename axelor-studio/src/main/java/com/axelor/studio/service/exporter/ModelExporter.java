...
public class ModelExporter {
  ...
  public void exportAppConfigs(ExporterService exporterService) throws ClassNotFoundException {

    this.exporterService = exporterService;

    List<MetaView> appConfigViews =
        metaViewRepo
            .all()
            .filter("self.name in (select concat('app-', app.code,'-config-form') from App app)")
            .fetch();

    for (MetaView metaView : appConfigViews) {
      Map<String, String> views = new HashMap<String, String>();
      views.put(metaView.getType(), metaView.getName());

      processModel(metaView.getModule(), metaView.getModel(), views);
    }
  }
  ...
  private void processModel(String module, String model, Map<String, String> views)
    ...
    try {
      ...
      if (model != null) {
        ...
        List<String[]> o2mViews =
            formExporter.export(exporterService, formView, getGridFields(grid));
  ...
  public MetaView getMetaView(String model, String type, String name) {
    ...
    MetaView view =
        metaViewRepo
            .all()
            .filter("self.type = ? and self.model = ? and self.name = ?", type, model, name)
            .fetchOne();
