...
public class FormImporter {
  ...
  @Inject private ViewPanelRepository viewPanelRepo;
  ...
  @Inject private FormulaImporter formulaImporter;
  ...
  private ViewPanel getLastPanel(ViewBuilder viewBuilder, boolean createPanel) {
    ...
    ViewPanel viewPanel =
        viewPanelRepo
            .all()
            .filter("self.viewBuilderSideBar = ?1 OR self.viewBuilder = ?1", viewBuilder)
            .order("-panelLevel")
            .fetchOne();
  ...
  @Transactional
  public ViewPanel createPanelCommon(
      String panelLevel, ViewBuilder viewBuilder, String[] basic, boolean addAttrs) {
    ...
    if (basic != null) {
      ...
      translationService.addTranslation(
          basic[3], row[CommonService.TITLE_FR], "fr", viewBuilder.getMetaModule().getName());
    }
  ...
  public void addButton(ViewBuilder viewBuilder, String[] basic) {
    ...
    viewItem.setOnClick(
        FormBuilderService.getUpdatedAction(viewItem.getOnClick(), row[CommonService.ON_CHANGE]));
  ...
  public void addField(ViewBuilder viewBuilder, MetaField metaField, String[] basic)
    ...
    viewItem.setOnChange(
        FormBuilderService.getUpdatedAction(viewItem.getOnChange(), row[CommonService.ON_CHANGE]));
  ...
  private ViewItem setNestedField(ViewPanel lastPanel, String parentField, ViewItem viewItem)
    ...
    ViewItem relationalField =
        viewItemRepo
            .all()
            .filter(
                "self.viewPanel = ?1 and LOWER(self.name) = ?2",
                lastPanel,
                parentField.toLowerCase())
            .fetchOne();

    if (relationalField == null) {
      throw new AxelorException(
          I18n.get("No parent field found '%s' for nested editor"), 1, parentField);
    }
  ...
  private ViewItem setEvent(ViewBuilder builder, ViewItem viewItem) {
    ...
    if (event != null) {
      ...
      if (viewItem.getTypeSelect() == 0) {
        String onChange = FormBuilderService.getUpdatedAction(viewItem.getOnChange(), event);
        viewItem.setOnChange(onChange);
      } else if (viewItem.getTypeSelect() == 1) {
        String onClick = FormBuilderService.getUpdatedAction(viewItem.getOnClick(), event);
