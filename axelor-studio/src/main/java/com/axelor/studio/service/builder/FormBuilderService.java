...
public class FormBuilderService {
  ...
  @Inject private FormBuilderService builderService;

  /**
   * Root method to access the service. It will generate FormView from ViewBuilder
   *
   * @param viewBuilder ViewBuilder record of type form.
   * @return FormView
  ...
  /**
   * Method update formView record by adding panels, toolbar buttons and onNew action into it.
   *
   * @param formView FormView generated from parent view.
   */
  private void processFormView(FormView formView, ViewBuilder viewBuilder) {
    ...
    String onSave = getUpdatedAction(viewBuilder.getOnSave(), formView.getOnSave());
    log.debug("OnSave actions final: {}", onSave);
    formView.setOnSave(onSave);

    String onNew = getUpdatedAction(viewBuilder.getOnNew(), formView.getOnNew());
    formView.setOnNew(onNew);

    String onLoad = getUpdatedAction(viewBuilder.getOnLoad(), formView.getOnLoad());
  ...
  private void processToolBar(FormView formView, List<ViewItem> items) {
    ...
    for (ViewItem viewButton : items) {
      log.debug("Button: {} onClick:{}", viewButton.getName(), viewButton.getOnClick());
  ...
  private void processMenuBar(FormView formView, List<ViewItem> menuItems) {
    ...
    for (ViewItem viewItem : menuItems) {
      ...
      log.debug(
          "Total items for menubar: {} is : {}",
          menu.getTitle(),
          viewItem.getMenubarItems().size());
   ...
   *
   * @param actions String of actions separated by comma.
  ...
  private FormView getFormView(ViewBuilder viewBuilder) throws JAXBException {
    ...
    formView.setXmlId(viewBuilder.getMetaModule().getName() + "-" + viewName);

    return formView;
  }

  /**
   * Method process viewPanels of viewBuilder to create new AbstractPanel and update AbstractPanel.
   *
   * @param sideBar boolean to check if panel is side panel or not
   */
   ...
   *
   * @param panelLevel Level of panel in formView.
   * @param isNotebook Boolean to check if panel is notebook.
  ...
  private List<AbstractWidget> updatePanelItems(
      List<AbstractWidget> panelItems, String panelLevel, AbstractPanel panel) {
  ...
  /**
   * Method to update panelMap with generated paneLevel as key and Panel as value.
   *
   * @param widgetIterator Iterator of formView items.
   * @param level Parent panel level
   * @param levelCounter Current level counter.
   */
  private void mapPanels(
      Iterator<AbstractWidget> widgetIterator, String level, Integer levelCounter) {
  ...
  /**
   * Method check if default value is there for ViewField. If defaultValue is there it will update
   * defaultFields list that will be used in onNew action generation.
   *
   * @param fieldType Type of field
   * @param fieldName Name of field
   * @param viewField ViewField to check for default vale.
   */
  private void checkDefaultValues(String fieldType, ViewItem viewItem) {
    ...
    if (defaultVal != null) {
      ...
      recordField.setExpression("eval:" + filterService.getTagValue(defaultVal, false));
  ...
  /**
   * Method to add panel related for o2m/m2m types of relational field. It will add PanelRelated
   * object created from viewField into list of panelItems of parent panel.
   *
   * @param fieldName Name of relational field.
   * @param viewField ViewField of relational type.
   * @param panelItems List of items of parent panel.
   */
  private void setPanelRelated(ViewItem viewItem, List<AbstractWidget> panelItems, String level) {
    ...
    if (itemLevel != null && !itemLevel.startsWith(level)) {
      if (itemLevel.contains(".")) {
        AbstractPanel parentPanel =
            (AbstractPanel) panelMap.get(itemLevel.substring(0, itemLevel.lastIndexOf(".")));
  ...
  /**
   * Method generate ActionRecord from defaultFields and using form view details. Also it set onNew
   * of formView.
   *
   * @param formView Updated form view with onNew action.
   */
  ...
  /**
   * Method remove old defaultFields and update new default field of ActionRecord.
   *
   * @param actionRecord ActionRecord to update.
   */
