...
public class FormulaImporter {
    ...
    for (int i = 0; i < formulas.size(); i++) {
      ...
      if (i % 2 == 0) {
        ...
        if (expr.startsWith("seq(")) {
          ...
          if (seqName != null) {
            line.setValue(
                "com.axelor.studio.service.builder.ActionBuilderService.getSequence('"
                    + seqName
                    + "')");
          }
  ...
  public List<String> importValidation(
      String[] row, String type, ViewBuilder viewBuilder, MetaModel model) {
  ...
  public List<String> addEvents(ViewBuilder viewBuilder, String events, String action) {
    ...
    for (String event : events.split(",")) {
      ...
      if (event.equals("save")) {
        ...
        viewBuilder.setOnSave(FormBuilderService.getUpdatedAction(onSave, action));
      } else {
        ViewItem viewItem =
            viewItemRepo
                .all()
        ...
        if (viewItem != null) {
          if (viewItem.getTypeSelect() == 0) {
            viewItem.setOnChange(
                FormBuilderService.getUpdatedAction(viewItem.getOnChange(), action));
          } else if (viewItem.getTypeSelect() == 1) {
            viewItem.setOnClick(FormBuilderService.getUpdatedAction(viewItem.getOnClick(), action));
          }
