...
/**
 * Service class generate domain xml files from MetaModels. It process MetaModel to create model and
 * MetaField to create field xml.
 *
 * @author axelor
 */
public class ModelBuilderService {
  ...
  /**
   * Root method to access the service. It will find all edited and customised MetaModels. Call
   * other methods to process MetaModel founds.
   *
  ...
  public void build() throws AxelorException {
    ...
    try {
      List<MetaModel> customizedModels =
          metaModelRepo.all().filter("self.customised = true").fetch();
  ...
  /**
   * Method create domain xml file from MetaModel. It create one single domain xml string for domain
   * and write it to file. Also call method to process fields.
   *
  ...
  /**
   * This method process MetaField and create field xml from it. The generated field xml will be
   * appended to model xml.
   *
   * @param fieldIterator MetaField iterator
   * @throws AxelorException
   */
  private void addFields(Iterator<MetaField> fieldIterator) throws AxelorException {
    ...
    if (reserveFields.contains(name)) {
      throw new AxelorException(
          I18n.get("In model '%s' field name '%s' is reserve word. " + "Please use different one"),
          5,
    ...
    if (Namming.isKeyword(name)) {
      fieldXml.append("column=\"" + Inflector.getInstance().underscore(name) + "_val\" ");
    }
  ...
  /**
   * Special method to append min/max attributes to field xml for string and integer type of fields.
   *
   * @param field MetaField to process
   */
  ...
  /**
   * Method to process decimal type of field for default,min,max attributes and append it to field
   * xml.
   *
   * @param field MetaField to process
   */
  ...
  /**
   * Method to process string type of field for default,large,min,max attributes and append it to
   * field xml.
   *
   * @param field MetaField to process
   */
  ...
  /**
   * Method to process boolean field for default attributes and append it to field xml.
   *
   * @param field MetaField to process
   */
  ...
  /**
   * Method to add 'ref' attribute in field xml for relational field. It search typeName of
   * MetaField for reference model and set fullName as reference.
   *
   * @param field MetaField to process
   */
  private void addRelational(StringBuilder fieldXml, MetaField field) {
    ...
    log.debug(
        "Type name for relational field : {}, typeName: {}", field.getName(), field.getTypeName());
   ...
   * @throws IOException Exception thrown by file handling.
   */
  ...
  public void sortFieldList(List<MetaField> fieldList) {
    ...
    Comparator<MetaField> comparator =
        new Comparator<MetaField>() {
  ...
  /**
   * Method filter customised field from list MetaField. Only customised field will be processed and
   * added in domain xml file.
   *
   * @param fields List of MetaField to filter.
   * @return List of customised field.
   */
  public List<MetaField> getCustomisedFields(MetaModel metaModel, boolean getWkf) {
    ...
    List<MetaField> customFields =
        metaFieldRepo.all().filter(query, metaModel).order("sequence").fetch();
   ...
   * @param language Translation language to update.
   */
  @Transactional
  ...
  /**
   * Method convert simple type name of field into adk supported 'typeName' (which is type class
   * name) for MetaField.
   *
  ...
  /**
   * It will check the domain files of all the customised modules. It removes a file if the related
   * model is not present in customizedModels.
   *
  ...
  private void checkFiles(List<MetaModel> customizedModels) throws AxelorException {
    ...
    for (MetaModule module : configService.getCustomizedModules()) {
      ...
      for (File file : moduleDir.listFiles()) {
        if (!fileNames.contains(file.getName()) && !file.getName().equals("Selection.xml")) {
