...
public class WkfDesignerService {
  ...
  /**
   * Method parse xml doc to create workflow nodes from it. It set incoming and outgoing tranistions
   * of node.
   *
   * @param doc
   */
  ...
  public void processXml(Wkf instance) throws Exception {
    ...
    if (bpmnXml != null) {
      ...
      WkfTransitionRepository wkfTransitionRepository = Beans.get(WkfTransitionRepository.class);
      ...
      for (int i = 0; i < list.getLength(); i++) {
        ...
        WkfTransition transition =
            wkfTransitionRepository
                .all()
                .filter(
                    "self.wkf.id = ? and self.xmlId = ?",
                    instance.getId(),
