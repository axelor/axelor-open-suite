...
public class ActionImporter {
  ...
  private MetaField getMetaField(MetaModel model, String name) {
    ...
    if (name != null) {
      return metaFieldRepo
          .all()
          .filter("self.name = ?1 and self.metaModel = ?2", name, model)
          .fetchOne();
    }
  ...
  private Set<ReportBuilder> getReportBuilders(String names) {
    ...
    List<ReportBuilder> reportBuilders =
        reportBuilderRepo.all().filter("self.name in (?1)", nameList).fetch();
    ...
    if (target != null) {
      ...
      if (builder.getMetaModel() != null) {
        MetaField field =
            metaFieldRepo
                .all()
                .filter("self.name = ?1 and self.metaModel = ?2", target, builder.getMetaModel())
                .fetchOne();
