...
class WkfTransitionService {
  ...
  protected void process(ActionGroup actionGroup) {
    ...
    String actionsToRemove =
        wkfService.clearViewButtons(viewBuilder.getToolbar(), null, wkfButtonNames);
   ...
   * condition to assign the status. Based on record fields onSave action will be created. It also
   * add button for transition if transition executed based on button.
   *
  ...
  private List<RecordField> proccessTransitions() {
    ...
    for (WkfTransition transition : wkfService.workflow.getTransitions()) {
      ...
      if (transition.getIsButton()) {
        buttonSeq++;
        addButton(wkfService.viewBuilder, transition, condition, buttonSeq);
        continue;
      }

      log.debug("Conditions : {}", transition.getConditions());
      String filters = filterService.getGroovyFilters(transition.getConditions(), null);
  ...
  private void addButton(
      ViewBuilder viewBuilder, WkfTransition transition, String condition, Integer sequence) {
    ...
    String permName =
        this.wkfService.moduleName + "." + wkfService.dasherizeModel.replace("-", ".") + name;
   ...
   */
  public void addButtonActions(ViewItem viewButton, WkfTransition transition, String buttonName) {
    ...
    if (alterType > 0 && alertMsg != null) {
      ...
      this.wkfService.updateMetaAction(alertAction, "action-validate", xml);
    ...
    if (successMsg != null) {
      ...
      this.wkfService.updateMetaAction(sucessAction, "action-validate", xml);
