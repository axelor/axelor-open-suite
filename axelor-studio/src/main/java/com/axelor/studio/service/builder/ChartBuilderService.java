...
public class ChartBuilderService {
  ...
  public AbstractView getView(ViewBuilder viewBuilder) throws JAXBException {
    ...
    String xml =
        "<chart name=\"" + viewBuilder.getName() + "\" title=\"" + viewBuilder.getTitle() + "\" ";
    ...
    xml +=
        Tab1
            + "<category key=\"groupField\" type=\"text\" title=\""
            + viewBuilder.getGroupOn().getLabel()
            + "\" />";
    xml +=
        Tab1
            + "<series key=\"fieldSum\" type=\""
            + viewBuilder.getChartType()
            + "\" title=\""
  ...
  private String[] prepareQuery(ViewBuilder viewBuilder) {
    ...
    String query =
        "SELECT"
            + Tab3
            + "SUM(self."
            + viewBuilder.getDisplayField().getName()
            + ") AS fieldSum,"
            + Tab3;
    ...
    query += Tab2 + "FROM " + Tab3 + viewBuilder.getMetaModel().getName() + " self";
   ...
   *
   * @param metaField Group field name selected in chart builder.
   * @param dateType Type of date (month,year,day) selected for group field.
   * @return Group field string.
   */
  private String getGroupFieldName(MetaField metaField, String dateType, String target) {
    ...
    if (dateType != null && "LocalDate,DateTime,LocalDateTime".contains(typeName)) {
  ...
  private void setDefaultValue(
      String fieldName, String typeName, String defaultValue, String[] modelField) {
  ...
  private String createFilters(List<Filter> filterList) {
    ...
    for (Filter filter : filterList) {
      ...
      if (filter.getIsParameter()) {
        addSearchField(field, filter.getTargetField(), filter.getDefaultValue());
      }
  ...
  private void addSearchField(MetaField field, String targetField, String defaultVal) {

    field = (MetaField) filterService.getTargetField(field, targetField).get(1);
    String relationship = field.getRelationship();
    String fieldName = field.getName();
    String typeName = field.getTypeName();
    String fieldStr = "<field name=\"" + field.getName() + "\" title=\"" + field.getLabel();
