...
public class ChartBuilderService {
  ...
  private String createXml(ChartBuilder chartBuilder, String[] queryString) {
    ...
    String sqlType = chartBuilder.getIsJpql() ? "jpql" : "sql";

    xml += "\t<dataset type=\"" + sqlType + "\"><![CDATA[";
  ...
  private String[] prepareQuery(ChartBuilder chartBuilder) throws AxelorException {
    ...
    boolean isJpql = chartBuilder.getIsJpql();
    ...
    String filters = null;
    if (isJpql) {
      filters = chartBuilder.getFilter();
      addSearchField(chartBuilder.getFilterMetadata());
    } else {
      filters = filterSqlService.getSqlFilters(chartBuilder.getFilterList(), joins, true);
    ...
    if (chartBuilder.getIsJson()) {
      String modelFilter = isJpql ? "self.jsonModel = '" : "self.json_model = '";
      if (filters != null) {
        filters = modelFilter + model + "' AND (" + filters + ")";
      } else {
    ...
    query += Tab2 + "FROM " + Tab3 + getTable(model, isJpql) + " self";
    ...
    if (!StringUtils.isEmpty(filters)) {
      query += Tab2 + "WHERE " + Tab3 + filters.trim();
    }

    query += Tab2 + "GROUP BY " + Tab3 + (isJpql ? groupField : "group_field");

    if (aggField != null && aggField != null) {
      query += "," + (isJpql ? aggField : "agg_field");
  ...
  private String createSumQuery(
      boolean isJson, MetaField metaField, MetaJsonField jsonField, boolean isJpql) {

    String sumField = null;
    if (isJson) {
      String type =
          isJpql
              ? filterJpqlService.getJsonJpql(jsonField)
      ...
      sumField = "self." + (isJpql ? metaField.getName() : filterSqlService.getColumn(metaField));
    }
  ...
  private String getGroup(
    ...
    if (isJson) {
      ...
      typeName = isJpql ? jsonField.getType() : filterSqlService.getSqlType(jsonField.getType());
      if (target != null) {
        object =
            isJpql
                ? filterJpqlService.parseJsonField(jsonField, target, joins, parent)
                : filterSqlService.parseJsonField(jsonField, target, joins, parent);
      }
    } else {
      group = isJpql ? metaField.getName() : filterSqlService.getColumn(metaField);
      typeName =
          isJpql ? metaField.getTypeName() : filterSqlService.getSqlType(metaField.getTypeName());
      if (target != null) {
        object =
            isJpql
                ? filterJpqlService.parseMetaField(metaField, target, joins, parent, true)
                : filterSqlService.parseMetaField(metaField, target, joins, parent, true);
      }
    }
    ...
    if (object != null) {
      String[] fieldArr =
          isJpql
              ? filterJpqlService.getJpqlField(object, parent.toString(), joins)
              : filterSqlService.getSqlField(object, parent.toString(), joins);
      typeName = fieldArr[1];
      group = fieldArr[0];
    }
  ...
  private void addSearchField(String filterMetaData) throws AxelorException {
    ...
    JSONObject jsonObj;
    try {
      jsonObj = new JSONObject(filterMetaData);
      JSONArray valArr = jsonObj.getJSONArray("values");

      int paramCnt = 1;
      for (Object valObj : valArr) {
        JSONObject valJsonObj = (JSONObject) valObj;
        paramCnt = this.computeRules(valJsonObj.getJSONArray("rules"), paramCnt);
      }
  ...
  private int computeRules(JSONArray rulesArr, int paramCnt) throws JSONException {
    for (Object ruleObj : rulesArr) {
      JSONObject ruleJsonObj = (JSONObject) ruleObj;
      if (ruleJsonObj.containsKey("rules")) {
        return this.computeRules(ruleJsonObj.getJSONArray("rules"), paramCnt);
      } else {
        if (ruleJsonObj.get("isField").toString().equals("param")) {
          JSONObject fieldObj = (JSONObject) ruleJsonObj.get("field");
          boolean isJson = fieldObj.containsKey("jsonField");

          String operator = ruleJsonObj.getString("operator");
          if (operator.equals("between") || operator.equals("notBetween")) {
            paramCnt = this.createJpqlSearchFields(fieldObj, isJson, paramCnt);
            paramCnt = this.createJpqlSearchFields(fieldObj, isJson, paramCnt);
  ...
  private int createJpqlSearchFields(JSONObject fieldObj, boolean isJson, int paramCnt)
      throws JSONException {

    String fieldStr = "param" + paramCnt;
    if (isJson) {
      fieldStr = this.getJpqlJsonSearchField(fieldStr, fieldObj);
    } else {
      fieldStr = this.getJpqlMetaSearchField(fieldStr, fieldObj);
    }
  ...
  private String getJpqlMetaSearchField(String fieldStr, JSONObject fieldObj) throws JSONException {
    String title = null;
    if (fieldObj.get("title") != null) {
      title = fieldObj.getString("title");
    }

    fieldStr = "<field name=\"" + fieldStr + "\" title=\"" + title;
    String fieldType = filterJpqlService.getJpqlSearchFieldType(fieldObj.getString("type"));

    if (!fieldObj.containsKey("target")) {
      fieldStr += "\" type=\"" + fieldType;
    } else {
      String target = fieldObj.getString("target");
      String targetName = fieldObj.getString("targetName");
      fieldStr +=
          "\" widget=\"ref-text\" type=\""
              + fieldType
              + "\" x-target-name=\""
  ...
  private String getJpqlJsonSearchField(String fieldStr, JSONObject fieldObj) throws JSONException {
    String title = "null";
    if (!fieldObj.get("title").equals("null")) {
      title = fieldObj.get("title").toString();
    }
    String fieldType = filterJpqlService.getJpqlSearchFieldType(fieldObj.getString("type"));

    fieldStr = "<field name=\"" + fieldStr + "\" title=\"" + title;

    if (fieldObj.containsKey("jsonTarget")) {
      String jsonModel = fieldObj.getString("jsonTarget");
      String targetName = fieldObj.getString("targetName");
      fieldStr +=
          "\" widget=\"ref-text\" type=\""
              + fieldType
    ...
    } else if (!fieldObj.containsKey("jsonTarget") && fieldObj.containsKey("target")) {
      String target = fieldObj.getString("target");
      String targetName = fieldObj.getString("targetName");
      fieldStr +=
          "\" widget=\"ref-text\" type=\""
  ...
  private String getTable(String model, boolean isJpql) {

    String[] models = model.split("\\.");
    MetaModel metaModel = metaModelRepo.findByName(models[models.length - 1]);

    if (metaModel != null) {
      return isJpql ? metaModel.getName() : metaModel.getTableName();
    }
