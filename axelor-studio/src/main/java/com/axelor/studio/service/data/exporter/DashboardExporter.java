...
public class DashboardExporter {
  ...
  public void export(ExporterService exporterService, String name) throws JAXBException {
    ...
    MetaView view =
        metaViewRepo.all().filter("self.type = 'dashboard'  and self.name = ?", name).fetchOne();
    ...
    if (view != null) {
      ...
      for (AbstractWidget widget : dashboard.getItems()) {
        String[] values =
            processDashlet((Dashlet) widget, view.getModule(), null, name, null, null);
        if (values != null) {
  ...
  private String[] addDashletAction(String[] values, Dashlet dashlet) {
    ...
    MetaAction metaAction =
        metaActionRepo.all().filter("self.name = ?1", dashlet.getAction()).fetchOne();
