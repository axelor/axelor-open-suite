...
class WkfNodeService {
  ...
  protected ActionGroup process() {
    ...
    if (!nodeActions.isEmpty()) {
      String actionName = "action-group-" + wkfService.dasherizeModel + "-wkf";
      String statusCondition =
          "__this__?." + statusField.getName() + " != __self__?." + statusField.getName();
      return this.wkfService.createActionGroup(actionName, nodeActions, statusCondition);
    }
  ...
  private String processNodes(MetaSelect metaSelect, MetaField statusField) {
    ...
    for (WkfNode node : nodeList) {
      ...
      if (node.getParentMenu() != null || node.getParentMenuBuilder() != null) {
  ...
  /**
   * Process WkfNode to create MenuBuilders from it if 'statusMenuEntry' or 'myMenuEntry' boolean is
   * true.
   *
   * @param node WkfNode to process.
   */
  private void addNodeMenu(WkfNode node, MetaField statusField) {
    ...
    if (parentMenu != null) {
      MetaMenu childMenu =
          metaMenuRepo
              .all()
              .filter("self.parent.name = ?1 and self.action is null", parentMenu.getName())
              .order("-order")
              .fetchOne();
      if (childMenu != null) {
      ...
      MenuBuilder menuBuilder =
          menuBuilderRepo
              .all()
              .filter("self.menuBuilder = ?1 and self.actionBuilder is not null", parentMenuBuilder)
              .order("-order")
              .fetchOne();
    ...
    if (node.getStatusMenuEntry()) {
      MenuBuilder menuBuilder =
          addMenuBuilder(
              node.getStatusMenuLabel(),
              menuName,
              domain,
              parentMenu,
              order,
              parentMenuBuilder,
              node.getWkf().getMetaModule());
      node.setStatusMenu(menuBuilder);
      order += 1;
    }

    if (node.getMyMenuEntry()) {
      domain += " AND self." + node.getMetaField().getName() + " = :__user__";
      MenuBuilder menuBuilder =
          addMenuBuilder(
              node.getMyMenuLabel(),
              "my-" + menuName,
   ...
   *
   * @param name Permission name.
   * @param node WkfNode containing roles.
   */
  //	@Transactional
   ...
   * @param domain Domain condition to set in action related to menu.
   * @param parent Parent menu name.
   * @param order Menu order.
   * @return MenuBuilder created or updated.
   */
  @Transactional
  public MenuBuilder addMenuBuilder(
      String title,
      String name,
      String domain,
      MetaMenu parentMenu,
      Integer order,
      MenuBuilder parentBuilder,
  ...
  private ActionBuilder getActionBuilder(
      MenuBuilder menuBuilder, MetaModule metaModule, String domain) {
    ...
    if (builder == null) {
      builder =
          actionBuilderRepo
              .all()
              .filter("self.metaModule = ?1 and self.name = ?2", metaModule, name)
              .fetchOne();
    }
