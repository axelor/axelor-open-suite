...
public class ModelCreatorService {
  ...
  //		} catch (IOException e) {
  //			e.printStackTrace();
  //			throw new AxelorException(TraceBackRepository.TYPE_TECHNICAL, I18n.get("Error in model
  // recording: %s"), e.getMessage());
  //		}
  //
  //	}
  //
  ...
  //
  //		List<MetaField> customFields = getCustomisedFields(metaModel);
  //
  ...
  //			throw new AxelorException(TraceBackRepository.CATEGORY_INCONSISTENCY, I18n.get("In model '%s'
  // field name '%s' is reserve word. "
  //					+ "Please use different one"),
  //					field.getMetaModel().getName(), name);
  //		}
  //
  //		if (field.getTrack()) {
  ...
  public List<MetaField> getCustomisedFields(MetaModel metaModel) {
    ...
    String query = "self.metaModel = ?1";

    List<MetaField> customFields =
        metaFieldRepo.all().filter(query, metaModel).order("id").fetch();
