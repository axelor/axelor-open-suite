...
public class ViewLoaderService {
  ...
  private void addFields(String model, List<AbstractWidget> items) {
    ...
    for (AbstractWidget abstractWidget : items) {
      ...
      MetaField metaField =
          metaFieldRepo
              .all()
              .filter("self.metaModel.fullName = ?1 and self.name = ?2", model, name)
              .fetchOne();
      if (metaField != null) {
  ...
  public ViewBuilder getViewBuilder(String module, String viewName, String type) {
    ...
    ViewBuilder viewBuilder =
        viewBuilderRepo
            .all()
            .filter(
                "self.metaModule.name = ?1 AND self.name = ?2 AND self.viewType = ?3",
                module,
                viewName,
                type)
            .fetchOne();
  ...
  public ViewBuilder getViewBuilderForm(
    ...
    ViewBuilder viewBuilder =
        viewBuilderRepo
            .all()
            .filter(
                "self.name = ?1 AND self.model = ?2 AND self.viewType = 'form' AND self.metaModule.name = ?3",
                formName,
                modelName,
                module)
            .fetchOne();
  ...
  public ViewBuilder addParentView(ViewBuilder viewBuilder, String parentModules, String name) {
    ...
    log.debug(
        "Search parent view for : {}, model: {}, parent modules: {}",
        name,
        viewBuilder.getMetaModel().getName(),
        parentModules);
    ...
    MetaView metaView =
        metaViewRepo
            ...
            .filter(
                ...
                name,
                viewBuilder.getMetaModel().getFullName(),
                modules)
            .fetchOne();
    ...
    if (metaView != null) {
      ...
      ViewBuilder parent =
          viewBuilderRepo
              .all()
              .filter(
                  "self.name = ?1 AND self.metaModel.id = ?2 AND self.metaModule.name in ?3",
                  name,
                  viewBuilder.getMetaModel().getId(),
                  modules)
              .fetchOne();
  ...
  private ViewBuilder addDefaultPanel(ViewBuilder viewBuilder) {
    ...
    List<MetaField> fields = modelBuilderService.getCustomisedFields(metaModel, false);
  ...
  private ViewBuilder addDefaultFields(ViewBuilder viewBuilder) {
    ...
    List<MetaField> fields = modelBuilderService.getCustomisedFields(model, false);
    modelBuilderService.sortFieldList(fields);
  ...
  /**
   * Get level for default panel to add in ViewBuilder. Default panel will be last panel in case of
   * extended view.
   *
   * @param viewBuilder ViewBuilder to search for panels.
