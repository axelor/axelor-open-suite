...
public class ActionBuilderService {
  ...
  public String build(String module, File viewDir, boolean updateMeta)
    ...
    for (String model : modelActionMap.keySet()) {
      ...
      if (!updateMeta) {
        ...
        viewBuilderService.writeView(viewDir, models[models.length - 1], null, actions);
      }
    }
  ...
  private List<ActionBuilder> getActionBuilders(String module, boolean updateMeta) {
    ...
    return actionBuilderRepo.all().filter(query, module).fetch();
  }

  @Transactional
  public void updateEdited(List<ActionBuilder> actionBuilders, boolean updateMeta) {
  ...
  private void processActionBuilder(Iterator<ActionBuilder> actionIter) {
    ...
    log.debug(
        "Processing action: {}, type: {}", actionBuilder.getName(), actionBuilder.getTypeSelect());
  ...
  private Action createActionRecord(MetaModel model, ActionBuilder actionBuilder, boolean create) {
    ...
    if (loopOnField != null) {
      ...
      createParentAction(modelName, name, loopOnField.getName(), expr, create);
    ...
    if (targetField != null) {
      ...
      actionRecord.setName(assign);
      createParentAction(modelName, name, targetField.getName(), expr, false);
    }
  ...
  private List<RecordField> createFields(ActionBuilder actionBuilder) {
    ...
    for (ActionBuilderLine line : actionBuilder.getLines()) {
      ...
      if (!Strings.isNullOrEmpty(condition) && !checkGroovy(condition, true)) {
      ...
      if (metaField == null) {
        log.debug("No meta field found for target: {}", line.getTargetField());
        continue;
      }
      String expr = getRecordExpression(target, line.getMetaField(), value, line.getFilter());
      if (!Strings.isNullOrEmpty(expr) && !checkGroovy(expr, true)) {
        addError(builderName, value, false);
        continue;
      }
      log.debug("Expr: {}", expr);
      if (expr != null
          && (expr.startsWith("call:") || expr.startsWith("action:") || expr.startsWith("eval:"))) {
  ...
  private String getRecordExpression(
      String[] target, MetaField metaField, String value, String filter) {
  ...
  private String getRelationalValue(String[] target, String value, String typeName, String filter) {

    target = Arrays.copyOfRange(target, 1, target.length);

    if (value == null && filter != null) {
      return "__repo__.of(" + typeName + ").all().filter(&quot;" + filter + "&quot;).fetchOne()";
    }
  ...
  private String getSumValue(String value, String filter) {
    ...
    if (!expr.isEmpty() && expr.contains(")")) {
      ...
      if (Character.isUpperCase(val[0].charAt(0))) {
        ...
        val[0] =
            val[0] + ".findAll{it->" + filter.replace("$$", "_parent.").replace("$", "it.") + "}";
      }
  ...
  private String getTypeName(String[] target, String typeName) {
    ...
    MetaField metaField =
        metaFieldRepo
            .all()
            .filter("self.name = ?1 and self.metaModel.name = ?2", target[0], typeName)
            .fetchOne();
  ...
  private void createParentAction(
      String model, String name, String field, String expr, boolean dummy) {
  ...
  private List<ViewBuilder> sortViewBuilders(
      final List<String> viewOrder, Set<ViewBuilder> viewBuilders) {

    ArrayList<ViewBuilder> builders = new ArrayList<ViewBuilder>();
    builders.addAll(viewBuilders);
    Collections.sort(
        builders,
        new Comparator<ViewBuilder>() {
  ...
  private ActionViewBuilder addDomainContext(
      ActionViewBuilder builder, ActionBuilder actionBuilder) {
  ...
  private String createLoopOnExpression(ActionBuilder actionBuilder, String action) {

    MetaField loopOnField = actionBuilder.getLoopOnField();
    String firstGroupBy = actionBuilder.getFirstGroupBy();
    String secondGroupBy = actionBuilder.getSecondGroupBy();
    StringBuilder builder = new StringBuilder("eval:" + loopOnField.getName());

    String filter = filterService.getGroovyFilters(actionBuilder.getFilters(), "it");
    ...
    if (firstGroupBy != null && secondGroupBy != null) {
      builder = updateMultiGroup(actionBuilder, builder, firstGroupBy, secondGroupBy);
    } else if (firstGroupBy != null) {
      builder = updateGroup(actionBuilder, builder, firstGroupBy);
    }

    builder.append("?.collect{__config__.action.execute('" + action + "', it, __this__)}");

    return builder.toString();
  }

  private StringBuilder updateMultiGroup(
      ActionBuilder actionBuilder,
      StringBuilder builder,
      String firstGroupBy,
      String secondGroupBy) {

    builder.append(".groupBy({it." + firstGroupBy + "},{it." + secondGroupBy + "})");
    ...
    for (ActionBuilderLine line : actionBuilder.getLines()) {
      ...
      if (value != null && !value.startsWith("$$")) {
        if (!firstGroupBy.equals(value) && !secondGroupBy.equals(value)) {
          builder.append(",'" + value + "' : y.collect{t->t." + value + "}.sum()");
        }
      }
    }
  ...
  private StringBuilder updateGroup(
      ActionBuilder actionBuilder, StringBuilder builder, String firstGroupBy) {
    ...
    for (ActionBuilderLine line : actionBuilder.getLines()) {
      ...
      if (value != null && !value.startsWith("$$")) {
        if (!firstGroupBy.equals(value)) {
          builder.append(",'" + value + "' : it.collect{t->t." + value + "}.sum()");
        }
      }
    }
    builder.append("]}");
  ...
  private Action createActionReport(MetaModel model, ActionBuilder actionBuilder) {
  ...
  private ActionViewBuilder processFilters(ActionViewBuilder builder, ActionBuilder actionBuilder) {
  ...
  private ActionMethod createActionEmail(MetaModel model, ActionBuilder builder) {
  ...
  public void generateMessage(long id, String model, String tag, String templateName)
      throws ClassNotFoundException, InstantiationException, IllegalAccessException,
          AxelorException, IOException {

    Template template = Beans.get(TemplateRepository.class).findByName(templateName);

    if (template != null) {
      Beans.get(TemplateMessageService.class).generateMessage(id, model, tag, template);
    }
  }
  ...
  private void addError(String builder, String expr, boolean condition) {
    ...
    if (condition) {
      errors +=
          String.format(
              I18n.get("Invalid condition action builder: %s") + " " + I18n.get(", condition %s"),
              builder,
              expr);
    } else {
      errors +=
          String.format(
              I18n.get("Invalid expression action builder: %s") + " " + I18n.get(", expression %s"),
              builder,
              expr);
    }
  }
