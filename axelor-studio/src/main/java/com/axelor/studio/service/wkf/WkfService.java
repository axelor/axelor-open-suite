...
public class WkfService {
  ...
  @Inject private WkfTrackingService trackingService;
   ...
   *
   * @param viewBuilder ViewBuilder linked with workflow.
   * @param navSelect Selection of widget type for 'wkfStatus' field.
   */
  ...
  /**
   * Method set 'clearWkf' boolean in ViewBuilder. Also call methods to remove wkf related buttons,
   * actions and status. Method call when related workflow deleted.
   *
   * @param viewBuilder ViewBuilder linked with deleted workflow.
  ...
  public String clearViewBuilder(ViewBuilder viewBuilder) {
    ...
    try {
      ...
      String actions =
          clearViewButtons(
              viewBuilder.getToolbar(), "action-" + modelName + "-wkf", new ArrayList<String>());
      ...
      if (onSave != null) {
        ...
        onSave = onSave.replace("save," + WkfTrackingService.ACTION_TRACK, "");
   ...
   *
   * @param viewButtons List of ViewButton to remove.
   * @param actionNames Comma separated names of actions related with buttons.
   * @param skipList Buttons names to skip from deleting
   * @return Comma separated names of actions related with button.
   */
  @Transactional
  public String clearViewButtons(
      List<ViewItem> viewButtons, String actionNames, List<String> skipList) {
    ...
    while (buttonIter.hasNext()) {
      ...
      if (viewButton.getWkfButton() && !skipList.contains(buttonName)) {
        ...
        if (onClick != null
            && !onClick.equals(WkfTrackingService.ACTION_OPEN_TRACK)
   ...
   *
   * @param viewBuilder ViewBuilder having 'wkfStatus' field.
   */
  @Transactional
  public void removeWkfStatus(ViewBuilder viewBuilder) {
    ...
    MetaField metaField =
        metaFieldRepo
            .all()
            .filter(
                "self.name = '" + WKF_STATUS + "' AND self.metaModel = ?1",
                viewBuilder.getMetaModel())
            .fetchOne();

    if (metaField != null) {
      List<ViewItem> viewFields =
          viewItemRepo
              .all()
              .filter("self.name = '" + WKF_STATUS + "' AND self.metaField = ?1", metaField)
              .fetch();
   ...
   *
   * @param actions String of comma separated action names.
   * @param action Name of action to add in actions string.
   * @param save Boolean to check if needs to add 'save' in actions string.
   * @return Updated actions with new action.
   */
  protected String getUpdatedActions(String actions, String action, boolean save) {
   ...
   *
   * @param viewBuilder ViewBuilder to check for button.
   * @param buttonName Name of button to search.
  ...
  /**
   * Method to find all edited workflow and process it. Also it clear all ViewBuilders having
   * 'clearWkf' boolean true.
   *
   * @return Result string in clearing views or processing workflow.
   */
  public String processWkfs() {

    List<ViewBuilder> clearWkfList = viewBuilderRepo.all().filter("self.clearWkf = true").fetch();
    String result = null;
    ...
    List<Wkf> processWkfs = wkfRepo.all().filter("self.edited = true").fetch();
   ...
   */
  @Transactional
  public void updateMetaAction(String actionName, String actionType, String xml) {
  ...
  protected ActionGroup createActionGroup(String name, List<String> actions, String condition) {
