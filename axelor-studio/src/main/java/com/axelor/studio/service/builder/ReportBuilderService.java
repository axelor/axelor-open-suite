...
public class ReportBuilderService {
  ...
  public void processReports() {
    ...
    List<ReportBuilder> builders =
        reportBuilderRepo.all().filter("self.edited = true and self.buttonView != null").fetch();
   ...
   *
   * @param viewBuilder ViewBuilder to add button.
   */
  @Transactional
  public void updateButtonView(Iterator<ReportBuilder> builderIter) {
    ...
    ViewItem viewButton =
        viewItemRepo
            .all()
            .filter(
                "self.name = ? and self.viewBuilderToolbar.id = ?", buttonName, viewBuilder.getId())
            .fetchOne();

    String actionName = "action-custom-print-" + inflector.dasherize(builderName);
    ...
    log.debug("Action builder searched: {}, action found: {}", actionName, actionBuilder);
