...
public class ReportPrinterService {
  ...
  private static final Pattern tablePattern =
      Pattern.compile("<table class=\"table table-bordered table-header\">(.*?</table>)");
  ...
  /**
   * Root method to access the service. It process template of reportBuilder and generate html from
   * it. It also process fileName of report builder.
   *
   * @param reportBuilder ReportBuilder to process;
   * @param recordId Id of record to process.
  ...
  public String[] getHtml(ReportBuilder reportBuilder, Long recordId) {
    ...
    try {
      ...
      Model entity = JPA.find((Class<Model>) Class.forName(model), recordId);
      ...
      fileName = fileName.replace("$TODAY$", LocalDate.now().toString("ddMMYYYY"));
      fileName = fileName.replace("$NOW$", LocalDateTime.now().toString("ddMMYYYYHHmm"));
  ...
  /**
   * Method find html template from given model. It also process o2m/m2m tables to match correct
   * syntax of template.
   *
   * @param model Model fullName to search template.
  ...
  private String getTemplate(ReportBuilder reportBuilder) {
    ...
    while (tableMatcher.find()) {
      ...
      if (fieldMatcher.find()) {
        ...
        if (rowMatcher.find()) {
          ...
          String rep = row.replace("$" + fieldName[0] + ".", "$item.");
          rep = "$" + fieldName[0] + ": {item | " + rep + "}$";
          String tableReplace = tableGroup.replace(row, rep);
          String modifiedTable = table.replace(tableGroup, tableReplace);
    ...
    if (reportBuilder.getFooter() != null) {
      template = "<div class=\"footer\">" + reportBuilder.getFooter() + "</div>" + template;
    }

    if (reportBuilder.getHeader() != null) {
      template = "<div class=\"header\">" + reportBuilder.getHeader() + "</div>" + template;
    }
