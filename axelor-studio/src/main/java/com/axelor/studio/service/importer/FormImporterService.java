...
import com.axelor.studio.service.creator.ActionCreatorService;
...
public class FormImporterService {
  ...
  private Map<String, List<ActionBuilder>> viewActionMap =
      new HashMap<String, List<ActionBuilder>>();
  ...
  @Inject private FormulaImporterService formulaImporter;
  ...
  public void importForm(
    ...
    String panelLevel = valMap.get(CommonService.PANEL_LEVEL);
  ...
  private ViewBuilder getViewBuilder(MetaModule module, MetaModel model, String[] name)
      throws AxelorException {
    ...
    if (viewBuilder == null) {
      viewBuilder =
          viewLoaderService.getViewBuilderForm(module.getName(), model, name[0], name[1], !replace);
    } else if (name[1] != null) {
  ...
  private String[] getViewName(String model, String module) {
    ...
    String viewName = valMap.get(CommonService.VIEW);
    String title = null;
  ...
  //				.all()
  //				.filter("self.sideBarViewBuilder = ?1 OR self.viewBuilder = ?1",
  ...
  private String getPanelLevel(String level, ViewBuilder viewBuilder, boolean side, boolean tab) {
  ...
  //			translationService.addTranslation(basic[3], valMap.get(CommonService.TITLE_FR),
  //					"fr", viewBuilder.getMetaModule().getName());
  //		}
  //
  ...
  //		if (addAttrs) {
  //
  //			panel.setIfModule(valMap.get(CommonService.IF_MODULE));
  //
  //			panel.setColspan(valMap.get(CommonService.COLSPAN));
  //
  //			String readonly = valMap.get(CommonService.READONLY);
  ...
  //			}
  //
  //			String hidden = valMap.get(CommonService.HIDDEN);
  ...
  //			}
  //
  //			panel.setShowIf(valMap.get(CommonService.SHOW_IF));
  //
  //			panel.setIfConfig(valMap.get(CommonService.IF_CONFIG));
  //
  //		}
  //
  ...
  //		ViewPanel panel = createPanelCommon(panelLevel, viewBuilder, basic, true);
  //
  //		panel.setSideBarViewBuilder(viewBuilder);
  //
  //		return viewPanelRepo.save(panel);
  //	}
  //
  //	@Transactional
  //	public ViewPanel createPanel(String panelLevel, ViewBuilder viewBuilder, String[] basic,
  //			boolean addAttrs) {
  //
  ...
  public void addButton(ViewBuilder viewBuilder, String[] basic) {
    ...
    //        basic[3], valMap.get(CommonService.TITLE_FR), "fr",
    // viewBuilder.getMetaModule().getName());

    if (basic[1] != null && basic[1].startsWith("toolbar")) {
      //      viewItem.setToolbarViewBuilder(viewBuilder);
    ...
    //    viewItem.setOnClick(
    //        ActionCreatorService.getUpdatedAction(
    //            viewItem.getOnClick(), valMap.get(CommonService.ON_CHANGE)));
    //
  ...
  @Transactional
  public ActionBuilder getActionViewBuilder(
      String name,
      String title,
      String viewName,
      ViewBuilder targetView,
      MetaModule module,
      MetaView metaView) {
    ...
    if (targetView != null) {
      ...
      abView.setViewName(targetView.getName());
      abView.setViewType(targetView.getViewType());
      builder.addActionBuilderView(abView);
      builder.setModel(metaModelRepo.findByName(targetView.getModel()).getFullName());
      actionBuilderRepo.save(builder);
    } else if (metaView != null) {
      ActionBuilderView abView = new ActionBuilderView();
      abView.setViewName(metaView.getName());
      abView.setViewType(metaView.getType());
      String model = metaView.getModel();
      if (model != null) {
        builder.setModel(
            metaModelRepo.all().filter("self.fullName = ?1", model).fetchOne().getFullName());
      }
      actionBuilderRepo.save(builder);
    } else {
  ...
  public void addWizard(String module, ViewBuilder viewBuilder, String[] basic)
    ...
    //        getActionViewBuilder(
    //            actionName, basic[3], viewName, targetView, viewBuilder.getMetaModule(), null);
  ...
  public void addField(ViewBuilder viewBuilder, MetaField metaField, String[] basic)
    ...
    if (metaField != null) {
      ...
      //      translationService.addTranslation(
      //          basic[3],
      //          valMap.get(CommonService.TITLE_FR),
      //          "fr",
      //          viewBuilder.getMetaModule().getName());
    ...
    //        ActionCreatorService.getUpdatedAction(
    //            viewItem.getOnChange(), valMap.get(CommonService.ON_CHANGE)));
    //    viewItem.setPanelLevel(valMap.get(CommonService.PANEL_LEVEL));
  ...
  //					I18n.get("No parent field found '%s' for nested editor"),
  //					parentField);
  //		}
  //
  //		viewItem.setNestedField(relationalField);
  //		Long sequence = viewItemRepo.all().filter("self.nestedField = ?1",
  //				relationalField).count();
  ...
  private void updateViewActionMap(ActionBuilder builder, String viewName) {
    ...
    log.debug("Updating view action map view: {} action: {}", viewName, builder.getName());
  ...
  public ViewBuilder clearView(ViewBuilder viewBuilder) {
    ...
    if (replace) {
      viewBuilder.setMetaViewGenerated(null);
      //      viewBuilder.setParentViewBuilder(null);
    }
  ...
  private ViewItem setEvent(ViewBuilder builder, ViewItem viewItem) {
    ...
    if (event != null) {
      log.debug(
          "Set Event for View item : {}, ViewItem type: {}, Event : {}",
          viewItem.getName(),
          //          viewItem.getTypeSelect(),
          event);
      //      if (viewItem.getTypeSelect() == 0) {
      //        String onChange = ActionCreatorService.getUpdatedAction(viewItem.getOnChange(),
      // event);
      //        viewItem.setOnChange(onChange);
      //      } else if (viewItem.getTypeSelect() == 1) {
      //        String onClick = ActionCreatorService.getUpdatedAction(viewItem.getOnClick(),
      // event);
  ...
  private ViewItem setCommonAttributes(ViewBuilder viewBuilder, ViewItem viewItem) {
    ...
    //      String required = valMap.get(CommonService.REQUIRED);
    ...
    //    viewItem.setShowIf(valMap.get(CommonService.SHOW_IF));
    //    viewItem.setIfModule(valMap.get(CommonService.IF_MODULE));
    //    viewItem.setIfConfig(valMap.get(CommonService.IF_CONFIG));
    //
    ...
    //        viewItem.setDomainCondition(valMap.get(CommonService.DOMAIN));
    //      }
    //      addFormula(viewBuilder, viewItem);
    //    }

    String colspan = valMap.get(CommonService.COLSPAN);

    if (StringUtils.isNumeric(colspan)) {
      //      viewItem.setColSpan(Integer.parseInt(colspan));
    }

    //    viewItem.setWidget(valMap.get(CommonService.WIDGET));
  ...
  public void addMenubar(ViewBuilder viewBuilder) throws AxelorException {
    ...
    if (title == null) {
      throw new AxelorException(
          ...
          I18n.get("Menubar must have title. Row: %s"),
          rowNum);
    }

    ViewItem viewItem = new ViewItem();
    //    viewItem.setTitle(title);
    //    viewItem.setMenubarViewBuilder(viewBuilder);
  ...
  public void addMenubarItem(ViewBuilder viewBuilder) throws AxelorException {
    ...
    if (title == null) {
      ...
      //      translationService.addTranslation(
      //          title, titleFr, "fr", viewBuilder.getMetaModule().getName());
    }

    if (title == null) {
      throw new AxelorException(
          TraceBackRepository.CATEGORY_CONFIGURATION_ERROR,
          I18n.get("Menubar item must have title. Row: %s"),
          rowNum);
    }

    List<ViewItem> menubar =
        viewItemRepo.all().filter("self.menubarBuilder = ?1", viewBuilder).fetch();

    if (menubar == null || menubar.isEmpty()) {
      throw new AxelorException(
          TraceBackRepository.CATEGORY_NO_VALUE,
          I18n.get("No menubar found for menubar item. Row: %s"),
          rowNum);
    }
    ...
    //    viewItem.setOnClick(valMap.get(CommonService.ON_CHANGE));
  ...
  private void createValidation(String type, ViewBuilder viewBuilder, MetaModel model) {

    List<String> pendingEvents = formulaImporter.importValidation(valMap, type, viewBuilder, model);
  ...
  private Map<String, String> addEvents(Map<String, String> fieldMap, String action, String event) {
    ...
    if (oldActions != null) {
      action = ActionCreatorService.getUpdatedAction(oldActions, action);
    }
  ...
  private void addFormula(ViewBuilder viewBuilder, ViewItem viewItem) {
    ...
    List<String> pendingEvents = formulaImporter.importFormula(valMap, viewBuilder, viewItem);
    if (pendingEvents.size() > 1) {
      updateEventMap(pendingEvents, viewBuilder.getId());
    }
  }
  ...
  //        viewBuilderRepo
  //            .all()
  //            .filter("self.name = ?1 and self.metaModule = ?2", viewName, metaModule)
  //            .fetchOne();
  //
  //    MetaView view = metaViewRepo.findByName(viewName);
  //
  //    ActionBuilder builder =
  //        getActionViewBuilder(basic[2], basic[3], viewName, targetView, metaModule, view);
  //    builder.setMetaModule(metaModule);
  //    String[] domainCtx = getDomainContext(valMap.get(CommonService.DOMAIN));
