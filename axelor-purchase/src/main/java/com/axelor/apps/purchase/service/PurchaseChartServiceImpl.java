...
public class PurchaseChartServiceImpl implements PurchaseChartService {
  @Inject MetaViewRepository metaViewRepo;
  @Inject AppBaseService appBaseService;
  @Inject UserService userService;
  @Inject BaseChartService baseChartService;

  protected static final String QUERY_PATTERN = "(?i)(select)(.+?)(_percent)";
  ...
  protected String getQueryStr(Map<String, Object> context, ChartView chartView) {
    ...
    String whereFilter = String.format("(%s = :chartFilterValue)", chartConfigs.get("_filterKey"));
    String queryStr =
        StringUtils.normalizeSpace(chartView.getDataSet().getText().trim().replaceAll("\\n", " "))
            .replaceFirst(QUERY_PATTERN, String.format("$1 %s.id", "self"))
            .replaceAll(baseChartService.QUERY_ORDER_BY_PATTERN, " ")
            .replaceAll(
                baseChartService.QUERY_GROUP_BY_PATTERN,
                String.format("AND %s $1 %s.id", whereFilter, "self"))
            .replaceAll(
