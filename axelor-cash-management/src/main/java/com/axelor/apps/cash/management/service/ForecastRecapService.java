...
public class ForecastRecapService {
  ...
  @Inject protected ReportFactory reportFactory;
  ...
  public void populateWithSalaries(ForecastRecap forecastRecap) {
    ...
    while (!itDate.isAfter(forecastRecap.getToDate())) {
      if (itDate.isEqual(
          new LocalDate(
              itDate.getYear(), itDate.getMonthOfYear(), itDate.dayOfMonth().getMaximumValue()))) {
  ...
  public String getURLForecastRecapPDF(ForecastRecap forecastRecap) throws AxelorException {
    ...
    try {
      language =
          forecastRecap.getCompany().getPrintingSettings().getLanguageSelect() != null
    ...
    return reportFactory
        .createReport(IReport.FORECAST_RECAP, title + "-${date}")
