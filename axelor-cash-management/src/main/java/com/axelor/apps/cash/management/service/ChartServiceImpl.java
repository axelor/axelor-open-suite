...
public class ChartServiceImpl implements ChartService {
  ...
  @Inject protected ForecastRecapRepository forecastRepo;
  @Inject protected ForecastRecapLineRepository forecastLineRepo;
  ...
  public List<Map<String, Object>> getCashBalanceData(User user, BankDetails bankDetails) {
    ...
    ForecastRecap recap =
        forecastRepo
            .all()
            .filter(
                "self.isUseForReport = true AND (self.user = :user OR :user is null) AND (:bankDetails is null OR :bankDetails MEMBER OF self.bankDetailsSet)")
            .bind("user", user)
