	...
	public void validate(StockMoveLinePostRequest stockMoveLinePostRequest, StockMove stockMove) {
		...
		if (stockMove != null && isLocationOnStockMoveLine) {
			...
			if (typeSelect.equals(StockMoveRepository.TYPE_INCOMING) && toStockLocationId == null) {
				throw new BadRequestException(String.format(StockExceptionMessage.REST_STOCK_MOVE_LINE_STOCK_LOCATION_REQUIRED, "toStockLocationId"));
			}
			
			if (typeSelect.equals(StockMoveRepository.TYPE_OUTGOING) && fromStockLocationId == null) {
				throw new BadRequestException(String.format(StockExceptionMessage.REST_STOCK_MOVE_LINE_STOCK_LOCATION_REQUIRED, "fromStockLocationId"));
			}
			if (typeSelect.equals(StockMoveRepository.TYPE_INTERNAL) && (toStockLocationId == null || fromStockLocationId == null)) {
				throw new BadRequestException(String.format(StockExceptionMessage.REST_STOCK_MOVE_LINE_STOCK_LOCATION_REQUIRED, "fromStockLocationId", "toStockLocationId"));
			}
		}
