...
public class StockMoveLineServiceImpl implements StockMoveLineService {
  ...
  public StockMoveLine createStockMoveLine(
    ...
    if (product != null) {
      ...
      if (taxed) {
        unitPriceTaxed =
            unitPrice.setScale(
                generalService.getNbDecimalDigitForUnitPrice(), RoundingMode.HALF_UP);
        unitPriceUntaxed =
            unitPrice.divide(
  ...
  public void generateTrackingNumber(
    ...
    while (stockMoveLine.getQty().compareTo(trackingNumberConfiguration.getSaleQtyByTracking())
        == 1) {
  ...
  public List<? extends LocationLine> getLocationLines(Product product, Location location)
    ...
    List<? extends LocationLine> locationLineList =
        Beans.get(LocationLineRepository.class)
            .all()
            .filter(
                "self.product = ?1 AND self.futureQty > 0 AND self.trackingNumber IS NOT NULL AND self.detailsLocation = ?2"
  ...
  public void updateLocations(
    ...
    switch (fromStatus) {
      case StockMoveRepository.STATUS_PLANNED:
        locationLineService.updateLocation(
            fromLocation, product, qty, false, true, true, null, trackingNumber);
        locationLineService.updateLocation(
            toLocation, product, qty, false, true, false, null, trackingNumber);
        break;

      case StockMoveRepository.STATUS_REALIZED:
        locationLineService.updateLocation(
            fromLocation, product, qty, true, true, true, null, trackingNumber);
        locationLineService.updateLocation(
            toLocation, product, qty, true, true, false, null, trackingNumber);
        break;
    ...
    switch (toStatus) {
      case StockMoveRepository.STATUS_PLANNED:
        locationLineService.updateLocation(
            ...
            trackingNumber);
        locationLineService.updateLocation(
            toLocation, product, qty, false, true, true, lastFutureStockMoveDate, trackingNumber);
        break;

      case StockMoveRepository.STATUS_REALIZED:
        locationLineService.updateLocation(
            fromLocation, product, qty, true, true, false, null, trackingNumber);
        locationLineService.updateLocation(
            toLocation, product, qty, true, true, true, null, trackingNumber);
        break;
