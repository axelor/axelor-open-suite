...
public class MassStockMoveResponse extends ResponseStructure {
  ...
  private final List<Long> stockMoveLineIdsfetched;

  public MassStockMoveResponse(MassStockMove massStockMove, List<Long> stockMoveLineIdsfetched) {
    ...
    this.productToMoveList =
        massStockMove.getProductToMoveList() != null
            ? massStockMove.getProductToMoveList().stream()
                .map(MassStockMoveNeed::getId)
                .collect(Collectors.toList())
            : new ArrayList<>();
    this.stockMoveLineIdsfetched =
        stockMoveLineIdsfetched != null ? stockMoveLineIdsfetched : new ArrayList<>();
  }
  ...
  public List<Long> getStockMoveLineIdsfetched() {
    return stockMoveLineIdsfetched;
  }
