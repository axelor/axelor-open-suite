...
public class LocationController {
  ...
  public void checkIsDefaultLocation(ActionRequest request, ActionResponse response) {
    ...
    if (location != null
        && location.getIsDefaultLocation()
        && location.getCompany() != null
        && location.getTypeSelect() != null) {

      Location findLocation =
          locationRepo
              .all()
              .filter(
                  "company = ? and typeSelect = ? and isDefaultLocation = ?",
                  location.getCompany(),
                  location.getTypeSelect(),
                  location.getIsDefaultLocation())
              .fetchOne();
      ...
      if (location.getName() != null) {
        title =
            lstSelectedLocations == null
                ? I18n.get("Location") + " " + location.getName()
                : I18n.get("Location(s)");
      }
