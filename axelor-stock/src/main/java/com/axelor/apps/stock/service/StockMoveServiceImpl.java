...
public class StockMoveServiceImpl implements StockMoveService {
  ...
  public String getSequenceStockMove(int stockMoveType, Company company) throws AxelorException {
    ...
    switch (stockMoveType) {
      case StockMoveRepository.TYPE_INTERNAL:
        ...
        if (ref == null) {
          throw new AxelorException(
              String.format(I18n.get(IExceptionMessage.STOCK_MOVE_1), company.getName()),
      ...
      case StockMoveRepository.TYPE_INCOMING:
        ...
        if (ref == null) {
          throw new AxelorException(
              String.format(I18n.get(IExceptionMessage.STOCK_MOVE_2), company.getName()),
              IException.CONFIGURATION_ERROR);
        }
        break;
      ...
      case StockMoveRepository.TYPE_OUTGOING:
        ...
        if (ref == null) {
          throw new AxelorException(
              String.format(I18n.get(IExceptionMessage.STOCK_MOVE_3), company.getName()),
              IException.CONFIGURATION_ERROR);
        }
        break;
      ...
      default:
        throw new AxelorException(
            String.format(I18n.get(IExceptionMessage.STOCK_MOVE_4), company.getName()),
            IException.CONFIGURATION_ERROR);
    }
  ...
  public int getStockMoveType(Location fromLocation, Location toLocation) {
    ...
    if (fromLocation.getTypeSelect() == LocationRepository.TYPE_INTERNAL
        && toLocation.getTypeSelect() == LocationRepository.TYPE_INTERNAL) {
      return StockMoveRepository.TYPE_INTERNAL;
    } else if (fromLocation.getTypeSelect() != LocationRepository.TYPE_INTERNAL
        && toLocation.getTypeSelect() == LocationRepository.TYPE_INTERNAL) {
  ...
  public void plan(StockMove stockMove) throws AxelorException {
    ...
    LOG.debug(
        "Plannification du mouvement de stock : {} ", new Object[] {stockMove.getStockMoveSeq()});
    ...
    if (fromLocation == null) {
      throw new AxelorException(
          String.format(I18n.get(IExceptionMessage.STOCK_MOVE_5), stockMove.getName()),
          IException.CONFIGURATION_ERROR);
    }
    if (toLocation == null) {
      throw new AxelorException(
          String.format(I18n.get(IExceptionMessage.STOCK_MOVE_6), stockMove.getName()),
          IException.CONFIGURATION_ERROR);
    }
  ...
  public String realize(StockMove stockMove) throws AxelorException {
    ...
    stockMoveRepo.save(stockMove);
    if (!stockMove.getIsWithBackorder() && !stockMove.getIsWithReturnSurplus()) return null;
    ...
    if (stockMove.getIsWithReturnSurplus() && this.mustBeSplit(stockMove.getStockMoveLineList())) {
      ...
      if (newStockSeq != null) newStockSeq = newStockSeq + " " + newStockMove.getStockMoveSeq();
      else newStockSeq = newStockMove.getStockMoveSeq();
    }
  ...
  public Long splitInto2(Long originalStockMoveId, List<StockMoveLine> stockMoveLines) {
    ...
    StockMove newStockMove =
        Beans.get(StockMoveManagementRepository.class).copy(originalStockMove, true);
    ...
    for (StockMoveLine moveLine : stockMoveLines) {
      if (BigDecimal.ZERO.compareTo(moveLine.getQty()) == 0) {
        ...
        StockMoveLine currentOriginalStockMoveLine =
            originalStockMove.getStockMoveLineList().get(lineNumber);
