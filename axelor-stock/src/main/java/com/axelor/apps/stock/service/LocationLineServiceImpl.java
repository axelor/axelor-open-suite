...
public class LocationLineServiceImpl implements LocationLineService {
  ...
  @Inject protected LocationLineRepository locationLineRepo;

  @Inject protected MinStockRulesService minStockRulesService;
  ...
  public void updateLocation(
    ...
    this.updateLocation(
        location, product, qty, current, future, isIncrement, lastFutureStockMoveDate);
  ...
  public void updateLocation(
    ...
    locationLine =
        this.updateLocation(
            locationLine, qty, current, future, isIncrement, lastFutureStockMoveDate);
  ...
  public void minStockRules(
      Product product, BigDecimal qty, LocationLine locationLine, boolean current, boolean future)
      throws AxelorException {

    if (current) {
      minStockRulesService.generatePurchaseOrder(
          product, qty, locationLine, MinStockRulesRepository.TYPE_CURRENT);
    }
    if (future) {
      minStockRulesService.generatePurchaseOrder(
          product, qty, locationLine, MinStockRulesRepository.TYPE_FUTURE);
    }
  }
  ...
  public void updateDetailLocation(
    ...
    detailLocationLine =
        this.updateLocation(
            detailLocationLine, qty, current, future, isIncrement, lastFutureStockMoveDate);
  ...
  public void checkStockMin(LocationLine locationLine, boolean isDetailLocationLine)
      throws AxelorException {
    if (!isDetailLocationLine
        && locationLine.getCurrentQty().compareTo(BigDecimal.ZERO) == -1
        && locationLine.getLocation().getTypeSelect() == LocationRepository.TYPE_INTERNAL) {
      throw new AxelorException(
          String.format(
              I18n.get(IExceptionMessage.LOCATION_LINE_1),
              locationLine.getProduct().getName(),
              locationLine.getProduct().getCode()),
          IException.CONFIGURATION_ERROR);
    } else if (isDetailLocationLine
        && locationLine.getCurrentQty().compareTo(BigDecimal.ZERO) == -1
        && ((locationLine.getLocation() != null
                && locationLine.getLocation().getTypeSelect() == LocationRepository.TYPE_INTERNAL)
            || (locationLine.getDetailsLocation() != null
                && locationLine.getDetailsLocation().getTypeSelect()
  ...
  public LocationLine updateLocation(
  ...
  public LocationLine getLocationLine(Location location, Product product) {
    ...
    LOG.debug(
        ...
        new Object[] {
          locationLine.getLocation().getName(),
          product.getCode(),
          locationLine.getCurrentQty(),
          locationLine.getFutureQty(),
          locationLine.getLastFutureStockMoveDate()
        });
  ...
  /**
   * Récupération de la ligne détaillée de stock : On vérifie si l'entrepot contient une ligne
   * détaillée de stock pour un produit, une vairante de produit et un numéro de suivi donnés. Si
   * l'entrepot ne contient pas de ligne détaillée de stock pour ces paramètres, alors on vérifie
   * que et on créé une ligne détaillée de stock.
   *
   * @param detailLocation Entrepot détaillé
   * @param product Produit concerné
   * @param trackingNumber Le numéro de suivi concerné
   * @return Une ligne détaillée de stock
   */
  public LocationLine getDetailLocationLine(
      Location detailLocation, Product product, TrackingNumber trackingNumber) {

    LocationLine detailLocationLine =
        this.getDetailLocationLine(
            detailLocation.getDetailsLocationLineList(), product, trackingNumber);
    ...
    LOG.debug(
        ...
        new Object[] {
          detailLocationLine.getDetailsLocation().getName(),
          product.getCode(),
   ...
   *
   * @param locationLineList Une liste de ligne de stock
  ...
  /**
   * Permet de récupérer la ligne détaillée de stock d'un entrepot en fonction d'un produit, d'une
   * variante de produit et d'un numéro de suivi donnés.
   *
   * @param detailLocationLineList Une liste de ligne détaillée de stock
   * @param product Un produit
   * @param trackingNumber Un numéro de suivi
   * @return Un ligne de stock
   */
  public LocationLine getDetailLocationLine(
      List<LocationLine> detailLocationLineList, Product product, TrackingNumber trackingNumber) {
  ...
  public LocationLine createLocationLine(Location location, Product product) {
    ...
    LOG.debug(
        ...
        new Object[] {location.getName(), product.getCode()});
  ...
  /**
   * Permet de créer une ligne détaillée de stock pour un entrepot, un produit, une variante de
   * produit et un numéro de suivi donnés.
   *
   * @param location Un entrepot
   * @param product Un produit
   * @param trackingNumber Un numéro de suivi
   ...
   */
  public LocationLine createDetailLocationLine(
      Location location, Product product, TrackingNumber trackingNumber) {

    LOG.debug(
        "Création d'une ligne de détail de stock : Entrepot? {}, Produit? {}, Num de suivi? {} ",
        new Object[] {
          location.getName(), product.getCode(), trackingNumber.getTrackingNumberSeq()
        });
