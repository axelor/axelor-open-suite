      ...
      if (request.getContext().getParent().get("id") != null)
        response.setView(
            ActionView.define(I18n.get("Stock Move Lines"))
                ...
                .domain(
                    "self.product.id = :id AND (self.stockMove.stockMoveLineList.fromStockLocation.id = :locationId OR self.stockMove.stockMoveLineList.toStockLocation.id = :locationId) AND self.stockMove.statusSelect != :status AND (self.stockMove.estimatedDate <= :stockDate OR self.stockMove.realDate <= :stockDate)")
