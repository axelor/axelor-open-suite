...
public class LocationServiceImpl implements LocationService {
  ...
  @Inject LocationRepository locationRepo;

  @Inject LocationLineService locationLineService;
  ...
  public Location getDefaultLocation() {
    return locationRepo
        .all()
        .filter(
            "self.isDefaultLocation = true AND self.typeSelect = ?1",
  ...
  public List<Location> getInternalLocations() {
    return locationRepo
        .all()
        .filter("self.typeSelect = ?1", LocationRepository.TYPE_INTERNAL)
        .fetch();
  }
  ...
  public BigDecimal getQty(Long productId, Long locationId, String qtyType) {
    if (productId != null) {
      if (locationId == null) {
        ...
        if (!locations.isEmpty()) {
          ...
          for (Location location : locations) {
            LocationLine locationLine =
                locationLineService.getLocationLine(
                    locationRepo.find(location.getId()), productRepo.find(productId));
        ...
        LocationLine locationLine =
            locationLineService.getLocationLine(
                locationRepo.find(locationId), productRepo.find(productId));
