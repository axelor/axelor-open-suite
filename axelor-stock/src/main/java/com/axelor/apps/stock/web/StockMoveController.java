...
public class StockMoveController {
  ...
  public void printStockMove(ActionRequest request, ActionResponse response)
    ...
    if (!stockMoveIds.equals("")) {
      ...
      try {
        language =
            stockMove.getPartner().getLanguageSelect() != null
                ? stockMove.getPartner().getLanguageSelect()
                : stockMove.getCompany().getPrintingSettings().getLanguageSelect() != null
                    ? stockMove.getCompany().getPrintingSettings().getLanguageSelect()
                    : "en";
  ...
  public void viewDirection(ActionRequest request, ActionResponse response) {
    ...
    String msg = "";
    if (fromAddress == null) fromAddress = stockMove.getCompany().getAddress();
    if (toAddress == null) toAddress = stockMove.getCompany().getAddress();
    if (fromAddress == null || toAddress == null) msg = I18n.get(IExceptionMessage.STOCK_MOVE_11);
    ...
    if (msg.isEmpty()) {
      ...
      if (result != null) {
        ...
        response.setFlash(
            String.format(I18n.get(IExceptionMessage.STOCK_MOVE_13), dString, aString));
  ...
  public void splitStockMoveLinesSpecial(ActionRequest request, ActionResponse response) {
    ...
    Boolean selected =
        stockMoveService.splitStockMoveLinesSpecial(stockMoveLines, new BigDecimal(splitQty));
    if (!selected) {
  ...
  public void splitInto2(ActionRequest request, ActionResponse response) {
    StockMove stockMove = request.getContext().asType(StockMove.class);
    Long newStockMoveId =
        stockMoveService.splitInto2(stockMove.getId(), stockMove.getStockMoveLineList());
